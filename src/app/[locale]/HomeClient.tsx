...
export default function HomeClient({ articles, locale }: HomeClientProps) {
  ...
  useEffect(() => {
    if (sessionStorage.getItem('gemilab-hero-seen')) {
      setSkipAnim(true);
    } else {
      sessionStorage.setItem('gemilab-hero-seen', '1');
    }
    setMounted(true);
  }, []);
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 16 }}>
          {CATEGORIES.map((cat, i) => {
            ...
            return (
              ...
              >
                <div style={{ width: 40, height: 40, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, marginBottom: 16, color: cat.color, transition: "transform 0.3s ease", transform: hoveredCat === cat.id ? "translateX(4px)" : "translateX(0)", lineHeight: 1 }}>
                  {cat.icon}
                </div>
