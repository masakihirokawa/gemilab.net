...
const META = {
  ja: {
    title: "利用規約 — Gemini Lab",
    description: "Gemini Lab の利用規約。コンテンツの利用条件、著作権、メンバーシップ、免責事項について説明します。",
  },
  en: {
    title: "Terms of Use — Gemini Lab",
    description: "Gemini Lab Terms of Use. Content usage, copyright, membership terms, and disclaimers.",
  },
};
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      title: m.title,
      description: m.description,
      images: [{ url: "https://gemilab.net/og/default.png", width: 1200, height: 1200 }],
    },
    alternates: {
      canonical: locale === "ja" ? "https://gemilab.net/terms" : "https://gemilab.net/en/terms",
      languages: {
        ja: "https://gemilab.net/terms",
        en: "https://gemilab.net/en/terms",
        "x-default": "https://gemilab.net/en/terms",
      },
    },
  };
}
  ...
  return (
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "40px 24px 120px" }}>
      ...
      <p style={{ fontSize: 14, color: "var(--text-dim)", marginBottom: 36, lineHeight: 1.6 }}>
        {isJa
          ? "本規約は、Gemini Lab（以下「当サイト」）のご利用にあたっての条件を定めるものです。当サイトをご利用いただいた時点で、本規約に同意いただいたものとみなします。"
          : "These terms govern your use of Gemini Lab (\"this site\"). By accessing or using this site, you agree to be bound by these terms."}
      </p>
...
function JaContent({ prefix }: { prefix: string }) {
  return (
    <>
      <h2>サイトの性質</h2>
      <p>
        Gemini Lab は Google の非公式な情報サイトです。当サイトのコンテンツは情報提供を目的としており、Google が公式に認定・推奨するものではありません。最新かつ正確な情報については、Google の公式ドキュメントをご確認ください。
      </p>
      ...
      <p>
        Gemini、Google 等の商標はそれぞれの権利者に帰属します。当サイトでのこれらの商標の使用は、情報提供の目的に限定されるものであり、権利者との提携・推奨関係を示すものではありません。
      </p>
...
function EnContent({ prefix }: { prefix: string }) {
  return (
    <>
      ...
      <p>
        Gemini Lab is an unofficial information site and is not endorsed by or affiliated with Google. The content on this site is provided for informational purposes only. For the most up-to-date and accurate information, please refer to Google&apos;s official documentation.
      </p>
      ...
      <p>
        Trademarks such as Gemini and Google belong to their respective owners. Use of these trademarks on this site is for informational purposes only and does not imply any affiliation or endorsement.
      </p>
