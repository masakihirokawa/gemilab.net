...
本記事では、Gemini 2.5 Flash を使って、受信メールの自動分類・要約・アクション抽出を行うシステムをTypeScriptで実装する方法を解説します。
...
**Q: メールの内容がプライバシー的に問題になりませんか？**

Gemini API は送信データをモデルのトレーニングに使用しないポリシーを採用しています（Google Cloud の利用規約に準拠）。ただし、センシティブな情報を扱う場合は、メール本文を要約やキーワードに変換してから API に送信する前処理を追加することを推奨します。

**Q: Function Calling を使わずにJSON出力だけではダメですか？**

シンプルなプロトタイプではJSON出力でも十分ですが、本番環境では Function Calling を推奨します。enum でカテゴリの選択肢を制限できるため、想定外の値が返るリスクを大幅に低減できます。

**Q: どのくらいの処理速度が出ますか？**

Gemini 2.5 Flash は1メールあたり約1〜2秒で分類を完了します。同時実行数5のバッチ処理なら、100通のメールを約40秒で分類可能です。

**Q: 分類精度を上げるにはどうすればよいですか？**

カスタムルールの追加が最も効果的です。特に、urgentSenders（重要な送信者リスト）や customPromptRules（業務固有のルール記述）を充実させることで、ビジネスコンテキストに沿った分類精度が向上します。

## まとめ

Gemini API の Function Calling を活用することで、受信メールを自動的に分類・要約・アクション抽出するシステムを効率よく構築できます。スキーマを厳密に定義することで型安全なデータフローが実現でき、カスタムルールの追加でビジネスニーズに柔軟に対応できる設計です。

AI を使った業務自動化の全体像をさらに深めたい方には「[AI時代の業務自動化](https://www.amazon.co.jp/dp/4297145758/?tag=pinocchio-22)」がおすすめです。
