...
この記事では、その閾値を一段下げるための選択肢として sqlite-vec を紹介します。sqlite-vec は SQLite の拡張機能として動くベクトル検索エンジンで、アプリのプロセス内にそのまま埋め込めます。外部サービスを立てる必要がなく、バックアップは SQLite ファイルをコピーするだけ、デプロイ先も安価な VPS やコンテナで済みます。Gemini の埋め込み API と組み合わせれば、手元のノートを意味検索できる基盤を数時間で作り上げられます。
