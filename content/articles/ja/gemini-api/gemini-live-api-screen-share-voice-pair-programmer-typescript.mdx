...
highlights:
  - "Gemini Live API のドキュメントを読んでもストリーミング方式と画面キャプチャの統合で詰まっていた人が、動く TypeScript 実装を今日手に入れられる"
  - "getDisplayMedia・AudioWorklet・WebSocket・Function Calling を組み合わせ、画面の状態を読み取りながら音声で応答する完全実装パターンを習得できる"
  - "個人開発で孤独になりがちな実装時間に『第二の頭脳』として AI ペアを置き、設計判断と実装速度の両方を底上げする運用設計までを身につけられる"
---
...
公式の [Live API ガイド](https://ai.google.dev/gemini-api/docs/live) はバラバラの API リファレンスとしては優れているのですが、「ブラウザでの画面共有」「音声の双方向化」「コストの抑え方」「Function Calling での編集補助」を一つの完成形として組み立てる手順は、自分で繋ぎ込む必要があります。本稿では、私が実装の中で何度か手戻りしたポイントも含め、最短経路を共有します。
...
Function Calling 自体の詳しい挙動は [Gemini API の Function Calling 完全実装ガイド](/articles/gemini-api/gemini-api-function-calling-complete-guide) で踏み込んでいるので、応答スキーマの細かい話はそちらに譲ります。実装で重要なのは、**ツール結果をモデルへ即時に返すこと** です。返さないと、Gemini は「自分が呼んだツールの結果を待っている状態」のまま音声を止めるため、会話が不自然に途切れます。
...
技術書としては、Live API のような双方向ストリーミング設計の考え方は [『リアルタイム Web の本：WebSocket・WebRTC・SSE』（中村優介 著）](https://www.amazon.co.jp/dp/4297139383/?tag=pinocchio-22) の WebSocket 章が、状態管理の設計判断の参考になりました。Gemini に限らず双方向プロトコル全般の落とし穴が体系化されています。
