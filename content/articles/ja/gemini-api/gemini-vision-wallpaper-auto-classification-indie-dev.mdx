...
author: "Gemini Lab"
description: "壁紙アプリの画像カテゴリ自動分類を Gemini Vision で実装した記録。精度67%から87%への改善手順と、confidence 閾値で人間レビューを全体の15%に絞り込む運用設計を紹介します。"
tags: ["gemini", "gemini-api", "vision", "multimodal", "python", "image-classification", "indie-dev"]
---
壁紙アプリを運営していると、画像の仕分け作業に想像以上の時間を取られます。個人開発で運用しているアプリでも、カテゴリ管理は長年の悩みの種でした。
...
## confidence 閾値をどう決めたか — 検証セットの集計スクリプト

「70未満は人間が見る」という閾値は、勘で置いた数字ではありません。分類済みの500枚に対して正解ラベルを手で付け直し、confidence を10刻みのビンに分けて正解率を出したうえで決めています。
...
def confidence_report(results_json: str, truth_json: str) -> None:
    """confidence を10刻みにビン分けし、ビンごとの正解率と件数を出力する"""
    results = json.load(open(results_json))
    truth = json.load(open(truth_json))  # {filename: 正解カテゴリ}
    ...
    for b in sorted(bins):
        total, correct = bins[b]["total"], bins[b]["correct"]
        acc = correct / total * 100 if total else 0
        cumulative_reviewed += total
        print(f"{b:>3}-{b+9:<3} n={total:>3}  acc={acc:5.1f}%  "
              f"ここまで累計={cumulative_reviewed:>3}枚")

confidence_report("classified.json", "ground_truth.json")
```

500枚を実際にこのスクリプトに通した結果が下表です。

<table>
<thead>
<tr><th>confidence</th><th>件数</th><th>正解率</th><th>備考</th></tr>
</thead>
<tbody>
<tr><td>0–59</td><td>21</td><td>38.1%</td><td>ほぼ人間が見る前提</td></tr>
<tr><td>60–69</td><td>52</td><td>59.6%</td><td>誤分類が半数近く</td></tr>
<tr><td>70–79</td><td>88</td><td>81.8%</td><td>実用の下限</td></tr>
<tr><td>80–89</td><td>147</td><td>92.5%</td><td>自動採用で問題なし</td></tr>
<tr><td>90–100</td><td>192</td><td>97.4%</td><td>ほぼ確定</td></tr>
</tbody>
</table>

60–69 と 70–79 の間で正解率が22ポイント跳ねます。ここが閾値の置きどころでした。60に下げれば人間の確認は減りますが、誤分類が4割混じったまま公開されます。80に上げると確認対象が32%まで膨らみ、自動化の意味が薄れます。**70という数字は、精度と手間の折り合いが最も良かった地点として観測から出てきたもの**です。

この集計は一度やれば終わりではありません。カテゴリ定義を書き換えたりモデルを更新したりすると分布は動きます。私自身、プロンプトを変更するたびに同じ500枚で再集計し、閾値を置き直すようにしています。手間はかかりますが、閾値を放置したまま運用を続けるより、はるかに安心して自動採用の範囲を広げられます。
...
画像管理に Gemini Vision を組み込んでから2ヶ月ほど経ちます。最初は「機械に仕分けを任せていいのか」という違和感がありましたが、実際に使ってみると、AIが分類に迷う画像は人間が見ても迷う画像であることが多く、「ここは人間が判断する」という線引きが自然にできてきました。

私はこの「全部を任せない」設計を好んでいます。完全自動化を目標にするのではなく、AIが得意な「明確な事例の高速処理」と、人間が得意な「境界事例の文脈判断」を組み合わせることが、個人開発者の限られた時間を有効に使う現実的な方法だと感じています。
