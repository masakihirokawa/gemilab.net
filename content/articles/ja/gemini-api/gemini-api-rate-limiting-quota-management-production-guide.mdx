...
この記事では、Gemini APIのクォータ体系の正確な理解から始まり、指数バックオフ、アダプティブレート制御、マルチAPIキープール、そしてCloud Monitoringを使ったリアルタイム監視まで、**本番環境で必要なすべての実装パターン**を体系的に解説します。
...
対処: リクエスト間隔を広げる、またはリクエストをキューイングする
...
対処: コンテキストを短くする、またはリクエストの頻度を下げる
...
レート制限への最初の対策として、適切なリトライ戦略を実装します。単純な固定間隔リトライは「全クライアントが同時にリトライ」する thundering herd 問題を引き起こすため、**指数バックオフ + Jitter（ランダム揺らぎ）** の組み合わせが必須です。
...
Google APIのレスポンスには `Retry-After` ヘッダーが含まれる場合があります。これを活用することで、不必要な待機時間を避けられます。
...
A. 個人が複数のGoogleアカウントのキーを使うことは規約上グレーゾーンになり得ます。企業の場合は Google Cloud の IAM サービスアカウントを用途別に分けて使う方法が正規かつ推奨されます。また、Google Cloud の Vertex AI を使えばクォータが組織単位で管理でき、よりスケールしやすくなります。

**Q. `429 RESOURCE_EXHAUSTED` と `503 UNAVAILABLE` の違いは何ですか？**

A. `429` はクォータ超過（リクエストが多すぎる）で、ユーザー側の問題です。`503` はサービス側の一時的な過負荷で、Google のインフラ問題です。どちらもリトライ対象ですが、`503` の場合はより長めの待機時間（10〜30秒）が推奨されます。本記事のリトライ実装では両方を `ServiceUnavailable` として捕捉しています。

**Q. Context Cachingを使うと本当にコストが下がりますか？**

A. 同じシステムプロンプト（例：長い業務ルールや大量のドキュメント）を使い回すユースケースでは、入力トークンのキャッシュ済み部分は通常価格の約25%で課金されるため、大幅なコスト削減になります。ただし、キャッシュ自体の保存コストもかかるため、同じキャッシュを15回以上使う場合に効果が出始めます。

**Q. アダプティブレートリミッターは本当に必要ですか？固定スリープではダメですか？**

A. 固定スリープは動作しますが非効率です。例えば「60 RPM制限に対して毎リクエスト1秒スリープ」するとスループットが最大60 RPMになりますが、実際には成功している間はフルスピードで、失敗時だけ下げる方が全体のスループットは高くなります。アダプティブ制御はその「自動調整」を実現します。
...
本記事では、Gemini APIのレート制限と向き合うための包括的な実装パターンを解説しました。
