...
highlights:
  - "コールドスタートゼロ・グローバル280拠点展開のエッジAIバックエンドを、従来のNode.jsサーバー構成の1/10以下のコストで実現できる"
  - "D1（エッジSQLite）を使った会話履歴永続化・レート制限・コスト追跡の本番パターンを、そのままコピーして使えるコードで習得できる"
  - "月$8〜12で実際に運用したコスト内訳と、Gemini APIのトークン消費を最小化するD1キャッシュ設計を手に入れられる"
---
...
この記事では、その問題を根本から解決した構成——**Cloudflare Workers + D1（エッジSQLite）+ Gemini API**——の実装を、スキーマ設計から本番デプロイまで一気通貫で解説します。月$10以下で動く AI バックエンドを、一緒に構築してみましょう。
