...
author: "Gemini Lab"
description: "Gemini APIが応答を途中でカットして finish_reason に RECITATION を返すエラーの原因と、再現性のある回避策を実装パターン付きで解説します。"
...
みっつ目は、観測可能性の確保です。`finish_reason` を構造化ログに必ず吐き、Datadog や Cloud Logging で集計できるようにしておくと、本番投入後に RECITATION の発生率が見えます。経験則として、RECITATION 率が全リクエストの 1% を超えてきたらプロンプト設計の見直しサインです。LLM 全般のエラー設計をもう一段深く学ぶには、[『LangChain完全入門』（田村悠）](https://www.amazon.co.jp/dp/4295202215/?tag=pinocchio-22) のリトライ・フォールバック設計の章が、Gemini を使う場合にもそのまま応用できておすすめです。
...
私は 2013 年から個人開発でアプリ事業を続けていて、現在 6 アプリ（壁紙・浮世絵・癒し・引き寄せ系）を運用しています。最近のリニューアルで Gemini API を本格的に組み込んで分かったのは、**マルチモーダル機能は壁紙の自動カテゴリ分類のような単発タスクで明確に強い**ということでした。

Beautiful 4K/HDR Wallpapers の 10 万枚以上の壁紙を Gemini Vision API でカテゴリ分類し直したところ、人手で 3 ヶ月かかる作業が 8 時間で終わりました。コスト面でも、ライトモデル（Flash 系）を使えば 1 枚あたり 0.01 円以下に収まり、収益化アプリの運用コストを大きく圧迫することはありませんでした。
