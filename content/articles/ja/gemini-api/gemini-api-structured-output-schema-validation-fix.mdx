...
最初に、スキーマは「最小公倍数」で定義します。`required` は本当に欠けると困るフィールドだけにし、`enum` は将来追加されそうな値を `"other"` などのキャッチオール枠で受けます。`additionalProperties` のような未サポートキーは一切書かない。Pydantic から自動生成して、生の JSON Schema を手で書くこと自体を避けるのが一番楽です。
