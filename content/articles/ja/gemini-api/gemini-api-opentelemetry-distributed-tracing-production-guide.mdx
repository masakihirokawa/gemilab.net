...
この記事では、Gemini API を中心としたバックエンドに OpenTelemetry を組み込み、1リクエストの裏側を span 単位で全部見せる本番運用構成を、実装コードと運用ノウハウの両面から解説します。OpenTelemetry の GenAI セマンティック規約に沿うことで、Grafana Tempo・Jaeger・Datadog のどれを選んでも同じダッシュボードが組める、ベンダーロックインしない設計を目指します。
