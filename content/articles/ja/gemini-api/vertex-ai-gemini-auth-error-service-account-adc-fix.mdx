...
この記事では、Vertex AI + Gemini の認証エラーについて、症状別の原因と解決手順を丁寧に解説します。
...
**Q1：ローカルでは動くのに Cloud Run に deploy すると `UNAUTHENTICATED` エラーになります。なぜですか？**

A：ローカルでは個人の Google アカウントの ADC を使っているのに対し、Cloud Run はサービスアカウントで動作しているためです。Cloud Run に正しいサービスアカウントがアタッチされているか（`--service-account` フラグ）、そのサービスアカウントに `roles/aiplatform.user` が付与されているかを確認してください。

**Q2：`GOOGLE_APPLICATION_CREDENTIALS` を設定しているのに `DefaultCredentialsError` が出ます。**

A：パスが絶対パスになっているか確認してください。相対パス（`./key.json` など）は一部の環境で認識されません。また、JSONファイルが壊れていないか `python -c "import json; json.load(open('key.json'))"` で確認しましょう。

**Q3：複数のプロジェクトを使い分けたい場合、認証をどう管理すればいいですか？**

A：`vertexai.init(project="...", location="...")` をリクエストごとに呼び出す方法と、サービスアカウントごとに `credentials` オブジェクトを生成して明示的に渡す方法があります。後者は以下のように実装します：

```python
from google.oauth2 import service_account
import vertexai

# プロジェクトAのサービスアカウントを使う
creds_a = service_account.Credentials.from_service_account_file(
    "project-a-key.json",
    scopes=["https://www.googleapis.com/auth/cloud-platform"]
)
vertexai.init(project="project-a", location="us-central1", credentials=creds_a)
```

**Q4：Workload Identity Federation を使うと何が良いですか？**

A：GitHub Actions や AWS など、Google Cloud 外の環境から Vertex AI に接続する際、サービスアカウントキーを使わずに一時的な認証情報を取得できます。キーの漏洩リスクをゼロにしつつ、CI/CD パイプラインから Vertex AI を安全に呼び出せるのが最大のメリットです。
