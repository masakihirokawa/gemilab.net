...
私はステージング環境の `MODEL_REGISTRY` にだけ latest 系エイリアスを置き、本番は必ずバージョン付きの ID でピン止めしています。ステージングで 1〜2 週間ぶんのルーティングログを比較し、Flash 比率と再質問率が動いていないことを確認してから、本番の `model_id` を上げる。この手順にしてから、世代交代でコスト前提が壊れることはなくなりました。

なお、世代を混在させる選択肢も現実的です。単純な分類や短文翻訳は `gemini-2.5-flash` に残したまま、中程度の推論だけを `gemini-3.5-flash` へ寄せる。役割名を `fast` / `mid` / `smart` の 3 段に増やせば、`MODEL_REGISTRY` に 1 行足すだけで対応できます。ルート定義を触らずに段数を増やせることが、この分離の一番大きな見返りでした。
...
収益に触れる変更ほど、元に戻せる形で・観察できる形で進める。ルーターは一気に全量を切り替えたくなりますが、そこを我慢できるかどうかで、後から起きる事故の大きさが変わります。
...
繰り返し学び直しているのは、設計初期の数行が運用 1 年後の損益にそのまま出るということです。従量課金の API では、フォールバックの 1 行や月 1 回の embedding 再生成といった小さな運用習慣が、利益率の差になって現れます。派手な最適化より、続けられる習慣のほうが効きます。
