...
highlights:
  ...
  - "本番運用で実際に踏んだ「公式ドキュメントには書かれていない 7 つの実装インサイト」（embedding コスト・低信頼度フォールバック・centroid 陳腐化への対処を含む）"
...
従量課金の怖さは、壊れないことです。エラーも出ず、レイテンシも悪化せず、ただ請求額だけが月ごとに厚くなっていく。個人開発のバックエンドに Gemini API を組み込んでから最初の 2 ヶ月ほど、私はこの静かな出血に気づけませんでした。気づいたきっかけは機能の不具合ではなく、単に請求画面の桁を見間違えたと思ったことです。
...
- **gemini-2.5-flash-thinking**: thinking 付きで推論コストが加算される仕様
...
コスト最適化のベースとなるモデル選択については、[Gemini API モデル選択ガイド](/articles/gemini-basics/gemini-model-selection) も合わせてご参照ください。
...
## モデル名を直書きしたルーターは、世代交代で前提ごと崩れます

（2026 年 7 月の追記）この記事を書いた 4 月時点のルート定義は、`gemini-2.5-flash` と `gemini-2.5-pro` を文字列でそのまま書いていました。7 月現在、一般提供されている最新の Flash 系は `gemini-3.5-flash` で、`gemini-flash-latest` というエイリアスが指す実体もこちらへ切り替わっています。

ここで一度立ち止まる必要があります。`gemini-3.5-flash` の単価は入力 $1.50/1M・出力 $9.00/1M です。本記事の前半で挙げた `gemini-2.5-pro` は入力 $1.25/1M・出力 $10.00/1M でした。つまり最新世代の Flash は、前世代の Pro とほぼ同じ価格帯にいます。

「Flash だから安い」という前提が、世代を跨いだ瞬間に成り立たなくなるということです。

<table>
<thead><tr><th>モデル</th><th>入力 / 1M</th><th>出力 / 1M</th></tr></thead>
<tbody>
...
素直に「Flash を最新へ上げる」だけの更新をすると、ルーターは正しく動いているのに削減率だけが消えます。私はここで一度、月次のコストレポートを見て首をかしげました。ルーティング比率のダッシュボードは何も変わっていないのに、請求だけが戻っていたためです。

ルーターの価値は「安いモデルへ寄せること」ではなく「そのタスクに必要な最小のモデルを選ぶこと」です。世代が変わったら、削減率の試算そのものをやり直す必要があります。

### 役割名と実モデル ID を分離する

対処は単純で、ルート定義には役割名だけを書き、実モデル ID と単価の解決を 1 箇所へ集約します。
...
class ModelSpec:
    model_id: str        # API へ渡す ID。エイリアスではなく必ずピン止めする
    input_per_1m: float  # USD
    output_per_1m: float

# 世代交代のときはここだけを更新すればルーター全体が追随する
...
ルート定義側は `"model": "gemini-2.5-flash"` ではなく `"tier": "fast"` を持つだけにします。こうしておくと、モデルを差し替えたときに変更が必要なのは `MODEL_REGISTRY` の 3 行だけになり、コスト試算のコードが古い単価を掴んだまま残る事故がなくなります。

### `gemini-flash-latest` を本番のルート定義に書かない

エイリアスは便利ですが、アップグレードのタイミングを自分で決められません。ルーターにとってモデルの切り替わりは、単価とルート embedding の両方が同時に古くなる出来事です。単価が変われば削減率の試算が狂い、応答傾向が変われば低信頼度フォールバックの閾値も合わなくなります。

私はステージング環境の `MODEL_REGISTRY` にだけ latest 系エイリアスを置き、本番は必ずバージョン付きの ID でピン止めしています。ステージングで 1〜2 週間ぶんのルーティングログを比較し、Flash 比率と再質問率が動いていないことを確認してから、本番の `model_id` を上げます。この手順にしてから、世代交代でコスト前提が壊れることはなくなりました。

なお、世代を混在させる選択肢も現実的です。単純な分類や短文翻訳は `gemini-2.5-flash` に残したまま、中程度の推論だけを `gemini-3.5-flash` へ寄せるという分け方です。役割名を `fast` / `mid` / `smart` の 3 段に増やせば、`MODEL_REGISTRY` に 1 行足すだけで対応できます。ルート定義を触らずに段数を増やせることが、この分離の一番大きな見返りでした。
...
収益に触れる変更ほど、元に戻せる形で・観察できる形で進めます。ルーターは一気に全量を切り替えたくなりますが、そこを我慢できるかどうかで、後から起きる事故の大きさが変わります。
...
特定のユーザーに常に上位モデルを使いたい場合は、`user_tier` パラメーターを使ってプレミアムユーザーへのルーティングをオーバーライドできます。コードの `user_tier == "premium"` の分岐がそれに対応しています。価格設計については [Gemini API コスト最適化の考え方と実装](/articles/gemini-api/gemini-api-cost-optimization-complete-guide) も参考になります。

---

## 締めくくり

セマンティックルーターの本質は「すべてのクエリに最上位のモデルを当てない」というシンプルな発想です。2 段階ルーティング（ルールベース + セマンティック）により、ルーティングコスト自体を最小化しながら高精度なモデル選択が実現できます。適切な振り分けにより、典型的なプロダクション環境では API コストを 50〜70% 削減できます。フォールバック設計と信頼度閾値を設けることで、品質の安全網も確保されます。

繰り返し学び直しているのは、設計初期の数行が運用 1 年後の損益にそのまま出るということです。従量課金の API では、フォールバックの 1 行や月 1 回の embedding 再生成といった小さな運用習慣が、利益率の差になって現れます。派手な最適化より、続けられる習慣のほうが効くのだと感じています。
