...
そこで **Twilio Voice の着信を Gemini Live API に橋渡しし、本番運用に耐える電話応答エージェントを組み上げる道順** を、私が踏んだ順番のまま書き残します。コードはすべて動く形で置き、エラーハンドリング・割り込み制御・落とし穴・コスト試算まで通します。Gemini Live API の基本をひととおり触れられた中〜上級の方に向けた内容です。

いま思えば、最初に手が止まっていた原因は音声フォーマットではなく、**電話は黙ったまま繋がり続けるメディアだ**という当たり前の事実を実装に落とし込めていなかったことかもしれません。ブラウザのデモはタブを閉じれば終わります。電話は、誰も話していない十数秒のあいだも回線を保ち、その分だけ課金が積み上がっていきます。
...
- Twilio から届いた base64+μ-law の decode と 16kHz PCM へのアップサンプリング
- Gemini Live API の WebSocket へのバイナリ送信
- Gemini が返す 24kHz PCM 応答の 8kHz μ-law へのダウンサンプリングと base64 包装
- 変換後フレームの Twilio への WebSocket 返送
...
1. ユーザーが電話をかけると、Twilio が `Voice URL` の Webhook を呼びます
2. 中継サーバーは TwiML で `<Connect><Stream url="wss://.../media">` を返します
3. Twilio が中継サーバーの WebSocket に接続し、ユーザーの音声を流し始めます
4. 中継サーバーは Gemini Live API にも WebSocket で接続し、双方向ブリッジを開始します
5. Gemini の応答音声が中継サーバー経由で Twilio に戻り、ユーザーの耳に届きます
6. 通話終了で Twilio が `stop` を送り、中継サーバーが Gemini セッションを閉じます
...
予算を引き締めたい場合は、**System Instructions を短く保つ**（毎セッションで送信されます）、**応答の最大長を制限する**、**長時間の沈黙を検知して自動で通話を切る** の 3 点が効きます。三つ目がいちばん取りこぼされやすいので、次の節で実装ごと置いておきます。

## 沈黙とタイムアウトの見張り番 — 課金を止める 40 行

コスト試算の最後に挙げた「沈黙で切る」は、後回しにされたまま請求書で気づく類の項目です。私も最初のうち、通話は人間が切ってくれるものだと思い込んでおりました。実際には、保留にされたまま置かれた電話、話し終えたのに切り忘れられた電話、そして着信音だけを鳴らして立ち去られた電話が、想像よりずっと多く残ります。

ここで最初に共有したいのは、**Twilio の無音は「フレームが来ないこと」ではない**という点です。Media Streams は誰も話していない間も 20 ms ごとに `media` フレームを送り続けます。中身が無音の μ-law で埋まっているだけです。ですから「一定時間フレームが来なかったら切る」という素朴な実装は、まったく発火しません。切れているのは通話ではなく、こちらの期待の方でした。

見るべきは中身の音量です。`audioop.rms` は 1 行で振幅の実効値を返してくれますので、デコード済みの 8kHz PCM に当てて閾値と比べます。

```python
# server/watchdog.py
# 役割: 無音の継続と通話の長さを見張り、条件を満たしたら通話を終わらせる
import audioop
import time
from typing import Optional

SILENCE_RMS = 500        # これ未満の実効値を「無音」とみなす（環境に合わせて調整）
SILENCE_LIMIT_SEC = 12.0  # 無音がこれだけ続いたら切る
MAX_CALL_SEC = 300.0      # 1 通話の上限（暴走・置き忘れの最後の砦）

class CallWatchdog:
    """通話単位で「最後に声が出た時刻」と「開始からの経過」を持つだけの小さな見張り番。"""
...
ブリッジ側への差し込みは 4 か所だけです。`media` イベントで `watchdog.note_caller_frame(mulaw)`、Gemini から音声が届いたところで `watchdog.note_agent_audio()`、そして 1 秒ごとに判定を回す小さなタスクを `asyncio.gather` に足します。
...
async def watch_call(twilio_ws, watchdog: CallWatchdog, call_sid: Optional[str]) -> None:
    """1 秒ごとに見張り、条件を満たしたら通話そのものを終わらせる。"""
    while True:
...
`call_sid` は `start` イベントの `msg["start"]["callSid"]` から取れます。WebSocket を閉じるだけでも `<Connect><Stream>` は終端しますが、REST で `status="completed"` を明示した方が、通話が中途半端に生き残る余地がありません。ここは二重に閉じておく価値があると感じています。

ひとつだけ補足させてください。`<Connect>` は終端の動詞ですので、ストリームを閉じた時点で通話も終わります。「では失礼いたします」と言ってから切りたい場合は、切断の前に Gemini 側へ短い別れの発話を促し、その音声を流し終えてから `close` を呼ぶ、という順番にする必要があります。無言で回線が消える体験は、電話ではことのほか冷たく響きます。

閾値は環境で動きます。私が試した範囲では、`SILENCE_RMS` は 300〜800 のあいだで挙動が大きく変わりました。低すぎると背景のエアコンや店内の物音を「声」と拾い続けて永遠に切れず、高すぎると小さな声の方の相槌を無音と判定して会話の途中で切ってしまいます。実運用に入れる前に、自分の回線から数本かけて `audioop.rms` の値をそのままログに出し、話しているときと黙っているときの数字の開きを見てから決めていただければと思います。**閾値は推測ではなく、自分の回線から採った 2 つの数字のあいだに置く。** この手順だけは省かないようにしています。

`SILENCE_LIMIT_SEC` は短くしすぎない方が安全です。相手が予約の日付をメモ帳で探している十数秒は、無音ですが立派な会話の一部です。私は 12 秒あたりから始めて、切られた通話のログを見ながら調整しています。

<table>
  <tr><th>設定</th><th>短すぎる場合</th><th>長すぎる場合</th><th>出発点の目安</th></tr>
  <tr><td><strong>SILENCE_RMS</strong></td><td>相槌や小声を無音と誤判定</td><td>環境音を声とみなして切れない</td><td>300〜800（自回線で実測）</td></tr>
  <tr><td><strong>SILENCE_LIMIT_SEC</strong></td><td>考えている相手を切ってしまう</td><td>置き忘れ通話の課金が伸びる</td><td>10〜15 秒</td></tr>
  <tr><td><strong>MAX_CALL_SEC</strong></td><td>長い問い合わせが途中で終わる</td><td>暴走時の上限として機能しない</td><td>300 秒（用途で調整）</td></tr>
</table>

この 40 行が守ってくれるのは、月末の請求書だけではありません。ゾンビ化した Gemini セッションが Live API の同時接続クォータを食い潰し、本当にかけてきた方の電話が繋がらなくなる — その事故の芽も、同時に摘んでくれます。
...
中継サーバーの仕事は順番にこうなります。まず base64 デコードで 160 バイトの μ-law バッファに戻します。`audioop.ulaw2lin` で 16bit 線形 PCM に展開すると 320 バイトになります。`audioop.ratecv` で 8kHz から 16kHz にアップサンプルすると 640 バイトです。これを Gemini Live に送ります。
...
ひとつ目は、**回答の長さを縛ること**です。「明示的に求められない限り、2 文以内で答えてください」と書いておくと、電話 UX が一気に締まります。長文は画面でよりずっと冗長に感じられますし、聞き手は読み飛ばしができません。

ふたつ目は、**話のリズムと人格を平易な言葉で指定すること**です。「親しみやすく、テンポの良い日本語で。各ターンの最後は短い確認質問で締める」と書くと、Gemini はその型に乗せてくれます。長々と説明したり、同じ確認を繰り返す AI は、電話だと役所のカウンターのように響きます。

3 つ目は、**逃げ道を最初に書いておくこと**です。「わからないことがあれば『人間が折り返します』と伝え、お名前と番号を伺ってください。推測で答えないでください」と添えておくと、営業時間や住所の幻覚を防げます。電話で適当な情報を答えるのは、画面で答えるよりずっと致命的です。
