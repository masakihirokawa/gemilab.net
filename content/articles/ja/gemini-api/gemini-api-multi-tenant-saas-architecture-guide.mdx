...
author: "Gemini Lab"
description: "Gemini API を複数テナントに提供する SaaS の本番アーキテクチャを Python コードつきでまとめました。テナント分離モデルの選択、テナント別レート制限、リクエスト単位の課金計測、月予算ガードとサーキットブレーカーによる暴走防止まで扱います。"
...
厄介になるのは、チームが親テナントとは**別の**予算上限を持ちたいときです。2つの設計が考えられます。
