---
title: "Gemini API で『人間が確認するAI』を作る — Human-in-the-Loop ワークフローを本番導入する完全実装ガイド"
...
本記事では、私が実際に運用している HITL アーキテクチャを 3 層モデルに分解して、コードレベルで再現できる粒度まで落とし込んで解説します。読み終える頃には、「自動採用」「人間レビュー」「却下＆再生成」の振り分けロジックを、ご自身のサービスに今日から組み込めるようになっているはずです。
...
「composite が 0.7 を切ったらレビュー」のように決めて運用を始めるのは構いませんが、**閾値の妥当性を測定するメカニズムを最初から組み込む** ことが重要です。具体的には、自動採用された出力のうち、ランダムに 5% を「シャドウレビュー」と称してレビューに回し、レビュアーが「自動採用で問題なかった」と判断した割合を集計します。これが 95% を切ったら閾値を上げる、というガバナンスを最初から仕込んでおきます。
...
「過去 100 件の編集を全部 Few-Shot に入れれば最強」と思いがちですが、Few-Shot が増えるほどコンテキストが膨張し、推論コストとレイテンシが悪化します。私は **直近 1 週間の上位 3 件まで** に絞っています。「最強の Few-Shot」より「最新かつ少数の Few-Shot」のほうが現場では機能します。
...
書籍で体系的に学びたい方には、[エンジニアリング組織論への招待（広木大地）](https://www.amazon.co.jp/dp/4774196053/?tag=pinocchio-22) が「人間と機械の責任分担」を考える土台として効きます。AI 周りの設計判断は技術論だけでなく、組織と権限の話に必ず接続するため、この本の視点は HITL の運用ポリシー策定で何度も助けられました。LLM 側の運用ノウハウは [LLM のプロダクション運用 入門（オライリー）](https://www.amazon.co.jp/dp/4814401213/?tag=pinocchio-22) も実務に直結する一冊です。
