...
Gemini API のマルチモーダル動画入力を使うと、この一連の流れを 1 リクエストで完結できます。ただし「動く実装」と「実用に耐える実装」の差は意外と大きく、特にタイムスタンプ精度と翻訳品質の両立で何度かつまずきました。本記事では、5 分前後の動画を想定して、私自身が辿り着いた実装パターンをまとめます。
