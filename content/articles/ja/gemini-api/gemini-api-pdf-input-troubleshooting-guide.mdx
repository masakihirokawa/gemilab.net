...
この記事では、Gemini API（`gemini-2.5-pro` / `gemini-2.5-flash`）でPDFを処理するときに私自身が踏み抜いてきた落とし穴を、症状別に整理しています。コードはすべて Python 用 `google-genai` SDK で書いてありますが、Node.js でも考え方はそのまま適用できます。
...
PDF処理は腰を据えて取り組むほど面白い領域でもあります。`gemini-2.5-pro` のマルチモーダル処理を本番で使い倒したい方には、[Pythonによる新しい時代のAIプログラミング入門（我妻幸長）](https://www.amazon.co.jp/dp/4798182656/?tag=pinocchio-22) や [LangChain完全入門 第2版（田村悠）](https://www.amazon.co.jp/dp/4295018562/?tag=pinocchio-22) が、PDF・テキスト・画像を組み合わせるパイプライン設計の参考になります。
