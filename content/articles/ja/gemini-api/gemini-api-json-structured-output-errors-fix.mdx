...
この記事では、Gemini APIのJSON出力で発生しやすいエラーパターンを網羅的に整理し、それぞれの原因と具体的な解決手順をステップバイステップで解説します。コード例はすべてPythonとJavaScript（Node.js）の両方で示しているため、お使いの環境に合わせてすぐに適用できます。
...
### Q1：JSON Modeと Structured Outputの違いは何ですか？

**JSON Mode**（`response_mime_type: "application/json"`）は、レスポンスがJSON形式であることのみを保証します。一方、**Structured Output**（`response_schema` を追加指定）は、JSONの構造（フィールド名・型・必須項目）まで制約します。確実性を求める場合はStructured Outputを使用してください。より詳しくは[Gemini API 構造化出力の完全ガイド](/articles/gemini-api/gemini-structured-output-advanced)で解説しています。

### Q2：`response_schema` でネストしたオブジェクトはどこまでサポートされていますか？

Gemini 2.5 Pro / Flashでは、5階層程度のネストまで安定して動作します。それ以上の深さが必要な場合は、フラットな構造に変換してから出力し、受け取り側でネスト化することを推奨します。

### Q3：enum値が無視されることがあるのですが、どうすれば良いですか？

`enum` フィールドとあわせて `description` に「この値のみ許可」と明記すると遵守率が上がります。さらにバリデーション層で不正値を検出し、リトライするのが最も確実です。

### Q4：日本語のフィールド名は使えますか？

技術的には使用可能ですが、英語のフィールド名を使用した方がモデルの精度が高くなる傾向があります。フィールド名は英語、値は日本語という組み合わせが最も安定します。

### Q5：ストリーミングモードでJSON Modeは使えますか？

はい、ストリーミングとJSON Modeは併用可能です。ただしストリーミング中の各チャンクは部分的なJSONであるため、全チャンクを結合してからパースする必要があります。チャンク単位でのパースは行わないでください。
...
APIプログラミングの基礎を体系的に学びたい方には「[Web API: The Good Parts（水野貴明 著）](https://www.amazon.co.jp/dp/4873116864/?tag=pinocchio-22)」がおすすめです。
