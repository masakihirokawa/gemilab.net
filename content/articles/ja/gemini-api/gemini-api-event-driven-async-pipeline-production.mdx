...
highlights:
  - "Google Cloud Pub/Sub + Gemini APIで大量リクエストを安定処理する非同期パイプラインを実装できる"
  - "WebhookコールバックとRedisキューを組み合わせたスケーラブルなバックグラウンド処理設計を習得できる"
  - "デッドレターキュー・リトライ戦略・分散トレーシングによる本番グレードのエラー管理手法を理解できる"
---
...
本記事では、Google Cloud Pub/Sub、FastAPI Webhook、Redisキューという3つの主要パターンを詳しく解説し、プロダクション環境で実際に使える設計パターンを提供します。
...
**Q1: Pub/SubとRedisキューはどちらを選ぶべきですか？**

Google Cloud環境をすでに使っている場合はPub/Subが、オンプレミスやAWSなど他のクラウド環境では Redis（Celery/ARQ）が適しています。Pub/Subはフルマネージドで可用性が高く、Redisは低レイテンシで柔軟な制御が可能です。1日数万件以下の処理量であればRedisで十分で、それ以上のスケールが必要であればPub/Subを推奨します。

**Q2: Gemini APIのレート制限に引っかかった場合、メッセージは失われますか？**

適切にnack（否定応答）を返せば失われません。Pub/SubはnackされたメッセージをAck期限（デフォルト10分）経過後に再配信します。ただし、nackの際はウェイトを入れてすぐ再試行しないよう注意してください。エクスポネンシャルバックオフと組み合わせることで、レート制限時の挙動を最適化できます。

**Q3: Webhookエンドポイントのセキュリティをどう確保すればよいですか？**

HMAC-SHA256による署名検証が基本です。加えて、エンドポイントをCloudflare WAFやCloud Armorで保護し、既知のIPアドレスからのみリクエストを受け付けるIPホワイトリストを設定することを推奨します。また、受信したJSONのスキーマバリデーション（Pydanticなど）も必ず実施してください。

**Q4: 本番でのワーカー数の決め方は？**

Gemini APIのTierによって異なります。Tier 1（無料/低消費）は分あたり2〜10リクエストと制限が厳しいため、ワーカーは1〜2台で十分です。Tier 3以上（大量消費）では分あたり1,000リクエスト以上が可能なため、ワーカーを20台以上に増やしてスループットを最大化できます。まずは小さく始め、キューの深さとレイテンシを監視しながらスケールアウトするのが安全です。

**Q5: イベント駆動パイプラインのデバッグはどうすればよいですか？**

ローカル開発では`google-cloud-pubsub`のエミュレーターを使うと本番と同じコードをテストできます。加えて、各メッセージにトレースIDを付与してログに記録し、Cloud LoggingとCloud Traceを組み合わせることで、分散環境での問題を素早く特定できます。
