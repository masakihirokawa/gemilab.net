...
その時に痛感したのは「無効化すれば終わり」ではないということです。本番サービスから呼ばれているキーをただ削除すると、その瞬間からエラーが噴き出します。安全と継続稼働を両立させるには、キーを「捨てて作り直す」のではなく「重ね替える」発想が必要です。この記事ではそのための実装パターンを、Node.js / Python / Cloudflare Workers の3環境分のコードと一緒に整理します。
...
- **5分以内**: AI Studio で漏えいキーを「Disable」ではなく「Delete」する。Disable は将来うっかり再有効化される事故の温床になります
...
API設計とセキュリティの基礎を体系的に押さえたい方には、[Web API: The Good Parts（水野貴明）](https://www.amazon.co.jp/dp/4873116864/?tag=pinocchio-22)が今でも色褪せない参考書です。本記事の「ステップを分解してダウンタイムを潰す」考え方の下地になっている本でもあります。
