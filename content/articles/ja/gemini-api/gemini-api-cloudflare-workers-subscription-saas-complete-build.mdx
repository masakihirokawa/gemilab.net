---
title: "Gemini API × Cloudflare Workers で月額課金SaaSを構築する完全実装ガイド — 2026年版"
...
highlights:
  - "Gemini API（Flash/Pro/Ultra）を切り替え可能な構成で組み込み、低コストの優位性を実装に反映する完全な構成例"
  - "Stripe Checkout + Webhook + Cloudflare KV でアクセス制御を実装する具体コード（idempotency対策込み）"
  - "Geminiの低コストを活かしたヘビーユーザー対応設計（『使い放題プラン』を成立させるための設計判断）"
---

[前編のGemini API有料サービス立ち上げロードマップ](/articles/gemini-api/gemini-api-paid-service-launch-roadmap-2026)で書いた通り、Gemini APIの強みは圧倒的な低コストです。本記事はその実装編で、Cloudflare Workers上でStripe決済とGemini API呼び出しを組み合わせた月額課金SaaSを構築する完全な手順をまとめます。
...
`metadata.plan_type` を必ず付けることが重要です。Webhookで受け取ったときにサブスクと単発購入を区別する手がかりになります（STUMBLING_POINTS #14.5）。
