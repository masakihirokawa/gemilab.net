...
この記事では、**Gemini 3.1 Pro の REST API を curl・Python requests・JavaScript fetch で呼び出す方法** を、認証設定から実際のコード例・よくあるエラー対処まで一通り解説します。
...
**Q. `v1` と `v1beta` どちらを使えばいいですか？**

A. 最新モデル（Gemini 3.x 系）や思考機能・Function Calling などを使う場合は `v1beta` を選んでください。`v1` は安定していますが、一部の最新機能がまだ含まれていません。本番環境でも `v1beta` を使い続けているチームが多いのが現状です。

**Q. Vertex AI の Gemini と generativelanguage.googleapis.com はどう違いますか？**

A. `generativelanguage.googleapis.com`（= Google AI API）は API キーで手軽に使えますが、エンタープライズ向けの SLA やデータ処理地域の選択、VPC への接続などは Vertex AI 側にしかない機能です。個人開発やプロトタイプは Google AI API、本番の大規模サービスは Vertex AI という使い分けが一般的です。

**Q. API キーをソースコードに書いてはいけない理由は？**

A. API キーがリポジトリに混入すると GitHub Secret Scanning 等で検出され、悪用リスクがあります。必ず環境変数（`.env` ファイル + `.gitignore`）や Secret Manager を使ってください。ローカル開発では `python-dotenv` が便利です。
...
本記事のテーマをさらに深めたい方には「[Gemini APIではじめる生成AIプログラミング（我妻幸長 著）](https://www.amazon.co.jp/dp/4297140985/?tag=pinocchio-22)」がおすすめです。REST API の基礎から実践的な活用まで丁寧に解説されており、ステップアップに最適な一冊です。
