...
ややこしいのは、エラー文だけ見ると「会話が交互になっていない」という意味に読めるのに、実際には別の原因（`role` の表記ゆれ、Function Calling のレスポンス構造、`system_instruction` の混入など）で同じ例外が投げられる点です。本記事では、私がプロダクトのリリース前に踏んだ落とし穴を中心に、原因別の見分け方と修正パターンを整理しました。
...
LLM アプリ全般のエラーハンドリングをもう一段深掘りしたい方には、[『大規模言語モデル入門』（山田育矢ほか）](https://www.amazon.co.jp/dp/4297145146/?tag=pinocchio-22)が概念整理に役立ちます。本書の「対話モデルの履歴設計」の章は、Gemini に限らず役割設計の基礎として読み応えがあります。
