---
title: "Gemini API × Slack Bot 本番実装完全ガイド — Bolt SDK・スレッドコンテキスト・Cloud Run デプロイまで徹底解説"
...
**Q1. Slack のイベントが重複して届くことがありますが、どう対処すればよいですか？**

Slack は応答が3秒以内に返らないと同じイベントを再送します。`ack()` を最初に呼び出して即時応答し、重い処理は非同期で行うことが重要です。また、Bolt SDK は `process_before_response=True` オプションで応答前処理を明示的に制御できます。イベント ID（`event_id`）をキャッシュして重複を検出する二重防止ロジックも本番では推奨されます。

**Q2. スレッドコンテキストをインメモリで管理していますが、Cloud Run の複数インスタンス間で共有できますか？**

インメモリ実装では複数インスタンス間でコンテキストが共有されません。Cloud Run の `--max-instances 1` で単一インスタンスに固定するか、本番では **Redis（Cloud Memorystore）** または **Firestore** にコンテキストを永続化することを推奨します。Redis を使う場合は [Gemini API × Redis セマンティックキャッシュ](/articles/gemini-api/gemini-api-redis-semantic-cache-guide) の実装パターンが参考になります。

**Q3. Gemini のモデルはどれを選べばよいですか？**

用途に応じて選択します。

- **`gemini-2.5-flash`** — 高速・低コスト。チャット Bot の日常的な質問応答に最適
- **`gemini-2.5-pro`** — 高精度・複雑な推論。技術的な質問やコードレビューに適切
- **`gemini-2.0-flash-exp`** — 実験的機能。最新機能を試したい場合

Bot では `gemini-2.5-flash` をデフォルトとし、`/gemini-pro` のような Slash Command で `gemini-2.5-pro` に切り替えられるようにするのも実践的なアプローチです。

**Q4. Slack の Grounding 機能（社内 URL の参照）は実装できますか？**

Gemini の Grounding with Google Search を使えば最新情報を参照した回答が可能です。社内ナレッジを参照させたい場合は、Notion や Confluence との連携を検討してください。[Gemini API Grounding with Google Search 完全実装ガイド](/articles/gemini-api/gemini-api-grounding-google-search-production-guide)も参考になります。

**Q5. 会話ログを分析・改善するにはどうすればよいですか？**

Cloud Logging にすべての会話を記録し、BigQuery にエクスポートすることでユーザーの質問傾向や Bot の精度を分析できます。Cloud Run のログには自動的にリクエストログが記録されます。

---

## まとめ

この記事では Gemini API と Slack Bolt SDK を組み合わせた本番レベルの Slack AI Bot の実装方法を解説しましました。
...
本記事のテーマをさらに深めたい方には、[「Python で学ぶ Slack API 完全入門」（辻拓也 著）](https://www.amazon.co.jp/dp/4798066729/?tag=pinocchio-22)がおすすめです。Bolt SDK の内部動作から応用パターンまで丁寧に解説されています。
