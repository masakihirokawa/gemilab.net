...
highlights:
  - "gemini-2.5-pro-latest と固定バージョン（gemini-2.5-pro-001等）の使い分け — 本番で"-latest"を使うべきか"
  - "429レートリミット・503過負荷・500内部エラーそれぞれに必要な再試行戦略と実装コード"
  - "Prompt Caching とバッチAPIによるコスト削減の具体的な手順と効果測定結果"
---
...
この記事では、`gemini-2.5-pro-latest` を本番で使う前に知っておくべき挙動と、レート制限・エラー処理・コスト最適化の実務レベルでの対処法を、実際に運用してわかった教訓ベースで共有します。
