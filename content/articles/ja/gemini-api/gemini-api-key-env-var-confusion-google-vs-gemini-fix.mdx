...
私自身、個人開発のアプリで Python から Node.js に書き換えた際にこの罠にハマり、2 時間ほど無駄にした経験があります。この記事では、SDK ごとの読み込み優先順位、Vertex AI モードに切り替わってしまう条件、ハマりやすい 4 つのパターンを、5 分で原因が特定できる切り分け手順とともに整理します。
...
書籍で体系的に学びたい方には、環境変数とシークレット管理の章が手厚い[実践 Terraform AWS/GCPによるシステム設計とベストプラクティス（野村友規 著）](https://www.amazon.co.jp/dp/4297127008/?tag=pinocchio-22) が参考になります。Gemini API 自体の使いこなしまで一気に押さえたい場合は[ChatGPT/LangChainによるチャットシステム構築実践入門（吉田真吾・大嶋勇樹 著）](https://www.amazon.co.jp/dp/4297138395/?tag=pinocchio-22) も、サンプルコードの構造が参考になります。
