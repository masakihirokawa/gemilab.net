---
title: "Gemini API × SwiftUI 本番アプリ完全実装ガイド：ストリーミング・マルチモーダル・エラーハンドリング・App Store対策まで"
...
highlights:
  - "Firebase不要・URLSession直接統合でストリーミング応答を実装する本番パターンを完全解説"
  - "カメラ・フォトライブラリからのマルチモーダル入力処理とメモリ管理の実装例を提供"
  - "App Store申請時のAI機能プライバシーマニフェスト対策と審査通過チェックリストを網羅を実践的に習得できる"
---
...
**Q. Firebase AI Logic SDK と直接統合では何が違いますか？**

Firebase AI Logic SDK は Firebase との連携（Analytics, Crashlytics 等）や Google ログイン認証が必要な場合に便利です。一方、直接統合では外部依存が `URLSession` のみになり、アプリサイズの軽量化・Firebase プロジェクトの設定不要・より細かい API オプションの制御が可能です。シンプルな AI 機能追加なら直接統合が推奨です。

**Q. API キーをクライアントに置かず、バックエンドプロキシを立てる必要はありますか？**

個人利用やプロトタイプ段階ではキーチェーン保存で十分ですが、**不特定多数にアプリを配布する本番では強くバックエンドプロキシを推奨します**。バイナリを解析されるとキーチェーンの内容も取得されるリスクがあります。Cloudflare Workers や Google Cloud Run で数十行のプロキシを立て、クライアントはそこにリクエストを送る構成が安全です。

**Q. Gemini API を使ったアプリは App Store で審査通過できますか？**

問題なく通過できます。ただし、以下の要件を満たす必要があります：(1) AI 生成コンテンツであることをユーザーに伝えるか、免責事項を設ける、(2) 有害コンテンツフィルタリングを実装する、(3) API キーをコードにハードコードしない、(4) プライバシーポリシーに Google へのデータ送信を明記する。これらを満たせば審査に問題はありません。

**Q. ストリーミング中にアプリがバックグラウンドに入った場合、どう対処しますか？**

`URLSession` のタスクはバックグラウンドで一定時間動き続けますが、長時間の処理は中断されます。`Background Tasks` フレームワークを使うか、バックグラウンド移行時点でストリーミングを停止して結果を保存し、フォアグラウンド復帰後に再開する UX が現実的です。バックグラウンドダウンロードが必須な用途では `URLSessionConfiguration.background` を検討してください。

**Q. マルチモーダル入力で動画ファイルを送ることはできますか？**

Gemini API はインライン動画（Base64 エンコード）も受け付けますが、ファイルサイズの制限（20MB）があります。長い動画の場合は File API を使って先にアップロードし、URI 参照で送信するフローが必要です。iOS での動画処理は `AVFoundation` と `AVAssetExportSession` を使ってトリミング・圧縮を行ってから送信するのがベストプラクティスです。

## まとめ

本記事では、Gemini API を SwiftUI アプリへ本番レベルで統合するための5つの柱を解説しましました。
...
これらのパターンを組み合わせることで、ユーザーに愛されるクオリティの AI 搭載 iOS アプリが生まれます。SwiftUI と Gemini の組み合わせは、まさに個人開発者が世界に価値を届けるための最強の武器のひとつです。
