...
`thinking_budget` の値は `-1`（自動）のままで実用上はほぼ問題ありません。明示的に指定する場合、Pro なら 128 以上、Flash なら 64 以上を下限として考えると、極端に短い思考に縮小されてしまう事故を避けられます。逆に上限を張る場合は、月末近くのコスト管理として 512〜1024 程度で頭打ちにしておくと、暴走を防ぎながら十分な思考が得られます。Pro と Flash のモデル選定は [Gemini 2.5 Pro 思考バジェット推論制御ガイド](/articles/gemini-advanced/gemini-25-pro-thinking-budget-reasoning-control) が詳しく、併読すると実装判断が早くなります。

モデルを選ぶときは、思考サマリーが出るかどうかだけでなく、増えるトークンと待ち時間まで含めて決めたいところです。同じ質問を 10 回ずつ投げ、手元で平均した値を並べておきます。プロンプトの長さで揺れますので、桁感の参考としてご覧ください。

<table>
<thead>
<tr><th>モデル</th><th>思考サマリー</th><th>thinking_budget の実用下限</th><th>出力トークンの増分</th><th>初回チャンクまでの遅延増</th></tr>
</thead>
<tbody>
<tr><td><strong>gemini-2.5-pro</strong></td><td>対応</td><td>128</td><td>約 1.6 倍</td><td>+15% 前後</td></tr>
<tr><td><strong>gemini-2.5-flash</strong></td><td>対応</td><td>64</td><td>約 1.3 倍</td><td>+8% 前後</td></tr>
<tr><td><strong>gemini-2.5-flash-lite</strong></td><td>非対応（空配列が返る）</td><td>—</td><td>—</td><td>—</td></tr>
</tbody>
</table>

数字を見ると、思考サマリーを常時オンにするか、画面ごとに切り替えるかの線引きが具体的になります。私は、判断を伴う画面だけ Pro を使い、一覧の要約や下書き生成は Flash に寄せる、という分け方で落ち着きました。
...
## 中断・プラン制御・保存を1本の経路にまとめる

ここまでで、中断処理・プランによる出し分け・思考の保存を「後で詰める」と書いてきました。この三つは結局同じ Route Handler に集まりますので、私が実際に運用している形をそのまま置いておきます。

```typescript
// app/api/ask/route.ts（中断・プラン制御・保存を組み込んだ版）
import { GoogleGenAI } from "@google/genai";
...
export async function POST(req: Request) {
  ...
  // 思考サマリーを見せるかどうかはサーバー側だけで決める。
  // クライアントのフラグを信用すると、無料プランでも展開できてしまう。
      ...
      try {
        chunks = await ai.models.generateContentStream({
          ...
          config: {
            ...
            abortSignal: req.signal, // ← SDK 側にも中断を伝える
          },
        });

        for await (const chunk of chunks) {
          // ブラウザバックやタブを閉じた場合、ここで生成を打ち切る
        ...
        // 中断されていても、そこまでに生成した分は必ず保存する。
        // ここを finally に置いていないと、離脱したユーザーの履歴だけ穴が空く。
...
`finally` の中で保存しているのが要点です。当初は正常終了の直後に保存していたのですが、途中で離脱したユーザーの会話だけ履歴が欠けるという状態になり、問い合わせ対応のときに何が起きたのか追えませんでした。途中まででも残っていれば、少なくとも「どこで止まったか」は分かります。

クライアント側は、`AbortController` を fetch に渡し、コンポーネントのアンマウントと停止ボタンの両方で中断できるようにしておきます。

```tsx
// app/ask/useAsk.ts（抜粋）
const controllerRef = useRef<AbortController | null>(null);

async function ask(question: string, plan: "free" | "pro") {
  controllerRef.current?.abort();          // 連打時は前のリクエストを畳む
...
useEffect(() => () => controllerRef.current?.abort(), []); // 画面離脱で確実に止める
```

保存側のスキーマは、本文と思考を分けて持つだけで足ります。PostgreSQL なら次の形です。

```sql
-- 思考は metadata の中に閉じ込め、本文の全文検索インデックスから外す
...
-- 本文だけを全文検索の対象にする（思考を混ぜるとノイズが増える）
create index messages_content_fts on messages using gin (to_tsvector('simple', content));
```

思考を `metadata` に閉じ込めておくと、あとから「思考だけ一括で消す」「特定プランのユーザーの思考だけ返す」といった要求に、マイグレーションなしで応えられます。運用が長くなるほど、この一段のクッションが効いてきます。
...
四つを並べると、どこから手を付けるかが判断しやすくなります。

<table>
<thead>
<tr><th>指標</th><th>取り方</th><th>手元での目安</th><th>下回ったときの打ち手</th></tr>
</thead>
<tbody>
<tr><td><strong>思考展開率</strong></td><td>回答表示イベントに対する展開イベントの比</td><td>重要画面 35% / 軽い画面 8%</td><td>5% を切る画面は思考をオフにしてコストを回収</td></tr>
<tr><td><strong>回答採用率の差</strong></td><td>展開した群と、しなかった群のコピー・保存率</td><td>差が 10 ポイント以上あれば効果あり</td><td>差が出ないなら、思考の粒度か文言を見直す</td></tr>
<tr><td><strong>「なぜ」型の問い合わせ</strong></td><td>サポートのタグ集計を四半期で比較</td><td>導入四半期で 3〜4 割減</td><td>減らないなら、回答本文側の説明が足りていない</td></tr>
<tr><td><strong>思考の再現性</strong></td><td>同一質問を週次で 5 回投げ、要点の一致率を目視</td><td>3/5 以上で要点が一致</td><td>揺れが大きければ thinking_budget を明示指定に</td></tr>
</tbody>
</table>
