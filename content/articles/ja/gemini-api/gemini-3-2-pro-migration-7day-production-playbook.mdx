...
highlights:
  - "2.5 Pro で動いていたプロンプトが 3.2 で壊れる箇所を本番反映前に検出する自動評価ハーネスを、Day 1 から動かせるようになります"
  - "実トラフィックの 5% を 3.2 Pro へ流し、24 時間でコスト・品質・レイテンシを実測してから本格移行を判断できる仕組みを構築できます"
  - "問題発生時に 5 秒で 2.5 Pro へ戻せるフィーチャーフラグ付きロールバック機構で、ユーザー影響を最小化した本番移行を実現できます"
---
...
その後、1 週間かけて「壊さずに 3.2 Pro へ移行する」ための仕組みをきちんと作り直し、今は段階的に移行を進めています。本記事では、その実装プレイブックを、検証済みのコードと共に共有させていただきます。同じように本番運用中のシステムをモデル更新する方の参考になれば幸いです。
