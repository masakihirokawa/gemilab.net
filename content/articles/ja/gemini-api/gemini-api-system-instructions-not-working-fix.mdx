...
System Instructions は Gemini API でモデルの振る舞いを制御するための最も強力な手段ですが、実装の落とし穴がいくつかあります。「設定したはずなのに効かない」という状況のほとんどは、**設定自体が間違っているのではなく、API への渡し方に問題がある**ケースです。本記事では、よくある4つの原因パターンとその解決法を紹介します。
