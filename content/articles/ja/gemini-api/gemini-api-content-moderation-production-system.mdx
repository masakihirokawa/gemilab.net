...
## 取り組みの背景 — なぜ Gemini API でコンテンツモデレーションなのか
...
レート制限エラーへの対処は [Gemini API エラーハンドリングとレート制限対応パターン](/articles/gemini-dev/gemini-api-error-handling-rate-limit-production-patterns) に詳しくまとめています。モデレーションシステムではエラー時に「安全側」（REVIEW 判定）に倒す点が肝心です。
