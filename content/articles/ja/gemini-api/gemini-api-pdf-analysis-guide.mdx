---
title: "Gemini API で PDF を解析・要約する — Python で始めるドキュメント AI"
...
大量のファイルを処理する場合は、API のレート制限に注意してください。1 分あたりのリクエスト数には上限があるため、`time.sleep()` で適切な間隔を設けるとよいでしょう。大規模なバッチ処理には [Gemini Batch Processing API](/articles/gemini-api/gemini-batch-processing-api) の活用も検討してみてください。
