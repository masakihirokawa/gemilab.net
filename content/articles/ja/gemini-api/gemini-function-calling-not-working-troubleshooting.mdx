...
この記事では、Function Callingでよく起こるトラブルを症状・原因・解決手順の順で丁寧に解説します。コード例を確認しながら、一つずつ問題を潰していきましょう。
...
**Q1. `gemini-2.5-flash` でFunction Callingが動かないのですが、モデルを変えるべきですか？**

A. Gemini 2.5 Flash もFunction Callingをサポートしています。ただし複雑なスキーマや多段階のツール呼び出しでは、Gemini 2.5 Proの方が安定した動作が期待できます。まずはスキーマの `description` を改善してから、モデル変更を検討してください。

**Q2. Function Callingのコストはどう計算されますか？**

A. ツール定義もトークンとしてカウントされます。スキーマが複雑になるほどリクエストごとのコストが上がります。[Gemini APIの料金と課金を完全解説](/articles/gemini-api/gemini-api-pricing-billing-complete-guide-2026)も参照してください。

**Q3. 並列ツール呼び出し（Parallel Function Calling）はできますか？**

A. はい、Gemini 2.5以降は複数のツールを同時に呼び出すことができます。レスポンスに複数の `function_call` パートが含まれる場合があるので、ループで処理してください。

**Q4. Function Callingとシステムインストラクションを組み合わせるとおかしくなります。**

A. システムインストラクションでツールの使用ルールを明示することで改善できます。例: 「ユーザーが商品を探す場合は必ず `search_products` ツールを使用し、テキストだけで回答しないでください。」

**Q5. Vertex AI経由でFunction Callingを使うと構文が違いますか？**

A. はい、Vertex AI SDKでは`tools`の指定方法や`FunctionDeclaration`の構造が若干異なります。[Vertex AI + Geminiの比較ガイド](/articles/gemini-api/gemini-api-vs-vertex-ai-comparison-guide)で詳しく解説しています。
...
Function Callingのさらなる活用法については、[Gemini APIのFunction Calling完全ガイド](/articles/gemini-api/gemini-api-function-calling-complete-guide)も合わせてご覧ください。

本記事の実装をさらに深めたい方には、[「詳解 Google Gemini API プログラミング」（著者: Google AI公式）](https://www.amazon.co.jp/dp/B0CSYLNWT6/?tag=pinocchio-22)がおすすめです。APIの設計思想から実践的なFunction Calling実装まで体系的に学べます。
