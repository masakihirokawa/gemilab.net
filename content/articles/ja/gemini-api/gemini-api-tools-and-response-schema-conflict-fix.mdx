...
この記事では、なぜ Gemini API がこの 2 つを同時指定できないのかという仕様の背景と、本番運用で実際にうまく動いている 3 通りの設計パターンをコード付きで紹介します。読み終えたあとには、Function Calling と構造化出力を矛盾なく両立させる実装方針を、自分の用途に合わせて選べるようになるはずです。
