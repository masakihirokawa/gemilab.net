...
この記事では、Zod を使って Gemini API のレスポンスをランタイムで検証する実装パターンをまとめます。スキーマを Zod 側に一元化することで、Gemini に渡す JSON Schema と TypeScript の型定義の両方を 1 つの真実から導けるようになります。
...
書籍で体系的に Zod や TypeScript の設計パターンを学びたい方には [プロを目指す人のためのTypeScript入門（鈴木 僚太）](https://www.amazon.co.jp/dp/4297127474/?tag=pinocchio-22) が、Zod の `transform` や条件付き型の使いこなしにつながります。
