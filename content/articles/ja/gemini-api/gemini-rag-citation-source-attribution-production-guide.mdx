...
highlights:
  - "「Geminiが事実を作ってしまう」と諦めかけていた方が、出典を強制的に付与する実装パターンを今日コピペで導入できる"
  - "構造化出力 + 引用検証の二段構えで、ハルシネーションを定量的に検出する仕組みを習得できる"
  - "本番運用で発生しやすい引用ズレ・ID欠落・冗長引用の3つの落とし穴を、具体的な対策コードとともに回避できる"
---
...
この記事では、Gemini を使った RAG パイプラインに「信頼できる出典」を組み込むための実装パターンを、コードと運用の両面から整理していきます。読み終わるころには、ハルシネーションを定量的に検出できる引用検証パイプラインを、ご自身のプロダクトに組み込んでいただけるはずです。
...
この記事では、本番運用に堪えるアプローチC を中心に解説していきます。アプローチB の実装は「アプローチC から `quoted_span` を抜いただけ」と捉えていただいて構いません。
