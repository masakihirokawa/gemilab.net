...
同期処理で書いた分類バッチを走らせたまま、夜中に一度だけ様子を見に行ったことがあります。進捗の行はゆっくりとしか増えておらず、このまま朝まで置いても終わらないことだけは分かりました。個人開発で続けている壁紙アプリのコンテンツ更新で、Gemini API に画像のカテゴリ分類をさせようとしていたときのことです。

最初のバージョンは Python の同期処理で書いておりました。1 リクエストの応答が平均2秒、対象が1,000枚。画像の前処理とローカル I/O を足すと、推定で6時間を超える計算になります。カタログの更新はほぼ毎週発生しますので、この所要時間では運用に乗りません。

asyncio へ書き換えたところ、同じ枚数を81分で終えられるようになりました。ここでは、その書き換えで実際に効いた実装パターンと、本番で詰まった箇所を、なぜそう書くのかという理由まで含めて書き残します。
...
本記事のパイプラインを組んだ2026年5月の時点では、`google-generativeai` の Python SDK は同期クライアントのみを提供しておりました。asyncio で使うには `asyncio.to_thread()` で同期関数をスレッドプールへ逃がすのが、最も安定した方法でした。

この前提は現在は変わっております。後継の `google-genai` には native async のクライアントがあり、`to_thread()` は不要になりました。移行時の注意点は後半の「`asyncio.to_thread()` の前提が変わりました」の節にまとめておりますので、これから新規に書く方はそちらを先にご覧ください。以下はレガシー SDK を使い続ける場合の、いまも有効な形です。
...
FENCE = "`" * 3  # モデルが付けてくるマークダウンのコードフェンス
            ...
            for attempt in range(5):
                try:
                    ...
                    if json_str.startswith(FENCE):
                        json_str = json_str.split(FENCE)[1]
...
実際に1,200枚を処理した中で判明したのが、`gemini-2.5-flash` のJSON整形問題です。プロンプトに「JSONのみを返してください」と明記しても、3.2%の件数でマークダウンのコードフェンス（バッククォート3つに `json` を添えた、あの囲み）に包まれた形で返答が来ました。

これは公式ドキュメントには記載がありません。私が発見したのは、500件のバッチを初めて流した後に50件近くのパースエラーが出たときでした。`raw.strip().startswith(FENCE)` でコードフェンスを検出して取り除く処理を足したところ、その後は同じ問題が再発しなくなりました。
...
公式ドキュメントには書かれていない、実際に詰まったポイントを三つ挙げます。いずれも「知っていれば5分で避けられる、知らないと数時間溶かす」種類の問題です。
...
この経験を通じて感じたのは、非同期処理は複雑さを増やすけれども、その複雑さは投資に値する、ということです。同期処理が立ち行かなくなるのはある日突然で、コンテンツが増えた瞬間にバッチが終わらなくなります。asyncio の初期設計コストを先に払っておくと、その限界が来る地点を大きく先送りできます。
...
random.shuffle(image_paths)        # シャッフルによる TPM 集中の分散
```
...
なお、この「包む」前提は現在では不要になりました。後継の `google-genai` は `client.aio` に非同期版を持っており、`asyncio.to_thread()` を経由せずに直接 `await` できます。スレッドプールのオーバーヘッドはそのぶん完全になくなります。具体的な移行手順は次の節に書きました。
...
道具は目的に合わせて選ぶ——この当たり前が、コードの設計でもそのまま効くのだと感じています。asyncio は強力な道具ですが、すべての場面で必要な道具ではありません。ここで紹介したパターンが、皆さまのプロジェクトに asyncio が要るタイミングを見極める材料になれば幸いです。


## `asyncio.to_thread()` の前提が変わりました——`google-genai` の native async へ

ここまでのコードは `google-generativeai` を前提にしております。その前提そのものが、いまは動いております。

Google の[新しい SDK への移行ガイド](https://ai.google.dev/gemini-api/docs/migrate)は、後継の `google-genai` がすべての対応プラットフォームで一般提供に入ったこと、そしてレガシーのライブラリを使っているなら移行を強く推奨することを明記しています。asyncio についても、`client.aio` の下にすべてのメソッドの非同期版が用意されている、と書かれています。

つまり、スレッドプールへ逃がす理由がなくなりました。ネットワーク待ちをイベントループが直接扱えます。
...
client = genai.Client()  # GEMINI_API_KEY 環境変数から読み込み

async def classify_one(image_bytes: bytes, prompt: str) -> str:
    # native async クライアントでの 1 枚分の分類（to_thread 不要）
...
Semaphore のパターンは、そのまま残してください。SDK が変わってもレート制限は変わりませんので、並列数の上限を自分で決める必要は同じだけあります。変わったのは「どうやって非同期にするか」だけです。

### 移しただけで動かなくなるのは、リトライの except 節です

移行でいちばん気をつけたいのは、モデル名でも `contents` の書き方でもありませんでした。**リトライの `except` 節**です。

前述のリトライ関数は `google.api_core.exceptions.ResourceExhausted` を捕まえています。これはレガシー SDK が投げる例外で、`google-genai` は投げません。新しい SDK の例外は `google.genai.errors` にあり、レート制限は `APIError` の派生である `ClientError`（`code` が 429）として届きます。

厄介なのは、移行した直後は何も壊れて見えないことです。正常系は通り、テストも緑のまま残ります。429 が返ってきた瞬間にだけ、捕まえるはずだった `except` 節を素通りして、バッチ全体が落ちます。私がこれに気づいたのは、それまで自動で回復していた夜間バッチが、ある朝だけ途中で終わっていたときでした。
...
async def call_with_retry(
    ...
    # google-genai の例外階層に合わせたリトライ（code で一時的かどうかを判定）
    for attempt in range(max_retries + 1):
        try:
...
`e.code` で分岐させておくと、400番台の入力エラー（壊れた画像・サイズ超過）を一時的な障害と取り違えて延々と再試行してしまう事故も、同時に防げます。`e.message` には API が返した説明がそのまま入りますので、ログにはこの二つを並べて出しておくと、翌朝に原因を辿れます。

自分のコードに死んだ `except` が残っていないかは、2行で確かめられます。
...
前者が残っていて後者が消えている状態が、いちばん危ない組み合わせです。**インポートは移せても、例外は移りません。** この確認だけは、SDK を入れ替えるたびに必ず走らせるようにしております。

<table>
<thead>
<tr><th>項目</th><th>レガシー（<code>google-generativeai</code>）</th><th>現行（<code>google-genai</code>）</th></tr>
</thead>
<tbody>
<tr><td>非同期の作り方</td><td><code>asyncio.to_thread()</code> でスレッドプールへ</td><td><code>client.aio.models.generate_content()</code> を直接 await</td></tr>
<tr><td>スレッドのオーバーヘッド</td><td>1コルーチンあたり数ミリ秒</td><td>なし</td></tr>
<tr><td>レート制限の例外</td><td><code>api_core.exceptions.ResourceExhausted</code></td><td><code>errors.ClientError</code>（<code>code == 429</code>）</td></tr>
<tr><td>一時障害の例外</td><td><code>api_core.exceptions.ServiceUnavailable</code></td><td><code>errors.ServerError</code>（<code>code</code> が 500 / 503）</td></tr>
<tr><td>並列数の制御</td><td colspan="2"><code>asyncio.Semaphore</code>（どちらも自分で上限を決める必要あり）</td></tr>
<tr><td>後始末</td><td>不要</td><td><code>await client.aio.aclose()</code>、または <code>async with</code> で閉じる</td></tr>
</tbody>
</table>

書き換えそのものは半日で終わりました。時間がかかったのは、`except` を直したあとに「本当に 429 で再試行できているか」を確かめる作業のほうです。ここは実際に 429 を起こしてみるのがいちばん早く、並列数を一時的に上げて数十件だけ流し、リトライのログが出ることを目で見て確かめました。移行の前後で並列数と処理時間は変えておりませんが、スレッドプールを挟まなくなったぶん、メモリの上振れは素直に小さくなりました。

## 全体を振り返って：まず小さなバッチで動かしてみる

ここまでの asyncio パターンをまとめますと、核心は5つです。第一に、いま使っている SDK がどちらなのかを確かめること（`google-genai` なら `client.aio` で `to_thread()` は不要です）。第二に `asyncio.Semaphore` で並列数を制御してレート制限を守ること。第三に指数バックオフとジッターで一時的なエラーから回復すること。第四に処理結果をファイルへ逐次書き出して部分失敗を許容すること。そして第五に、リトライの `except` が自分の SDK の例外階層と噛み合っているかを確かめることです。

これらを一度に全部実装する必要はありません。まずは非同期のクライアント（レガシー SDK なら `asyncio.to_thread()`）と `asyncio.Semaphore` だけで試してみることをお勧めします。10〜50件のリストと並列数5から始め、同期処理と比べた速度差を体感してから、バックオフや部分失敗処理を追加していくアプローチが現実的です。Gemini API の asyncio 活用は、コンテンツを多く扱う個人開発者にとって確実に投資対効果のある技術です。少しずつ積み上げていきましょう。
