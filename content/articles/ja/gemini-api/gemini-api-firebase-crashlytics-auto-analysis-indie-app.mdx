...
author: "Gemini Lab"
description: "Firebase Crashlytics の生ログを Gemini API に渡して根本原因とコード修正案を自動生成するパイプラインを、累計 5,000 万 DL の壁紙アプリ v2.1.0 の実開発で検証した記録です。RecyclerView クラッシュの Before/After 付き。"
...
私は2014年から累計5,000万DLを超えるスマートフォンアプリを個人で開発・運営しています。壁紙アプリ「Beautiful HD Wallpapers」の Android 版 v2.0.0 をリリースした直後、Crashlytics の Issues タブに `java.lang.IndexOutOfBoundsException: Inconsistent detected. Invalid view holder adapter position` という行が静かに増えていきました。RecyclerView のアダプターに何かが起きている。でも「何が」おかしいのか、スタックトレースだけでは見えてきませんでした。
...
入力したスタックトレース（抜粋）と OS バージョン別分布（Android 10: 38%、Android 11: 29%、Android 12: 22%、その他: 11%）を渡したところ、Gemini 2.5 Pro から `confidence: "high"` の回答が返ってきました。根本原因として「WallpaperAdapter が保持する wallpapers リストがバックグラウンドスレッドから直接変更されており、UI スレッドが保持するビューホルダーの位置情報と不整合が生じている。防御的コピーが欠如しており、外部から渡されたリストの参照をそのまま保持しているのが原因」と診断されました。
...
宮大工だった両祖父が「手を動かすことが一つの信心」と言っていたことを、アプリ開発をしていると時々思い出します。ツールを整備して仕組みを作ることも、その「手を動かすこと」の一部だと感じています。クラッシュ対応の時間を削減することで、新機能の開発やユーザー体験の改善に集中できる時間が増えます。そのための投資として、このパイプラインを作る価値は十分にあると思っています。

2014年から個人開発を続けてきた中で、クラッシュ対応は常に「気づくのが遅れる」問題でした。ユーザーが既に離れた後に初めて問題に気づく、という経験を何度もしてきました。Gemini API を使った自動分析は、その「気づきの遅れ」を構造的に解消する手段の一つです。同じ課題に取り組んでいる方の参考になれば幸いです。お読みいただきありがとうございました。

2014年から12年、累計5,000万DLのアプリを一人で運営してきた経験から言えば、「問題の特定」に費やす時間を削れるだけで、体感できるほどリリースサイクルが短くなります。このパイプラインが同じ課題に取り組んでいる方の参考になれば幸いです。
