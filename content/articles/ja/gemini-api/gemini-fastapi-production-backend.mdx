...
この2つを組み合わせることで、**本番環境で動く実用的なAIバックエンド**を構築できます。ここでは単純なチャットボットの実装にとどまらず、ストリーミングレスポンス、レート制限、リトライロジック、Function Calling、コスト最適化、Dockerデプロイまで、プロダクションに必要なすべての要素を段階的に解説します。
