...
highlights:
  - "Few-Shot を増やすほどコストと精度がトレードオフになる本番の壁を、入力ごとに最適な実例を3〜5件だけ選ぶ動的設計で解決できる"
  - "Gemini Embeddings + Vertex AI Vector Search を使った例の検索・選択・更新の3段ループをコピペで動く実装で構築できる"
  - "本番のフィードバックを次の Few-Shot に取り込む「育てる」運用フローが手に入り、リリース後も精度が下がらないどころか伸びていく状態を作れる"
---
...
最初は同期ループだけ実装すれば動きます。残り2つは運用が回り始めてから足していくのが現実的です。本記事ではまず同期ループを完全に動く形で書き、その後で残り2つを追加実装していきます。
