...
この記事では、Apps Script を一切使わず、**Python だけで Google Sheets の読み書きと Gemini API による AI 処理を連携させる**方法を解説します。スケジューラに乗せてバッチ実行したり、FastAPI サーバーのエンドポイントとして組み込んだりと、自由度が一気に広がります。
