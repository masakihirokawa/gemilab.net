...
3つのアプローチが有効です。①バッチサイズを増やして API 呼び出し回数を減らす（ただし精度とのトレードオフに注意）、②コンテキストキャッシュで固定プロンプトのトークンを削減する、③分類・フィルタリングなど軽量タスクには `gemini-2.5-flash-8b` を使い、複雑な推論が必要な場合のみ `gemini-2.5-pro` にルーティングする「モデルカスケード」を導入する。

---

## まとめ

本記事では、Apache Kafka と Gemini API を組み合わせたリアルタイム AI イベントストリーミングパイプラインの設計と実装を解説しました。主要なポイントを整理します。
