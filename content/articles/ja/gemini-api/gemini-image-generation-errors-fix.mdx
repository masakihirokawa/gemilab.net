---
title: "Gemini 画像生成でつまずく5つのエラー — 429 から安全フィルターまで"
...
noindex: true
author: "Gemini Lab"
description: "Gemini API の画像生成でつまずきやすい5つのエラーを整理しました。429 RESOURCE_EXHAUSTED のクォータ管理、安全フィルターによるブロック、出力画像の破損、Model not found、タイムアウトについて、原因と実装レベルの対処を示します。"
tags: ["gemini", "image-generation", "troubleshooting", "429", "error", "imagen"]
premium: false
---
## 画像生成が急に通らなくなった背景

昨日まで動いていた画像生成のコードが、今日は 429 RESOURCE_EXHAUSTED を返し続ける。2025 年 12 月に無料ティアの IPM 制限が変更されて以降、この報告が目立って増えました。

原因はクォータだけではありません。安全フィルターによるブロック、出力データの破損、モデル名の指定ミス、生成時間の長期化。遭遇しやすい5つのエラーについて、**実装レベルの対処パターン**を順にまとめました。
