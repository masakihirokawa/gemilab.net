...
highlights:
  - "Gemini API をサーバレスに載せて『最初の1リクエストが6秒固まる』問題で詰まっていた人が、min-instances・SnapStart・グローバルスコープ初期化を今日から実装できる"
  - "Cloud Run / AWS Lambda / Cloudflare Workers それぞれのコールドスタート計測・最適化パターンを、実測値ベースで判断できる"
  - "『min-instances を増やすだけ』では解決しない理由と、SDK 初期化を遅延化するホットパス削減テクニックを習得できる"
---
