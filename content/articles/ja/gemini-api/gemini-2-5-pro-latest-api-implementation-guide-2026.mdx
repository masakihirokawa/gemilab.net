...
highlights:
  - "`gemini-2.5-pro-latest` と固定バージョンID（gemini-2.5-pro-002 等）の挙動差と、本番で起きうる事故"
  - "モデル更新時に黙って壊れる出力フォーマットの検出方法（contract test の組み方）"
  - "generativelanguage.googleapis.com 経由と Vertex AI 経由でのモデルIDマッピング"
  - "`-latest` を安全に使うフォールバック設計とコスト管理パターン"
  - "Pinning と Auto-update の使い分け判断基準"
---
...
エイリアスモデルIDは、開発時には便利ですが、本番で使うには明確な設計が必要です。この記事では、`gemini-2.5-pro-latest` を含む `-latest` 系IDの挙動と、安全に使うための実装パターンを `generativelanguage.googleapis.com`（旧 Generative Language API）と Vertex AI の両方の文脈で整理します。
