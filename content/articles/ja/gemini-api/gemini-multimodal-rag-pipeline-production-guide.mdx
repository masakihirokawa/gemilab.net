...
本記事では、ドキュメント処理からベクトルインデックス構築、検索・生成パイプラインまでを、Pythonの実装コード付きで解説します。[Function Callingの基本](/articles/gemini-advanced/function-calling-guide)を理解している前提で進めますので、初めての方はそちらを先にご覧ください。
...
### Q1: マルチモーダルRAGで最もコストがかかるのはどのフェーズですか？

ドキュメントの初回処理（Ingestフェーズ）が最もコスト高です。特に動画ファイルの分析はトークン消費が大きくなります。一度処理したドキュメントのエンベディングとテキスト表現をキャッシュし、更新がない限り再処理しない設計が重要です。検索・生成フェーズは、Context Cachingとモデルの段階的使い分けで大幅にコスト削減できます。

### Q2: Gemini Embeddings APIとOpenAI Embeddings APIの違いは？

Gemini の `text-embedding-004` は768次元のベクトルを生成し、`task_type`パラメータで検索クエリ用とドキュメント用を明示的に区別できます。OpenAI の `text-embedding-3-small` は1536次元です。精度面では同等ですが、Geminiは`task_type`の指定により検索精度が向上する傾向があります。コスト面では各サービスの最新の料金表を確認してください。

### Q3: 動画の処理でGeminiのコンテキスト長を超える場合は？

長時間の動画（2時間超等）では、まず動画を一定間隔のセグメントに分割し、セグメントごとにGeminiで分析します。Gemini 2.5 Proの100万トークンのコンテキストウィンドウは大半の動画に対応できますが、超える場合はフレーム抽出 + 画像分析のパイプラインに切り替えてください。

### Q4: 本番環境でのベクトルDBの選択肢は？

規模と要件に応じて選択してください。小〜中規模（100万チャンク以下）なら**pgvector**（PostgreSQL拡張）がSupabaseと相性がよく運用コストも低いです。大規模（1000万チャンク以上）でフィルタリングが複雑な場合は**Pinecone**や**Weaviate**が適しています。まずはpgvectorで始め、スケールに応じて移行するアプローチがおすすめです。
