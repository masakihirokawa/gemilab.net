...
highlights:
  - "Gemini のマルチモーダル機能（画像・動画・音声）を活かした収益化アプリの設計図を、ゼロから組み立てられるようになる"
  - "Stripe + Firebase + Gemini API のフルスタック実装パターンを、課金フロー・残高管理・障害対応まで完全コード付きで習得できる"
  - "「マルチモーダルは面白いが収益化に繋げられない」状態から脱却し、月次黒字化までの具体的な道筋に応用できる"
---
...
実装と運用の両方を体系的に学びたい方には、[『SaaS スタートアップの全戦略（Jason Lemkin 著）』](https://www.amazon.co.jp/dp/4799329421/?tag=pinocchio-22)が参考になります。マルチモーダル収益化に直接言及はありませんが、サブスクリプション設計の原理原則は普遍的で、価格設定や解約防止の章は AI アプリにもそのまま応用できます。
