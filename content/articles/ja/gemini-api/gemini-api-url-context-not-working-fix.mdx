...
この記事では、URL Context が「動いていないように見える」典型的な3パターンと、それぞれを切り分けて修正するための確認手順を、実際に現場で詰まったポイントを交えて整理します。導入そのものの解説は[Gemini API の URL Context 入門ガイド](/articles/gemini-api/gemini-url-context-guide)にまとめてありますので、まずは動かし方を確認したい方はそちらを先にお読みください。
