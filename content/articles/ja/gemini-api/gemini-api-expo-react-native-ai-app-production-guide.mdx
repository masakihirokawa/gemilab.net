...
highlights:
  - "ExpoでGemini APIのストリーミング実装に詰まっていた方が、動作するコードと具体的な落とし穴の解決策を手に入れられます"
  - "AdMob課金とAPIコスト管理を両立したフリーミアム設計パターンを習得できます"
  - "AIアプリとしてApp Store／Google Playの審査を通過するために必要な準備と対策を事前に把握できます"
---
...
この記事では、Expo環境でGemini APIを使ったAIチャットアプリを実際に作り、App Store・Google Playの審査を通過するまでの実装記録をまとめます。公式ドキュメントには書かれていない、実際の開発で詰まったポイントを中心に解説します。
