...
**Q. `gemini-2.5-pro-latest` と `gemini-2.5-pro` の違いは？**

`gemini-2.5-pro-latest` はモデルの最新バージョンを自動で使用するエイリアスです。一方、`gemini-2.5-pro-20250415` のような特定バージョン名を指定すると、そのバージョンのモデルが固定されます。本番環境では動作の一貫性を保つために特定バージョンを指定する方が安全です。開発・実験段階では `latest` エイリアスで最新機能を試せます。

**Q. 無料プランでも `gemini-2.5-pro-latest` は使えますか？**

Google AI Studio の無料枠では `gemini-2.5-pro-latest` へのアクセスが制限または利用できない場合があります。有料の Google Cloud Vertex AI や Gemini API の有料プランへの移行が必要になることがあります。最新の料金・無料枠については [Google AI Studio の価格ページ](https://ai.google.dev/pricing) をご確認ください。

**Q. APIのレートリミットはどのくらいですか？**

2026年4月時点での Gemini 2.5 Pro の無料枠は、1分あたり2リクエスト（RPM）、1日あたり50リクエスト（RPD）が目安です。有料プランではより高いレートが利用できます。本番環境では指数バックオフを実装したリトライ処理を必ず組み込んでください。
