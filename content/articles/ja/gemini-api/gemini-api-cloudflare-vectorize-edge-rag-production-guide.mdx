...
連続して Embedding API が落ちている状況で、Workers が呼び続けるのは無駄です。Durable Objects に状態を持たせて、一定時間は呼び出しをスキップするサーキットブレーカーを薄く実装しています。詳細は [Gemini API のリジリエンス設計（サーキットブレーカー＋バルクヘッド）](/articles/gemini-api/gemini-api-resilience-circuit-breaker-bulkhead-production-guide) で書きましたので、ここでは「入れている」とだけ触れておきます。
