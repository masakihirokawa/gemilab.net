...
ベクトル検索の基本については [Gemini Embedding × Pinecone のRAG実装](/articles/gemini-api/gemini-api-pinecone-rag-production) と [Qdrant ハイブリッドRAG](/articles/gemini-api/gemini-api-qdrant-hybrid-rag-production-system) に書きましたので、未読の方はそちらを先に読むと前提が揃います。本記事はそれらの**続き**として、検索した後の「順位を直す」レイヤーに焦点を当てます。
