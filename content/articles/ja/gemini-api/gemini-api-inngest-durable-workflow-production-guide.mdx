...
`NonRetriableError` を使い分けることが重要です。Gemini API が `400 INVALID_ARGUMENT` を返したケース（プロンプトの形式が不正）は、何度リトライしても結果は変わりません。即座に dead-letter に流して人間に判断を委ねる設計のほうが、無駄な API 課金を抑えられます。
...
書籍で体系的に学ぶなら、AI ワークフロー全般の設計思想は[実践入門 OpenAI APIによるLLMアプリ開発（吉田真吾）](https://www.amazon.co.jp/dp/4798070157/?tag=pinocchio-22)が、API呼び出しの周辺パターンは[実践 LLMアプリ開発入門（瀬戸口竜也）](https://www.amazon.co.jp/dp/4297143488/?tag=pinocchio-22)が参考になります。Inngest 自体ではなく、長時間ジョブ全般の考え方を整理したい方におすすめです。
