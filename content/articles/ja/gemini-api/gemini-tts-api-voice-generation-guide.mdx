...
従来のテキスト読み上げ（Google Cloud TTSなど）は、あらかじめ録音された音声データを組み合わせるため、音質は高いものの表現力が限定的でした。一方、Gemini TTSは生成型AIの技術を活用し、プロンプトでスタイルや感情を指定することで、より人間らしく柔軟な音声出力が可能です。
...
**Q1: Gemini TTSは日本語対応していますか？**

A: はい。Gemini 2.5 Flashは日本語を含む100以上の言語に対応しており、日本語音声生成も高品質です。

**Q2: 生成した音声は商用利用できますか？**

A: はい。Gemini APIの利用規約に従う限り、ポッドキャスト販売や映画ナレーション等の商用利用が可能です。

**Q3: APIキーが誰かに漏れたらどうすればいい？**

A: Google AI Studioダッシュボードから該当キーを無効化し、新しいキーを生成してください。

**Q4: 学習・教育目的での利用は無料ですか？**

A: 非営利教育機関向けのGrant Programがあります。詳細は [Google Research Education](https://research.google.com/edu) をご覧ください。
