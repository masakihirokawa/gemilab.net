...
highlights:
  ...
  - "余白なしの切り抜きが 93.77% のボックスで内容を削るという 20,000 件の実測と、1 目盛りの余白で 0% にする描画・保存コードが手に入ります"
...
Gemini が向いているのは、概念的な指示で物体を引き当てたい場面です。たとえば「このレシートの『合計金額』が書かれている領域を返して」「キッチンの写真から『ガスコンロの取手』だけを切り出して」のように、クラス名がカテゴリで定義しきれない、または事前学習されていない用途に強みがあります。専用検出器は事前にラベルを与えていないと動きませんが、Gemini はゼロショットで意味を解釈してくれます。
...
実装方針としては、私は次のように使い分けています。低レイテンシ・高頻度の検出は専用モデルに任せ、人間の意図に近い「探したい対象」が来たときだけ Gemini を呼びます。両者を組み合わせるのが現実的だと感じています。後述するフォールバック設計はこの考え方を前提にしています。
...
### 切り抜きが一皮足りない — 0〜1000 格子の丸めを見積もる

レシートの合計金額を切り出して OCR に回す処理を書いていた夜、切り抜いた画像を並べて眺めていて手が止まりました。ボックスの位置は合っているのに、数字の上端がほんの少しだけ削れている画像が混じっていたのです。プロンプトもパーサも疑いましたが、原因はもっと手前にありました——`[ymin, xmin, ymax, xmax]` が整数で返ってくる、という規約そのものです。

0〜1000 は連続値ではなく、1000 分割の格子です。幅 3024 ピクセルの写真なら、1 目盛りが約 3.02 ピクセルに相当します。モデルが真の境界を最も近い目盛りへ丸めた瞬間、辺ごとに半目盛り未満の誤差が入ります。そして丸めは、内側にも外側にも同じ確率で転びます。

どれくらい効くのかは、API を呼ばなくても測れます。真のボックスをピクセル座標で作り、0〜1000 へ量子化して戻し、元のボックスと比べるだけです。

```python
"""0〜1000 格子の丸めが切り抜きにどれだけ効くかを測る。API キー不要。"""
import random
import statistics
...
def measure(w, h, n=20000, pad_units=0, seed=42):
    ...
    for _ in range(n):
        ...
        # 復元したボックスが真のボックスを覆いきれなければ、内容を削っている
...
for w, h, label in [(1024, 768, "1024x768"),
    ...
    for pad in (0, 1, 2):
        ...
        print(f"{label} pad={pad} 最大={mx:.2f}px 平均={mean:.2f}px 削り={clip:.2f}%")
```

手元で走らせた結果です（各条件 20,000 ボックス、乱数シード固定）。

<table>
  <thead>
    <tr><th>画像サイズ</th><th>余白（目盛り）</th><th>辺の誤差 最大</th><th>辺の誤差 平均</th><th>内容を削ったボックスの割合</th></tr>
  </thead>
  <tbody>
...
いちばん目を引いたのは右端の列でした。余白を足さずに切り抜くと、画像サイズにかかわらず 9 割以上のボックスが、どこか一辺で内容を削っています。理屈のうえでも自然な数で、四辺それぞれが独立に内側へ丸まる確率が 1/2 ですから、少なくとも一辺が内側へ転ぶ確率は 1 −（1/2）の 4 乗 = 93.75% になります。実測の 93.77% は、その理論値にきれいに重なりました。

削れる量そのものは小さく、1024 ピクセルの画像なら辺あたり平均 0.22 ピクセルです。描画して人に見せるだけなら、誰も気づきません。ところが対象が細いと話が変わります。3024×4032 の写真に写った高さ 28 ピクセルの帯——レシートの一行はおおむねこの太さです——を切り抜くと、余白なしでは帯の面積の平均 3.70%、最悪 13.82% が失われました（同じスクリプトで `bw` を 600、`bh` を 28 に固定し、真のボックスと復元ボックスの重なりから失われた面積の割合を測った値です）。数字の上端が削れて桁が化ける、という OCR 側の誤読は、ここから来ていたのです。

**格子は連続値ではありません。切り抜きには、1 目盛り分の逃げを。** 逃がす量は 1 目盛りで足ります。丸め誤差は定義上どの辺でも半目盛り未満ですから、1 目盛りの余白があれば、理論上も実測でも削りはゼロになります。

```python
def to_pixel_box_padded(box_2d: List[int], img_w: int, img_h: int,
                        pad_units: int = 1):
    """切り抜き用。0〜1000 の格子で pad_units 目盛りだけ外へ広げてからピクセル化します。"""
    ymin, xmin, ymax, xmax = box_2d
...
ただし余白はただでは手に入りません。6000 ピクセル幅の画像では 1 目盛りが 6 ピクセルで、隣り合った商品を個別に切り出す場面では、その余白がそのまま隣の商品の写り込みになります。私は用途で分けています——画面への描画は素の `to_pixel_box`、OCR や検索エンジンへ渡す切り抜きは `to_pixel_box_padded`、そして対象どうしが接している棚の写真では、広げる前に `iou` で隣との重なりを確かめてから余白を決める、という順です。

後述する `smart_resize` との関係も、ここで整理しておきます。長辺を 1024 に縮めてから投げても、座標は正規化されて返るので、変換そのものの精度は落ちません。落ちるのはモデルが細部を見る力のほうです。一方で格子の粗さは元画像の大きさで決まりますから、縮小したかどうかに関わらず、切り抜きの余白は元画像のサイズを基準に考えることになります。
...
**間違い1 — 座標の Y/X 順を `(x, y)` だと思い込むこと。** 一番ハマりやすい罠です。Gemini は `[ymin, xmin, ymax, xmax]` で返してきます。`box_2d[0]` を X だと思って描画すると、ボックスが回転して見える、あるいは画像外にはみ出します。共有関数 `to_pixel_box` のように、変換関数を一箇所に閉じ込めるのが安全策です。

**間違い2 — マスクをそのまま元画像の座標系で使おうとすること。** マスクは「ボックスの中身にフィットさせた小さな PNG」です。元画像にそのまま貼り付けると左上にズレた小さな塊になります。`decode_mask_to_full_image` のように、ボックスサイズへリサイズしてから合成しないと意味がありません。

**間違い3 — `temperature` をゼロに設定すること。** 一見「決定的にしたい」と思いがちですが、ゼロだと検出が極端に保守的になり、ユーザーから見ると「全然見つけてくれない」アプリになります。0.1〜0.2 が経験上ちょうど良いです。

**間違い4 — 信頼度をそのまま「確率」だと信じること。** Gemini が返す `confidence` は較正された確率ではありません。アプリ内の閾値判断には使えますが、「0.85 の検出は 85% の確率で正しい」と統計的に主張するのは無理があります。`min_conf` で足切りに使う程度に留めましょう。

**間違い5 — 大きな画像をそのまま投げること。** 4000ピクセル級の画像を投げると、トークン消費だけでなく検出品質も微妙に落ちます。`smart_resize` で長辺を 1024〜1536 にしてから投げる前処理は、ほぼ常に入れる価値があります。

**間違い6 — 「ない物体」を聞いたときの挙動を決めていないこと。** Gemini はゼロ検出を渋ることがあり、無理矢理それらしい領域を返してくることがあります。プロンプトで「存在しない場合は空配列」と明記し、後段でも `filter_hallucinations` を通す——この二段構えがないと、本番で必ず誤検知が出ます。

**間違い7 — 並列リクエストでレート制限に引っかかること。** UI 側で「画像が選ばれたら即検出」を素直に実装すると、ユーザーが連続選択したときにレート制限を超えます。デバウンスとリクエストキャンセル（AbortController）を入れて、最後の選択だけが処理されるようにすると、UX もコストも改善します。

**間違い8 — 生のレスポンスをログに残していないこと。** 後段のフィルタや変換を重ねていくほど、「なぜこの結果になったのか」を後追いで調べられなくなります。私は入力画像のハッシュをキーに、Gemini からの生 JSON をオブジェクトストレージに保存しています。「AI が違う物を検出した」とユーザーから報告が来たとき、再課金せずに当時の応答を再生できるのは、デバッグの現実的な近道です。

これらの落とし穴は、関連する [Gemini API レート制限と 429 対策の運用ノート](/articles/gemini-api/gemini-api-rate-limiting-quota-management-production-guide) や [Gemini API のストリーミング応答をユーザー操作で中止する](/articles/gemini-api/gemini-api-streaming-cancel-abortcontroller-guide) と組み合わせて読むと、本番運用の地図がつながりやすくなると思います。
