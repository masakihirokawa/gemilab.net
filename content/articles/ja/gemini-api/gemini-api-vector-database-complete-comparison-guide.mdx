...
本記事では `text-embedding-004`（Geminiの最新埋め込みモデル）を統一条件として、Pinecone・Qdrant・pgvector（Cloud SQL）・Cloud Spanner Vectorの4つを実際に実装・計測した結果をまとめた。各DBのカタログスペックではなく、本番で動かして得た数値と、実際に痛い目に遭ってわかった落とし穴を共有します。
