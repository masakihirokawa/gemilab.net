...
[Gemini API 本番環境のオブザーバビリティ — ログ・監視・コスト追跡の実践パターン](/articles/gemini-api/gemini-api-observability-production-monitoring) では自前の構造化ログを中心に解説しましたが、ここではトレーシング専用ツールを使って一段深い観測を構築する方法に踏み込みます。実装は Python SDK を中心に、Node.js 側の注意点も補足します。本番に組み込んでしばらく運用してわかった落とし穴も含めて、正直に共有していきます。
...
`langfuse.flush()` を忘れると、短命プロセス（AWS Lambda・Cloud Functions）ではトレースが送信されないまま終了することがあります。ハンドラの最後で呼ぶか、`atexit` に登録しておくと取りこぼしません。
...
`from langfuse.openai import openai` にしておくだけで、OpenAI SDK の呼び出しが自動的に Langfuse にトレースされます。コード量が最小で済みますが、Gemini 固有の機能（Thinking Mode・Context Caching・Function Calling の一部形式）は OpenAI 互換経由ではフル活用できません。細かな制御が要る場面では、方式 A のほうが素直です。
...
Langfuse は主要モデルの単価表を同梱していて、Gemini もその対象です。既定の単価定義はリポジトリ側で管理され、**毎日の自動監査**が提供元の公開情報と突き合わせて差分を拾い、必要なときだけプルリクエストとして反映される、という運用になっています。ですので「新しいモデルが出るたびに自分で全部登録する」という前提は、いまはもう正確ではありません。

問題はその先の二つの性質にあります。

**一つめは、コストが取り込み時に確定することです。** Langfuse はトレースを受け取ったその瞬間に単価表を引き、計算した金額を保存します。あとから単価定義を直しても、**すでに保存されたトレースは計算し直されません**。私はここを長く取り違えておりまして、「新モデルの単価を登録すれば先月分も埋まる」と思い込んだまま月次を締めていた時期があります。埋まらない行を数えて、ようやくドキュメントを読み直しました。

**二つめは、値上げには施行日があることです。** 単価表は改定のたびに置き換わりますが、請求の側には「いつから」が付いています。期間限定の安い単価を前提に組んだパイプラインが、施行日をまたいだ翌月に倍の金額で返ってくる——という事故は、単価表を信じきっているほど起きやすいのです。価格に触れる設計をするときは、金額と一緒に**その金額がいつまでのものか**を控えるようにしています。

**単価表は真実の写しであって、真実そのものではありません。** 私は月に一度、Langfuse の集計値と提供元の請求額を並べて差を見る作業を、締め処理の一行として入れております。ずれが一定を超えたら、単価定義か、次の節で扱うトークンの数え方か、そのどちらかが疑わしい——という切り分けです。

### 200K 超で単価が変わるモデルは「価格ティア」で表す

Gemini 2.5 Pro のように入力トークン数で単価が変わるモデルは、ひとつの単価では表現できません。Langfuse には **価格ティア（Pricing Tiers）** があり、1 つのモデル定義に複数の価格帯を持たせられます。各ティアは名前・優先度・条件・価格を持ち、優先度順に評価されて、条件を満たした最初のティアが採用されます。どれも満たさなければ既定ティアが当たります。

条件に使える軸は三つです。

<table>
<thead>
<tr><th>軸</th><th>照合のしかた</th><th>Gemini での使いどころ</th></tr>
</thead>
<tbody>
<tr><td><strong>Usage details</strong></td><td>キーを正規表現で拾って値を合計し、<code>gt</code> / <code>gte</code> / <code>lt</code> / <code>lte</code> / <code>eq</code> / <code>neq</code> で比較します</td><td>入力が 200,000 トークンを超えたら長コンテキスト単価へ切り替えます</td></tr>
<tr><td><strong>Model parameters</strong></td><td>トップレベルのモデルパラメータのキーを、列挙した完全一致値と照合します</td><td>リクエストの属性で単価が変わる提供形態を表現します</td></tr>
<tr><td><strong>Metadata</strong></td><td>トップレベルのメタデータのキーを完全一致値と照合します</td><td><code>region</code> や自前の課金区分ごとに単価を分けます</td></tr>
</tbody>
</table>

ネストしたパスは条件に使えません。条件を複数置いた場合は、すべて満たしたときにだけそのティアが当たります。ここを一段だけ丁寧に作っておくと、長い資料を投げる機能と短いチャットの機能が同じダッシュボードに並んでも、金額が混ざらなくなります。

そのうえで効いてくるのが **機能単位の集計** です。`metadata.feature` にタグ付けしておけば、「article_summary が全体コストの 38%」「code_review が 17%」といった分解がダッシュボード上で即座に見えます。

コード側の実装ポイントは 1 点だけです。**Langfuse に送る `metadata` は常に平坦にし、粒度の粗いキーを使う**こと。`{"feature": "article_summary"}` は良い粒度ですが、`{"article_id": "12345"}` を直接メタデータに入れるとカーディナリティが爆発し、ダッシュボードのグルーピングが機能しなくなります。記事 ID のような高カーディナリティ情報は `input`/`output` 側に含めるか、`session_id` 経由で結びつけるほうが扱いやすくなります。

## Gemini のトークン内訳を、Langfuse の排他バケットへ引き直す

コストが提供元の請求より大きく出るとき、単価より先に疑うべきは**トークンの数え方**です。ここは Langfuse と Gemini で設計の前提がすれ違いやすく、私も実際に踏みました。

Langfuse の `usage_details` は、**キーごとに重ならないバケット**として扱われます。ドキュメントの言い方では、`input` は `input_*`（`input_cached_tokens` など）を含まず、`output` は `output_*` を含みません。`total` はバケットの合計であって、バケットの一つではない——これがこの仕組みの約束です。重なったまま渡すと、使用量も推定コストも二重に数えられます。

いっぽう Gemini の `usage_metadata` は**内包型**です。`prompt_token_count` は入力トークンの全体で、キャッシュから読まれた分もそこに含まれます。`cached_content_token_count` は、そのうちキャッシュ由来だった分を別に示す値です。思考の分は `thoughts_token_count` として別に返ります。

つまり `prompt_token_count` をそのまま `input` に入れると、Context Caching が効いている呼び出しほど請求より高く出ます。キャッシュヒットは本来の入力より安いのに、満額の入力として数えたうえで、キャッシュ分をもう一度足してしまうからです。私の手元では、長い資料を毎回添えるバッチだけが不自然に高く見えていて、単価表のほうを疑って半日を溶かしました。原因は単価ではなく、引き算をひとつ忘れていたことでした。

内包の数値は、Langfuse に渡す前に排他へ引き直します。この処理を関数ひとつに閉じ込めて、呼び出し側から `usage_metadata` を直接触らせないようにしております。
...
def to_langfuse_usage(um: Any) -> dict[str, int]:
    """Gemini の内包型 usage_metadata を Langfuse の排他バケットへ変換する。

    prompt_token_count はキャッシュ分を含むので、必ず差し引いてから渡す。
    """
    def n(name: str) -> int:
    ...
    usage = {
        "input": max(prompt - cached, 0),          # キャッシュ分を除いた実入力
        "input_cached_tokens": cached,             # キャッシュ読み出し分
        "output": n("candidates_token_count"),
        "output_reasoning_tokens": n("thoughts_token_count"),
    }

    # 合計との差を見る。ここが動いた日が、内訳の仕様が変わった日になる
    drift = total - sum(usage.values()) if total else 0
    if drift > 0:
        usage["untracked"] = drift             # 数え漏れ（新しい内訳が増えた合図）
    elif drift < 0:
        logger.warning("usage buckets overlap: drift=%d", drift)  # 二重計上の疑い
    return usage
```

最後の数行が、この関数のいちばんの働きです。提供側は内訳のフィールドを増やすことがあり、増えた瞬間に数字は黙ってずれます。合計との差を `untracked` という名前で残しておけば、ダッシュボードにそのバケットが立ち上がった日が、仕様の変わった日になります。差が負に振れたら逆に二重計上の疑いですので、こちらは金額に混ぜず警告ログへ流しています。

呼び出し側は、この辞書をそのまま渡すだけです。
...
`usage_details` のキーは、モデル定義に置いた単価のキーと**完全に一致**している必要があります。`input_cached_tokens` に単価を置いていなければ、そのバケットは 0 円として集計されます——ゼロ円という数字は、無料という意味にも、設定漏れという意味にもなりますので、私はバケットを増やした日にモデル定義の側も必ず開くようにしています。

思考トークンを返すモデルでは、トークン数を渡さないかぎり Langfuse 側でコストを推定できません。ドキュメントも、推論系のモデルでは使用量の取り込みが前提だと明記しています。自動計装に任せきりにせず、`usage_details` を明示的に渡す設計にしておくほうが、あとの月次で困らずに済むのだと思います。
...
**重要な落とし穴:** 上記の正規表現はあくまでサンプルで、実運用では Google Cloud DLP や Presidio などのライブラリを使うべきです。特に医療情報・金融情報を扱うプロダクトでは、自社の規程に沿って PII 辞書を定義することが前提になります。

さらに、セルフホスト版の Langfuse であれば、`LANGFUSE_FEATURES` 環境変数で「入出力本文を保存しないモード（メタデータのみ収集）」を有効化できます。機微度が特に高い業務領域では、このモードが有力な選択肢になります。
...
**落とし穴 3: 単価未登録のまま流れたトレースは、あとから直らない**
出たばかりの Gemini モデルが既定の単価表に載る前に本番へ乗ると、その期間のトレースはコストが `—` のまま残ります。厄介なのは、あとから単価を登録しても**過去のトレースは再計算されない**ことです。月次レポートは黙って過小のまま出ます。新しいモデル ID を本番へ入れる日は、単価定義を先に用意しておくか、`cost_details` を明示的に取り込む側へ倒しておくのが安全です。
...
開発中のデバッグで同じプロンプトを 20 回叩くと、本番トレースに混ざってダッシュボードが歪みます。開発環境では `LANGFUSE_PROJECT` を別プロジェクトに向ける、あるいは `tags=["dev"]` を必ず付けて、ダッシュボードから外しておきます。
...
export async function summarize(
  ...
  generation.end({
    output: out,
    usage: {
      // promptTokenCount はキャッシュ分を含むので、必ず差し引いてから渡す
...
上の書き方は JS SDK の v3 系のものです。v4 では `@langfuse/tracing` の `startObservation` に整理され、使用量のキーも `usage` から `usageDetails` へ変わりました。名前が変わったことに気づかないまま SDK を上げると、トレースは残るのにコストの列だけが空になります。私は SDK を上げた回は、翌日にコスト列を一度だけ見に行くようにしております。
...
Cloudflare Workers などのエッジランタイムでは、バンドラが `langfuse` をフル同梱しているか（壊れた形でツリーシェイクされていないか）と、`LANGFUSE_HOST` がそのエッジリージョンから到達可能かを、投入前に一度確かめておきます。過去に見た本番障害のいくつかは、リージョン固有のファイアウォールが Langfuse の送信エンドポイントを遮断していた事例でした。

## 代替ツールとの比較

「Langfuse を本当に選ぶべきか」は必ず検討が入る論点です。2026 年春時点での、私自身の実務判断を共有します。情報はすぐ古くなりますので、導入の際は公式サイトで最新の状況をご確認いただければと思います。
...
- **環境分離**: プロダクション用と、ステージング＋開発用で Langfuse プロジェクトまたはインスタンスを分けます。混ぜるとダッシュボードがノイズだらけになり、コンプライアンスレビューで説明が難しくなります。
...
**第 1 週（観測の骨格）** — `@observe` デコレータを主要エンドポイントに 3〜5 箇所だけ入れます。`user_id` と `feature` のメタデータも同時に付けます。ここまでで、問い合わせに答えるまでの時間がはっきり短くなります。

**第 2 週（コスト可視化）** — 単価定義と価格ティアを整えたうえで、機能別のコスト分解ダッシュボードを作ります。ここで「想定外に費用がかかっている機能」が見つかります。

**第 3 週（評価）** — `answer_relevance` と `tone_compliance` の二つだけ LLM-as-Judge を入れます。サンプリングは 5% で足ります。プロンプトを変えたときの劣化検知が、ここで効き始めます。

**第 4 週（PII・サンプリング）** — 本番トラフィックが見えてきたところで、マスキングフックとサンプリングルールを入れます。早すぎるサンプリングは初期の観測機会を奪うので、ここが適切なタイミングです。

この進め方なら、既存プロダクトに対するリスクを最小限に抑えつつ、Langfuse の価値を段階的に引き出せます。関連トピックとして、コスト削減の源泉となる[Gemini API キャッシュ戦略の運用ノート](/articles/gemini-api/gemini-api-caching-strategy-masterclass-production)や、レート制限時の挙動を観測する前提となる[Gemini API レート制限と 429 対策の運用ノート](/articles/gemini-api/gemini-api-rate-limiting-quota-management-production-guide)と組み合わせて運用すると、本番の安定性が一段上がります。
