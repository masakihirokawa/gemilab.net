...
---
8日目の朝、ターミナルに残っていたのは `invalid_grant` の一行だけでした。前の晩まで滞りなく届いていた予定サマリーが、その日から来なくなっていたのです。コードは1文字も変えていません。変わっていたのは、Google Cloud 側の同意画面を「テスト」のまま置いていたという、私がまったく見ていなかった一点でした。

GmailとCalendarをまたぐ秘書AIで手が止まるのは、Gemini の Function Calling そのものではありませんでした。認証の寿命、スコープの粒度、書き込み系のツールをどこで人に返すか——難所はほとんどが周辺にあったのです。

ここには、毎朝1回の実行を90日ほど続けてきた実装と、その途中で私が踏んだ失敗を書き残します。動くコードだけでなく、「なぜそう書かないと本番で困るのか」まで辿れるように並べたつもりです。
...
OAuth2トークンは1時間で失効します。長時間稼働するプロセスやcronジョブで動かす場合、トークン更新の失敗は誰にも気づかれないままエラーになります。`workspace_auth.py` の `get_credentials()` はトークン更新ロジックを含んでいますが、リフレッシュトークン自体が失効することもあります（長期間未使用・パスワード変更・セキュリティイベント発生時など）。

この場合は適切な例外を上流に伝播させ、Slackや通知サービスでアラートを受け取るようにしてください。「なぜか3日間サマリーが届いていなかった」という事態を防げます。

### 同意画面を「テスト」のままにすると、リフレッシュトークンは7日で切れる

冒頭の `invalid_grant` は、ここが原因でした。

Google Cloud の OAuth 同意画面には公開ステータスがあり、「テスト」のまま外部ユーザー種別で認可すると、テストユーザーへの認可そのものが同意から7日で失効します。`access_type="offline"` で受け取ったリフレッシュトークンも、同じ期限で無効になります。手元で試している最中は毎日触るので気づきません。動きが安定して放っておける状態になった頃、ちょうど8日目が来ます。

**認証は一度通せば終わる作業ではなく、期限のある約束です。** 私はこれを、朝のサマリーが3日続けて届かなかったあとに理解しました。

<table>
  <thead>
    <tr><th>公開ステータス</th><th>リフレッシュトークンの寿命</th><th>向く場面</th></tr>
  </thead>
  <tbody>
    <tr><td>テスト（Testing）</td><td>同意から<strong>7日</strong>で失効。再認可のたびに更新</td><td>スコープを詰めている開発中だけ</td></tr>
    <tr><td>本番環境（In production）</td><td>失効しない（取り消し・長期未使用・パスワード変更を除く）</td><td>毎朝走らせる定期実行</td></tr>
  </tbody>
</table>

秘書AIが要求する `gmail.readonly` や `gmail.compose` は制限付きスコープに当たるため、外部ユーザー種別のまま本番環境へ切り替えると、未確認アプリの警告画面が挟まります。自分ひとりで使う分にはその画面から先へ進めますが、他の方へ配布する段階になると確認プロセスが必要です。Google Workspace のアカウントをお持ちであれば、ユーザー種別を内部（Internal）にしてしまうのがいちばん静かな解決でした。詳細は[Google の OAuth 2.0 ドキュメント](https://developers.google.com/identity/protocols/oauth2)と[アプリの公開状態の解説](https://developers.google.com/identity/protocols/oauth2/production-readiness/overview)に一次情報があります。

そのうえで、失効を「起きてから気づく」のではなく「起きた瞬間に知る」形にしておきます。cron の先頭で一度だけ更新を試し、駄目なら本処理へ進まずに通知だけ出す、という小さな関門です。

```python
# preflight.py — 本処理の前に認証だけを確かめる
...
class ReauthRequired(Exception):
    """再認可が必要な状態。本処理へ進まずに通知へ回す。"""


def preflight(notify) -> "Credentials":
    """
    秘書AIの本処理より先に、認証だけを1回確かめる。
    ここで落としておけば「Gemini に課金してから認証で落ちる」順序を避けられる。
    notify は Slack や自分宛メールなど、朝に必ず目へ入る経路を渡す。
    """
    creds = get_credentials()
...
`cron` からは `python preflight.py && python morning_brief.py` のように繋いでいます。認証で落ちた朝は Gemini を1回も呼ばずに終わるため、余計な課金も残りません。
...
秘書AIのように「毎朝同じ形式のサマリーが届くこと」を前提にした処理では、この予告のない入れ替わりが効きます。個人開発で複数の処理を夜間に委ねている都合上、実体が変わったあとはまず出力を並べて見比べました。要約の情報量は落ちていないものの、優先度の並び順の判断がやや変わっていました。壊れてはいませんでした。けれど、同じでもありませんでした。
...
cron で無人実行する場合、`input()` は使えません。私は承認待ちの内容を JSONL に積んで、朝の通知にまとめて出す形にしています。「AI が5通の下書きを作った」ではなく「AI が5通の下書きを作りたがっている」まで戻すようにしています。この一段があるだけで、安心して寝られるようになりました。
...
3. **ループの上限が見えること**。`max_iterations = 5` のような制御が自分のコードにあることは、コスト面の安心につながります

逆に、Managed Agents に寄せたほうが素直な場面もはっきりしています。手元の環境に依存しない使い捨ての処理、コード実行やファイル操作を伴う調査タスク、複数の環境から同じエージェントを叩きたい場合です。自前のサンドボックスを用意する手間がまるごと消えるのは、やはり大きいと感じています。
...
もうひとつ、`with_retry()` を入れる前は月に1回ほど 429 で無言のまま止まっていました。cron は失敗しても静かです。サマリーが届かない朝があっても、忙しければ気づけません。バックオフを入れたのは、コストのためではなく「気づけない失敗」をなくすためでした。
