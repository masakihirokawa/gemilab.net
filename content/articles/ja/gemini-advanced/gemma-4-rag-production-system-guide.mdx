...
highlights:
  - "ChromaDB と pgvector 両方の実装コード——ユースケース別の選択基準と、Gemma 4 の埋め込みモデルとの最適な組み合わせ"
  - "256Kコンテキストを活かした Long-Context RAG の設計パターン——通常のチャンキングが不要になるケースと必要なケース"
  - "プロダクション環境でのパフォーマンス最適化：バッチ埋め込み生成、ハイブリッド検索、クエリ拡張の実装例"
---

「ローカルLLMでRAGを動かしたいが、Ollama で動かした Gemma 4 と、別途用意したベクトルデータベースをどう繋ぐか分からない」——この悩みは、オープンソースLLMを本番環境に持ち込む際に必ずぶつかる壁だ。

この記事では、Gemma 4 と ChromaDB または pgvector を組み合わせた RAG システムの実装を、アーキテクチャ設計から本番最適化まで段階的に解説する。Apache 2.0 ライセンスのコンポーネントで固めているため、商用サービスへの組み込みに制約がない。

## RAG と Gemma 4 の組み合わせが強い理由

従来の RAG 実装では「コンテキスト窓が短い→チャンクを細かく切る→文脈が失われる」というトレードオフが常につきまとっていた。Gemma 4 の 31B Dense と 26B MoE は 256K トークンのコンテキスト窓を持ち、E2B/E4B でも 128K ある。これは、RAG の設計思想そのものを変え得るスペックだ。

具体的には、検索でヒットしたチャンクを複数まとめて1回のプロンプトに含める「Long-Context RAG」が現実的な選択肢になる。100件のチャンク（各1,000トークン）を一度に渡しても、256K 窓には余裕がある。モデルが文書間の関係を直接参照しながら回答を生成できるため、チャンクの境界で文脈が切れる問題が大幅に軽減される。

もう一つの強みが Apache 2.0 ライセンスだ。Gemma 3 では商用利用に制約のあるカスタムライセンスが使われていたが、Gemma 4 は完全な Apache 2.0 に移行した。これは SaaS への組み込み、OEM 提供、派生モデルの商業販売がすべて自由になることを意味する。
...
Gemma 4 自体は汎用生成モデルであり、直接埋め込みベクトルを出力する設計ではない。RAG では専用の埋め込みモデルと組み合わせて使う。
...
プロダクションでは `multilingual-e5-large` と Ollama の `nomic-embed-text` を比較検証することを勧める。前者は品質が高く、後者はローカル実行で外部依存がない。

## Step 2: ChromaDB との統合

ChromaDB は開発環境や中規模用途（数百万件程度のドキュメント）に最適なベクトルストアだ。Python ネイティブな API と永続化機能を持つ。
...
既存の PostgreSQL インフラに AI 機能を追加したい場合は pgvector が最適だ。既存の RDB との JOIN も可能なため、ユーザーデータと知識ベースを組み合わせた高度なクエリが書ける。
...
検索結果を Gemma 4 に渡して回答を生成する。256K コンテキストを活かした Long-Context RAG では、複数の検索結果を1回のプロンプトにまとめて送れる。
...
セマンティック検索だけでは漏れが生じる。キーワード検索と組み合わせた「ハイブリッド検索」と、クエリを言い換えて検索範囲を広げる「クエリ拡張」で精度を上げる。
...
大量のドキュメントを処理する場合、埋め込み生成をバッチ化することで処理時間が大幅に短縮される。
...
Gemma 4 + ChromaDB/pgvector の RAG 構成が最も効果を発揮するのは次のような場面だ。

社内ドキュメント検索システム——外部 API に送れない機密文書（法的書類、財務データ、医療記録）を社内サーバーで完全オフライン処理できる。Apache 2.0 ライセンスなので商用利用に追加費用がかからない。

多言語ナレッジベース——Gemma 4 の140言語サポートと多言語埋め込みモデルを組み合わせることで、日英混在の技術ドキュメントや多言語カスタマーサポートに対応できる。

コードベース検索——256K コンテキストを活用して、大規模なコードベース全体を一度に参照しながら実装方針を問い合わせられる。チャンキングが不要なため、関数をまたいだ依存関係も正しく理解される。

まず `ollama pull gemma4:27b` でモデルを取得し、ChromaDB のローカル環境で検証してから pgvector 移行を検討する順序が現実的だ。
