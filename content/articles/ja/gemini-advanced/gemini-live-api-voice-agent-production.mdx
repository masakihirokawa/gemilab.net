...
A2. 主な違いはエンタープライズ機能とSLAです。Vertex AI経由では、VPC Service Controls・Customer-managed Encryption Keys（CMEK）・エンタープライズSLAが利用可能です。Google AI StudioのAPIキーは開発・プロトタイプ用途に向いており、本番サービスにはVertex AIを推奨します。詳細は [Vertex AI Gemini本番運用ガイド](/articles/gemini-advanced/vertex-ai-gemini-production) を参照してください。
