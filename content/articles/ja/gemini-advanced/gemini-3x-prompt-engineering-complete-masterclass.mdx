---
title: "Gemini 3.x 時代のプロンプトエンジニアリング完全マスタークラス — System Instructions・Few-shot・CoT・ReAct・自己評価ループを動くコードで比較実装"
...
highlights:
  - "Gemini 3.1 Pro のThinking Budget と CoT を組み合わせて推論品質を最大化する手法を、コピペで動くコードとともに習得できる"
  - "System Instructions・Few-shot・ReAct・Critic-Refiner の4つのパターンをBefore/After形式で比較し、自分のユースケースに最適な技法を選べるようになる"
  - "プロンプトバージョン管理・A/Bテスト・自動評価パイプラインまで、本番運用に必要なMLOps的アプローチを一通り実装できる"
---
...
この記事では、Gemini 3.1 Pro と 3.2 を対象に、プロンプトエンジニアリングの主要技法を動くPythonコードで比較実装します。公式ドキュメントには書かれていない実践的な発見も含め、体系的に整理しました。
