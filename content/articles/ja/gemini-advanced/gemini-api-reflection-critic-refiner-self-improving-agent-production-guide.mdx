...
author: "Gemini Lab"
description: "Gemini 3 Pro と 3.5 Flash-Lite を組み合わせて自己批判するエージェントを構築する設計と本番運用ノウハウ。Reflection / Critic-Refiner パターンの実装、コスト上限、過剰修正の防ぎ方まで踏み込んで解説します。"
...
highlights:
  ...
  - "Gemini 3 Pro と 3.5 Flash-Lite を組み合わせて、品質を上げつつコストを 1.5〜2 倍以内に抑える設計と監視メトリクスを習得できる"
...
ここでは Gemini 3 Pro と 3.5 Flash-Lite を組み合わせて自己批判するエージェントを実装する手順を、私が実際にプロダクトに組み込んだ経験を踏まえて段階的に解説します。動くコードは3つ用意しました。コピーして温度感を確かめ、自分のユースケースに合わせて調整していただければと思います。
    ...
    try:
        resp = client.models.generate_content(
            ...
            config=types.GenerateContentConfig(
                # 決定性は response_schema と指示の明示で担保する（サンプリング設定は非推奨）
...
- Critic（批評）: 検出精度と速度のバランスから、Gemini 3.5 Flash-Lite や 3.6 Flash が現実的
...
### コード — Gemini 3 Pro × 3.5 Flash-Lite の Critic-Refiner

何をするコードか: Generator を gemini-3-pro、Critic を gemini-3.5-flash-lite に分担し、severity 3 以上の指摘のみ Refiner に渡すことで過剰修正とコスト膨張を抑えます。
...
def critic(draft: str, constraints: List[str]) -> CriticReview:
    ...
    try:
        resp = client.models.generate_content(
            ...
            contents=prompt,
            config=types.GenerateContentConfig(
                # 3.x Flash 系はサンプリングパラメータを受理したうえで無視するため指定しない
                response_mime_type="application/json",
...
ダッシュボードに「停止理由の分布」「制約違反率（前後比較）」「LLM-as-Judge 勝率」「p95 レイテンシ」「コスト/件」の5つを並べておけば、ほとんどの劣化を1日以内に検知できます。LLM の挙動はモデルアップデートで突然変わることがあるため、Reflection を入れたらこれら指標の継続的な観測がほぼ必須になります。Thinking Budget の制御で計算量と品質のバランスを取りたい場合は [Gemini 2.5 Pro Thinking Budget で推論深度を制御する](/articles/gemini-advanced/gemini-25-pro-thinking-budget-reasoning-control) も合わせて参照してください。
...
私が実際に組み込んでいる例として、個人開発で運用している SaaS の通知メール下書き機能があります。要件は次のようなものでした。
...
## レイテンシの内訳 — 増えた待ち時間はどこに消えているのか

Reflection を本番に載せた直後、必ず出てくる疑問が「結局どこで時間を食っているのか」です。前述のメール下書きパイプラインに区間計測を仕込んでみたところ、典型的な1リクエストの内訳はおおよそ次のようになりました。

<table>
  <thead>
    <tr><th>区間</th><th>所要時間の比率</th><th>使用モデル</th></tr>
  </thead>
  <tbody>
    <tr><td>Generator（初稿生成）</td><td>約 60%</td><td>Gemini 3 Pro</td></tr>
    <tr><td>Critic（指摘の抽出）</td><td>約 15%</td><td>Gemini 2.5 Flash（計測当時の Critic）</td></tr>
    <tr><td>Refiner（指摘箇所の修正）</td><td>約 20%</td><td>Gemini 3 Pro</td></tr>
    <tr><td>クライアント側の繋ぎ（JSON パース・リトライ・ネットワーク揺らぎ）</td><td>約 5%</td><td>—</td></tr>
  </tbody>
</table>

意外だったのは Critic が全体の 15% しか占めていなかったことです。呼び出し回数だけ数えれば3分の1を想像しますが、Critic のプロンプトは短く、レスポンススキーマも小さく、Flash 系モデルは同じプロンプトサイズなら Pro 系よりはっきり速く完了します。この3つが重なった結果でした。

この内訳が見えると、最適化の順番が変わります。p95 を削るなら効果が大きいのは次の3つです。

- Generator のプロンプトを絞る（入力が減れば Generator と Refiner の両方が同時に速くなる）
- Generator の出力をストリーミングし、初稿が揃った瞬間に Critic を投げる（Critic は全文を必要とするので並列化はできませんが、二重のバッファリング待ちは消せます）
- `timeout_seconds` を攻めた値に設定し、テール側の Generator 1件がループ全体の時間予算を食い潰さないようにする

逆に、Critic を Pro 系モデルへ引き上げてもレイテンシはほとんど動きません。もともと軽い工程を最適化しても全体には効かない、という当たり前の話に落ち着きます。私はこの順番を一度間違えていて、体感の遅さを Critic のせいだと決めつけて Pro に差し替え、コストだけ増やした期間がありました。計測してから触る、という原則はここでも効きます。

## 障害モード別のランブック — ループが暴れたときに見る順番

Reflection が本番で初めておかしな挙動を見せたとき、ゼロから調査を始めると時間だけが溶けていきます。私が個人開発の運用メモに残しているランブックを、症状から辿れる形で置いておきます。

<table>
  <thead>
    <tr><th>観測される症状</th><th>疑うべき原因</th><th>最初に打つ手</th></tr>
  </thead>
  <tbody>
    <tr>
      <td>停止理由 <code>budget</code> の比率が上がってきた</td>
      <td>Critic の出力量が多すぎる（1回あたり issues が平均8件超）</td>
      <td>Critic プロンプトを「問題は最大5件、severity 4〜5 を優先」に締める</td>
    </tr>
    <tr>
      <td>停止理由 <code>no_issues</code> が 80% を超えた</td>
      <td>Generator が改善したか、Critic が見落としている</td>
      <td><code>no_issues</code> 判定の50件をより厳しい Critic プロンプトに通す。実問題が 10% 以上出れば本番 Critic がドリフトしている</td>
    </tr>
    <tr>
      <td>LLM-as-Judge の勝率が下がる（Judge が初稿を選ぶ）</td>
      <td>Refiner の過剰修正</td>
      <td>初稿と最終稿の差分を抽出し、指摘外の箇所に手が入っていれば「変更禁止」の列挙を Refiner プロンプトに追加</td>
    </tr>
    <tr>
      <td>p95 レイテンシが突然2倍になる</td>
      <td>Generator 側のモデル更新で新しい失敗モードが増えた</td>
      <td>停止理由の分布を確認し、<code>max_iterations</code> / <code>timeout</code> の急増と指摘カテゴリの構成比を前週と比較</td>
    </tr>
    <tr>
      <td>レイテンシは変わらないのにコストだけ上がる</td>
      <td>Critic プロンプトの入力トークンが静かに肥大している</td>
      <td>Critic 呼び出しの平均プロンプトトークンを月次で追う。Few-shot の追記は数ヶ月で入力を倍にします</td>
    </tr>
  </tbody>
</table>

このうち私が実際に一番長くハマったのは最後の行です。Few-shot を「よくある失敗例」として少しずつ足していった結果、半年で Critic の入力トークンが1.8倍になっていました。レイテンシにはほとんど現れないため、月次のコストレポートを見るまで気づけませんでした。プロンプトも履歴を追える対象として扱う、という当たり前の運用が抜けていた自覚があります。

## ストリーミングと部分出力をどう見せるか

レイテンシに敏感な画面では、Critic と Refiner を裏で走らせながら Generator の初稿をユーザーへ流したくなります。私が採っているのは「初稿を出し、あとから差分を当てる」やり方です。ユーザーは Generator のレイテンシで初稿を読み始められ、数秒後に UI が変更箇所だけ静かに書き換わります。差分のキーは `Issue.snippet` から導けるスパン識別子に寄せておくと実装が素直になります。

ただしトレードオフがあります。人によっては出力が二度続けて変わったように見え、落ち着かない印象を与えます。修正差分が初稿の 10% を超えるのが平均的なワークロードなら、ストリーミングの初稿は「最終稿を生成しています」の表示の裏に隠すほうが体験としては安定します。10% を下回るなら、その場での書き換えはほとんど気づかれません。

## Gemini 3 世代で変わった前提

Gemini 3 Pro の登場で Reflection の採算計算が少し変わりました。私が計測しているワークロードでは、2.5 Pro に比べてワンショットの制約遵守率が数ポイント高い状態から始まります。それなら Reflection の価値は下がるはずですが、実務では逆でした。3 Pro は Flash 系よりトークン単価が高いため、「全部を Pro で回す」構成と「Generator / Refiner だけ Pro、Critic は Flash」構成の差額が、2.5 世代よりむしろ広がったのです。役割分割の効果は世代が進むほど大きくなっています。

もう一点、構造化出力の安定性も変わりました。3 系はレスポンススキーマの保持が 2.5 系より安定しており、Critic 側の Pydantic 検証がほぼ一度で通ります。2.5 のライフサイクル前半では Critic 呼び出しにリトライを入れていましたが、3 Pro と 2.5 Flash の組み合わせでは、そのリトライ経路が発火するのは 0.5% 未満に収まっています。Critic のリトライ実装が残っているなら、そのぶんの複雑さは削れるかもしれません。私自身、2.5 世代に書いた防御的なコードを 3 世代に持ち込んだまま放置していて、読み返して初めて不要になっていたと気づきました。世代が変わったらガードの棚卸しをする、という工程を運用の定期作業に入れています。

この前提に、2026 年に入って二つの期限が加わりました。ひとつは Gemini 2.5 系モデルの提供終了で、2026 年 10 月 16 日が予定日です。本記事の Critic を最初に担っていた 2.5 Flash も対象です。私は Critic を gemini-3.5-flash-lite へ移しました。大量呼び出し前提の低遅延枠という位置づけが、短いプロンプトを毎リクエスト捌く Critic の仕事とよく重なるためです。指摘の取りこぼしが許容しにくいワークロードなら、トークン効率を改善した gemini-3.6-flash へ一段上げる選択肢もあります。どちらを選ぶにせよ、移行の前後で severity の分布を採って比べておくと、検出力が落ちていないかを一日で確かめられます。

もうひとつが、サンプリングパラメータ temperature・top_p・top_k の非推奨化です。2026 年 7 月 21 日の Gemini 3.6 Flash / 3.5 Flash-Lite 公開にあわせて方針が示され、3.x Flash 系ではこれらを受理したうえで無視します。リクエストは 200 で通り、警告も出ません。temperature=0.0 で Critic の判定を安定させるという本記事の初版の書き方は、ここで意味を失いました。本文のコードからサンプリング指定を外したのはそのためです。

では決定性をどこで担保するのか。答えは、この記事がずっと述べてきた仕組みの側にあります。response_schema による構造の強制、severity の判断基準をプロンプトに明記する指示設計、そして出力を別呼び出しで検証する Critic ループそのもの。温度という一つのつまみに預けていた制御を構造へ移すことが、公式に推奨される流れになりました。既存のコードベースに対しては、三つのパラメータ名を grep で洗い出すことと、同じプロンプトを温度違いで複数回投げて分散が変わらないことを実測すること。この二つの確認に掛かるのは十数分ですが、効かなくなった設定を抱えたまま運用を続ける事態を避けられます。
