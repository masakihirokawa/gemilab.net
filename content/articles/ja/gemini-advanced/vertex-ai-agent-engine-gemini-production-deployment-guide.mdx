...
---
エージェント開発のローカル実装が動き出したあと、本番環境に持っていく段階で多くのチームが失速します。Cloud Run に Flask で包んでデプロイし、Redis でセッションを持ち、Cloud Logging で何とかトレースを追う — 動くことは動きますが、エージェントが3〜4本に増えた瞬間に運用負荷が破綻します。私もこのパターンで2回ほど痛い目を見ました。1度目は業務の検証用、2度目は個人開発で作っていた小さな社内向けツールで、後者は運用の手間に負けて結局そのまま止めてしまいました。
...
## 個人開発の規模では、どこが損益分岐点になるのか

前節のガイドラインは、複数人で運用することを前提に書きました。ただ私自身は個人開発でアプリを作り続けている立場でもあるので、「一人で回すなら Agent Engine は割に合うのか」という問いのほうが、実際には切実でした。チーム前提の記事だけを読んで判断すると、個人開発では過剰にも過小にも振れます。

結論から書くと、分岐点はコンピュートの単価ではなく **状態ストアの固定費** にあります。

### 自前構成とマネージド構成を、同じ負荷で並べてみる

想定は「1日200リクエスト、1リクエストあたり平均12秒」という、個人開発の実務ツールとしては現実的な負荷です。月に換算すると実稼働はおよそ20時間。この規模で両構成を並べると、差が出る場所がはっきりします。

<table>
<thead>
<tr><th>費目</th><th>自前構成（Cloud Run + Memorystore）</th><th>Agent Engine</th></tr>
</thead>
<tbody>
<tr><td>実行コンピュート（月20時間相当）</td><td>数ドル程度</td><td>数ドル程度</td></tr>
<tr><td>会話状態のストア</td><td><strong>Memorystore 1GB で月30ドル台の固定費</strong>（リクエストが0でも発生）</td><td>Sessions API に含まれる（従量）</td></tr>
<tr><td>トレース基盤</td><td>相関IDの自作 + Cloud Logging のログ量課金</td><td>Cloud Trace へ自動送出</td></tr>
<tr><td>Gemini トークン</td><td colspan="2">両者同額（モデル選定と thinking budget で決まる）</td></tr>
</tbody>
</table>

金額の単価は改定されるので、実際の判断前に必ず Google Cloud の料金ページで最新値を確認してください。ここで見てほしいのは絶対額ではなく、**トラフィックに比例しない費目がどちらに寄っているか** です。自前構成では、誰も使っていない月でも Redis の月額がそのまま請求されます。個人開発のアプリは AdMob の広告収益や App Store の売上が原資なので、30ドル台の固定費は「毎月これだけ稼がないと赤字」というラインをそのまま押し上げます。

### 見落としがちなのは、人間の時間のほうです

もうひとつ、見積書に載らない費目があります。自前構成では、セッションの TTL 設計・トレースの相関ID・ツールのリトライを全部自分で書くことになります。私の場合、最初の実装で夜の作業時間が数日分そこに溶けました。動き出したあとも、SDK のバージョン差分やライブラリの非互換で月に1〜2時間は持っていかれます。

個人開発では、この1〜2時間が「新機能を1つ足せたかどうか」に直結します。月30ドルの固定費より、こちらのほうが実質的には重い出費でした。

### 私はこう線を引いています

- **状態を持つ会話型で、1日あたり数百リクエストまで** — Agent Engine。固定費の状態ストアを抱えずに済む点が、この規模では効きます。
- **状態を持たない1ショットの分類・要約** — Cloud Functions。エージェントの器を被せる意味がありません。
- **秒間十数リクエストを超える規模** — 実測してから決めます。この帯域では自前構成のほうが単価で有利になる場合があり、固定費が総額に埋もれて相対的に軽くなるためです。

判断を保留したまま両方を並行運用するのが一番高くつきます。負荷の想定を1つ決めて、2週間だけ実測してから畳むほうが、結果的に安く済みました。
