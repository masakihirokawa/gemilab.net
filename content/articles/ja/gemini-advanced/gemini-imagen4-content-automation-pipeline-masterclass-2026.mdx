---
title: "Gemini 2.5 Pro + Imagen 4 コンテンツ自動生成パイプライン完全構築ガイド"
...
highlights:
  - "ブログ記事・SNS投稿・サムネイル画像を1回のパイプライン実行で自動生成できる完全動作コードを、そのまま使えるかたちで習得できる"
  - "Imagen 4 APIのコスト構造と品質管理の実装パターンを理解し、月5〜15万円規模のコンテンツ自動化サービスを設計できるようになる"
  - "非同期処理・エラーハンドリング・APIキー管理まで網羅した本番運用設計で、個人開発でも安定稼働するシステムを構築できる"
---
...
def quality_check(article: dict) -> tuple[bool, list]:
    ...
    if any(ng in title for ng in ["この記事では", "完全ガイド", "徹底解説"]):
