...
<table>
  <thead>
    <tr><th>Budget 設定</th><th>平均レイテンシ</th><th>平均出力品質スコア（独自評価）</th><th>1,000 リクエスト推定コスト</th></tr>
  </thead>
  <tbody>
    <tr><td>0（Thinking OFF）</td><td>1.2 秒</td><td>6.4 / 10</td><td>約 $4</td></tr>
    <tr><td>4,096</td><td>3.8 秒</td><td>7.2 / 10</td><td>約 $6</td></tr>
    <tr><td>8,192</td><td>6.5 秒</td><td>8.1 / 10</td><td>約 $9</td></tr>
    <tr><td>16,384</td><td>11.4 秒</td><td>8.8 / 10</td><td>約 $14</td></tr>
    <tr><td>24,576</td><td>18.9 秒</td><td>8.9 / 10</td><td>約 $19</td></tr>
  </tbody>
</table>
...
## 公式の推奨値ではなく、自分のタスクで測る

ここまでは私の環境での実測値を共有してきました。ですが Budget の最適点はタスクに強く依存します。翻訳と数学証明では、効く Budget がまったく違います。最後は「自分のタスクで測る」のが確実です。

測定といっても難しくありません。3 つの指標を同時に記録するだけです。

- **精度**: タスク固有の採点（正答率・スコア）
- **消費思考トークン**: `usage_metadata.thoughts_tokens`
- **レイテンシ**: リクエストから最終トークンまでの時間

この 3 つを Budget を変えながら複数回測ると、「これ以上 Budget を増やしても精度は伸びないのに、コストとレイテンシだけが線形に悪化する」点が見えてきます。私はこれをサチり点と呼んでいます。
...
`evaluator` はタスク固有の採点関数です。分類なら正解との一致判定、生成なら参照との類似度、ツール呼び出しなら関数名と引数のマッチ率、というふうにタスクに合わせて決めます。

実際に私が 3 種類のタスクで測った結果を共有します。各タスクとも同一プロンプトを 20 回ずつ走らせた平均です。

**中難度の数学問題（高校レベル）**

- `budget=0`: 正答率 42%
- `budget=1,024`: 正答率 71%（思考トークン平均 890）
- `budget=4,096`: 正答率 85%（思考トークン平均 2,100）
- `budget=8,192`: 正答率 87%（思考トークン平均 3,400）
- `budget=16,384`: 正答率 86%

数学系は 4,096 まではリニアに伸び、そこから先は頭打ちでした。4,096 と 16,384 で精度差は 1 ポイント、コストは約 1.8 倍。私はここでは 4,096 を採っています。

**商品レビュー 100 件を肯定・否定・中立に分類**

- `budget=0`: 正答率 91%
- `budget=256`: 正答率 92%
- `budget=4,096`: 正答率 91%

意外かもしれませんが、この種の分類では Budget を上げても精度は伸びません。`budget=0` で十分です。ここに思考コストをかけるのは純粋な無駄でした。

**コード生成（LeetCode Medium 相当）**

- `budget=0`: 通過率 38%
- `budget=2,048`: 通過率 62%
- `budget=8,192`: 通過率 81%
- `budget=16,384`: 通過率 84%
- `budget=24,576`: 通過率 85%

コード生成は複雑度に応じてリターンが伸び続けますが、8,192 以降の傾きは緩やかです。私は 8,192 を基本値にして、難問と分かっているときだけ 16,384 に上げています。

この探索は手作業でも回せますが、代表プロンプトを 10〜20 件用意してスクリプトに任せると、1 時間ほどで結論が出ます。

```python
import statistics

def find_budget_sweet_spot(prompts, evaluator, budgets=None, trials=10):
    """精度の伸びが鈍化した地点を推奨 Budget として返す。"""
...
精度の増加が 1% を切った手前の Budget を採用する、という単純なロジックです。公式の推奨値をそのまま使うのではなく、自分のタスクで測る。この 1 時間の投資が、月のコストを確実に最適化してくれます。

### Budget をめぐる 3 つの誤解

最後に、私がよく相談される誤解を 3 つ解いておきます。

- **「大きくしておけば安全」ではありません**。大きすぎると推論が発散し、分類・抽出ではかえって精度が落ちることがあります。
- **「budget=0 は劣ったモード」ではありません**。単純なタスクなら、より速く・安く・同じ結果が得られます。
- **「毎回同じだけ消費する」わけではありません**。実消費はタスクの難度で大きく変わります。`thoughts_tokens` を継続的に計測してください。

Thinking Budget は、モデルの賢さを「どこに費やすか」を決めるコントロールパネルのようなものです。測って回すほど、同じコストで引き出せる精度が上がっていきます。

## 公式ドキュメントには書かれていない5つの実装インサイト

公式の API リファレンスを読み込んでも、実際に運用してみないと見えてこない点があります。個人開発のアプリに Gemini API を本番環境で組み込んでみて気づいた点を 5 つ共有します。
...
私は個人開発でいくつかのアプリを長く運用しており、その本番環境に Gemini 2.5 Pro を組み込んでいます。広告配信のバックエンドや、ユーザー向けのレコメンド生成など、性質の違うリクエストが混在する環境です。Thinking Budget の配分は次のように設計しています。
...
この配分にしたきっかけは、初期に全リクエストを Budget=16,384 で運用してしまい、1 ヶ月の API 費用が想定の約 4.2 倍に膨れ上がった経験です。新しい機能が出ると、検証フェーズでつい予算管理を後回しにしてしまう。個人開発をしている方なら、共感していただけるかもしれません。
...
<table>
  <thead>
    <tr><th>タスク種別</th><th>推奨 Budget</th><th>理由</th></tr>
  </thead>
  <tbody>
    <tr><td>翻訳・要約・形式変換</td><td>0</td><td>推論不要。レイテンシ最小化</td></tr>
    <tr><td>FAQ への回答</td><td>0〜1,024</td><td>テンプレ化可能なため最小</td></tr>
    <tr><td>コードレビュー（小規模）</td><td>4,096</td><td>表面的な改善提案で十分</td></tr>
    <tr><td>コードレビュー（アーキ視点）</td><td>8,192</td><td>設計トレードオフの考察まで</td></tr>
    <tr><td>データ分析・統計解釈</td><td>8,192</td><td>落とし穴の指摘まで届く</td></tr>
    <tr><td>多段論理推論・最適化</td><td>16,384</td><td>このあたりが品質コスト最良点</td></tr>
    <tr><td>数学証明・極めて複雑な意思決定</td><td>24,576</td><td>念のため上限。常用しない</td></tr>
    <tr><td>バッチ系（夜間処理）</td><td>16,384</td><td>品質優先・レイテンシ無視</td></tr>
    <tr><td>アプリ内 UI から直接呼び出す処理</td><td>0〜2,048</td><td>UX 優先</td></tr>
  </tbody>
</table>
