...
2014 年から個人で iOS アプリを運用してきました。累計 5,000 万ダウンロードのうち、壁紙系・癒し系・引き寄せ系のアプリは「電波が悪い場所」で開かれる比率がとても高いカテゴリです。Gemini API を本格的に組み込み始めた 2025 年以降、私は「API を呼ばずに前回の応答を返す」ことが本番品質に直結すると痛感しました。個人開発者として2014年からiOSアプリを運用してきた廣川政樹（@dolice）が、累計5,000万ダウンロードのアプリ群でAdMob収益を支えながら実装してきたキャッシュ層を、SwiftData × Gemini API の文脈で実装と運用の両面から整理します。
...
ここからは机上の設計ではなく、実アプリで遭遇した運用知見です。
