...
本記事では、実例を交えながら、Gemini × Figma Make による新世代のデザインワークフローを徹底解説します。
...
**Q1: Figma Make を使わずに、Gemini だけで完結できますか？**

A: 技術的には可能ですが、実用的ではありません。Geminiはテキストベースの出力なので、複数のUIバリエーションを視覚的に比較できません。Figma Make により、デザイナーが「視覚的に選べる」というUXが実現します。

**Q2: 生成されたデザインの著作権は？**

A: Gemini API / Figma Make 双方とも、生成物の著作権は利用者に帰属します。商用利用・改変・再配布が可能です。ただしGemini APIの利用規約を確認してください。

**Q3: 独自デザインシステムがある場合は？**

A: Figma Make のシナリオに、既存コンポーネントの参照を組み込むことで対応できます。「生成時に既存ボタンコンポーネントを自動挿入」といったカスタマイズが可能です。
