---
title: "Gemini API × Electron.js 完全実装マスターガイド — Mac/Windows デスクトップ AI アプリをひとりで作る"
...
highlights:
  - "Electron 特有の IPC セキュリティ設計を誤ると API キーがレンダラーに漏れる——この罠を避ける正しい設計パターンを今日手に入れられる"
  - "Gemini API のストリーミング応答・Function Calling・ローカルファイル処理を Electron のメインプロセスで安全に実装するコードをまとめて習得できる"
  - "electron-builder によるパッケージング・コード署名・自動更新まで、一人でアプリを配布・運用できる体制を整えられる"
---
...
Electron は Web 技術（HTML・CSS・JavaScript/TypeScript）でクロスプラットフォームのデスクトップアプリを作れるフレームワークです。Gemini API との相性は悪くありませんが、**セキュリティ設計を間違えると API キーがレンダラープロセスに露出するという致命的な罠**があります。この記事では、その罠を避けつつ、実用的な AI デスクトップアプリを一人で作り上げるための設計パターンを、動くコードと一緒に解説します。
