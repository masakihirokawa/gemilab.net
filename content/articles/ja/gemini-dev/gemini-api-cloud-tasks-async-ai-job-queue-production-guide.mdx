---
title: "Gemini API × Cloud Tasks で作る非同期AIジョブキュー — タイムアウトとリトライを設計から解く実践ガイド"
...
本記事では、Cloud Run + Gemini API の典型構成を、Cloud Tasks を軸にした非同期ジョブキューに設計移行する手順を、動作するコード付きで解説します。タイムアウトの考え方・リトライ戦略・レート制限との衝突回避・進捗通知・Dead Letter Queue まで、本番で通る粒度まで踏み込みます。
...
- **API レイヤ（Cloud Run ① — job-api）**: ユーザー要求を受けて Firestore に `jobs/{jobId}` を作成し、Cloud Tasks にタスクを enqueue する。HTTP レスポンスは `202 Accepted` と `jobId` のみで、ほぼ即時に返す。
...
- **クライアント通知レイヤ**: Firestore のドキュメントをクライアントが `onSnapshot` で購読し、進捗と完了を UI に反映する。あるいは完了時に FCM / Webhook でプッシュする。
...
- **入力長の想定外**: 1M トークンを超えるドキュメントが紛れ込んだ。API の 413 / 400 として弾かれる。
...
本記事で扱った分散処理・リトライ・冪等性の考え方をより体系的に学ばれたい場合は、[データ指向アプリケーションデザイン（Martin Kleppmann）](https://www.amazon.co.jp/dp/4873118700/?tag=pinocchio-22) の第 8〜10 章がキューと耐障害性の議論に時間をかけていて、私自身も何度も読み返しています。Google Cloud 側の設計パターンとしては [Site Reliability Engineering（Betsy Beyer 他）](https://www.amazon.co.jp/dp/4873117917/?tag=pinocchio-22) が SLI/SLO と負荷制御の現実解を並べて論じていて、本記事の「リトライを一層に絞る」姿勢の理論的背景になっています。
