...
highlights:
  - "Google AI SDK for Android の Gradle 統合と Kotlin コルーチン × Flow によるストリーミング実装を完全理解できる"
  - "Jetpack Compose でのマルチモーダル AI チャット UI の設計パターンと ViewModel アーキテクチャを習得できる"
  - "APIキー保護・secrets-gradle-plugin・ProGuard 設定など本番運用に必要なセキュリティ設計を体系的に学べる"
---

## はじめに：なぜ Android ネイティブ × Gemini API なのか
...
本記事では、Google が公式提供する **Google AI SDK for Android**（`generativeai`）を使い、以下を実装する完全な設計パターンをお伝えします。
...
### Q1: 無料の Gemini API キーで開発から本番まで対応できますか？

A: Google AI Studio から取得した無料キー（`gemini-2.0-flash` モデル）は、1分あたり15リクエスト、1日あたり1500リクエストの制限があります。プロトタイプや個人開発レベルでは十分ですが、商用アプリには Google Cloud の従量課金制プランへの移行を検討してください。本番では Vertex AI SDK への切り替えも選択肢です。

### Q2: ストリーミング中にユーザーが送信をキャンセルできるようにするには？

A: `viewModelScope.launch` で起動したコルーチンを `Job` に保持し、キャンセルボタンのクリック時に `job.cancel()` を呼ぶことで実現できます。`Flow.cancellable()` を付けることで、コレクター側からのキャンセルも伝播します。また、UI 状態に `isCancellable: Boolean` フラグを追加し、ストリーミング中のみキャンセルボタンを表示するとより UX が向上します。

### Q3: 会話履歴が長くなるとトークン制限に引っかかりますか？

A: Gemini 2.0 Flash は最大 1M トークンのコンテキストウィンドウをサポートしていますが、トークン数が増えると API コストも上昇します。実装上は、直近 N ターン分のメッセージのみを API に送信する「スライディングウィンドウ」戦略が一般的です。また、`countTokens()` API で事前にトークン数を確認し、上限を超える場合に古いメッセージを要約・圧縮するロジックを組み込むことをお勧めします。
...
AI × モバイル開発の実装について、より詳しく学びたい方には[「Android アプリ設計パターン入門」（著：上田哲也）](https://www.amazon.co.jp/dp/4297140748/?tag=pinocchio-22)がアーキテクチャ設計の基礎固めに役立ちます。Gemini API との組み合わせで理解が一層深まるでしょう。
