...
author: "Gemini Lab"
description: "Firebase Genkit と Gemini API を個人開発アプリのレビュー要約バックエンドで 1 ヶ月動かした運用ノート。Flow / Tool の設計、Cloud Functions と Cloud Run の使い分け、コストとレイテンシの実測値、公式にない 7 つの落とし穴をまとめました。"
...
<table>
  <thead>
    <tr><th>用途</th><th>推奨モデル</th><th>理由</th></tr>
  </thead>
  <tbody>
    <tr><td>要約・分類</td><td>Gemini 2.5 Flash</td><td>高速で低コスト</td></tr>
    <tr><td>コンテンツ生成</td><td>Gemini 2.5 Pro</td><td>品質と速度のバランス</td></tr>
    <tr><td>複雑な推論</td><td>Gemini 2.5 Pro</td><td>深い思考が必要</td></tr>
  </tbody>
</table>
...
<table>
  <thead>
    <tr><th>モデル</th><th>1 リクエストあたり平均トークン</th><th>平均レイテンシ</th><th>p95 レイテンシ</th><th>1 リクエストあたりコスト</th><th>月間総コスト（12,000件）</th></tr>
  </thead>
  <tbody>
    <tr><td>Gemini 2.5 Pro</td><td>入力 1,800 / 出力 250</td><td>4.2 秒</td><td>11.8 秒</td><td>$0.018</td><td>約 $216</td></tr>
    <tr><td>Gemini 2.5 Flash</td><td>入力 1,800 / 出力 250</td><td>1.1 秒</td><td>3.4 秒</td><td>$0.0014</td><td>約 $17</td></tr>
    <tr><td>Gemini 2.0 Flash</td><td>入力 1,800 / 出力 250</td><td>0.9 秒</td><td>2.7 秒</td><td>$0.0009</td><td>約 $11</td></tr>
  </tbody>
</table>
...
<table>
  <thead>
    <tr><th>状態</th><th>Pro</th><th>Flash</th></tr>
  </thead>
  <tbody>
    <tr><td>Warm（直前に呼ばれた直後）</td><td>4.2 秒</td><td>1.1 秒</td></tr>
    <tr><td>Cool（5 分以上経過）</td><td>28 秒</td><td>20 秒</td></tr>
    <tr><td>Cold（初回起動）</td><td>42 秒</td><td>24 秒</td></tr>
    <tr><td>Cold + minInstances=1</td><td>5.0 秒</td><td>1.4 秒</td></tr>
  </tbody>
</table>
...
上の表を取ったのは 2026 年 4 月です。その後 Flash 系のラインナップが入れ替わったため、この表をそのまま「Flash なら月 $17」と読むと外れます。

`2.5 Flash` で測ったときの実効単価は 1 リクエストあたり $0.0014、月 12,000 件で約 $17 でした。一方 2026 年 7 月時点で公開されている `3.6 Flash` の単価は、100 万トークンあたり入力 $1.50・出力 $7.50 です。同じトークン数を流したときの計算はこうなります。

<table>
  <thead>
    <tr><th>項目</th><th>月間トークン数</th><th>単価（100 万トークンあたり）</th><th>月額</th></tr>
  </thead>
  <tbody>
    <tr><td>入力（1,800 × 12,000 件）</td><td>21.6M</td><td>$1.50</td><td>$32.40</td></tr>
    <tr><td>出力（250 × 12,000 件）</td><td>3.0M</td><td>$7.50</td><td>$22.50</td></tr>
    <tr><td><strong>合計</strong></td><td>—</td><td>—</td><td><strong>約 $54.9</strong></td></tr>
  </tbody>
</table>

同じワークロード、同じトークン数、モデル名に入っている単語も同じ「Flash」です。それで月額が $17 から $55 に上がります。3 倍以上の差です。

`3.6 Flash` は `3.5 Flash` に比べて出力トークンがおよそ 17% 少なくなると案内されています。それを織り込んで出力を 250 → 208 トークンで見積もっても、出力側は $18.7、合計で約 $51 にしかなりません。消費量の改善は確かに効きますが、単価の差を埋めるほどではない、というのが計算して出た結論でした。

ここで候補に上がるのが `3.5 Flash-Lite` で、入力は 100 万トークンあたり $0.3 とされています。レビュー要約のような「定型的な日本語の整形」は、判断の難しさより処理量のほうが支配的なタスクです。世代を上げること自体が目的でないなら、素直に Lite 系に降りるほうが筋が良いと考えています。

### トークン数は実測、単価は変数として持つ

この件で学んだのは、**実測して意味があるのはトークン数と p95 レイテンシまでで、金額はいつでも再計算できる形にしておく**、ということでした。金額をドキュメントに直接書き込むと、モデルが変わった瞬間にすべてが古くなります。しかも古くなったことが誰にも見えません。

そこで、計測結果は「1 リクエストあたりのトークン数」で保存し、単価は別ファイルから読むようにしました。

```typescript
// pricing.ts — 単価だけを分離して置く。公式の価格ページを見て更新する
export const PRICE_PER_1M = {
  "gemini-2.5-flash":      { input: 0.30, output: 2.50 },
  "gemini-3.6-flash":      { input: 1.50, output: 7.50 },
  "gemini-3.5-flash-lite": { input: 0.30, output: null },  // 出力単価は未確認
} as const;

type Measured = {
  inputTokens: number;      // 1 リクエストあたりの実測値
  outputTokens: number;
...
// 2026-04 に取った実測トークン数を、いまの単価で読み直す
const measured: Measured = { inputTokens: 1800, outputTokens: 250, requestsPerMonth: 12000 };

monthlyCost("gemini-2.5-flash", measured);  // → 約 $14.0
monthlyCost("gemini-3.6-flash", measured);  // → 約 $54.9
```

`gemini-2.5-flash` の計算結果 $14.0 は、実測の $17 とずれます。この差はリトライと失敗分です。理論値はあくまで成功した呼び出しだけを数えるので、レート制限で弾かれて再送した分、スキーマ検証に落ちて投げ直した分は乗ってきません。私の場合はこの差がおよそ 2 割で、以降は理論値に 1.2 を掛けたものを予算の目安にしています。

`output: null` を握りつぶさずに例外にしているのも意図的です。一度「Lite に寄せれば入力単価は据え置きだから総額も据え置きのはず」と早合点しかけたことがあり、確認していない値は静かに通さないようにしました。

単価は変わります。上の数字も記事化の時点で [Gemini API の価格ページ](https://ai.google.dev/gemini-api/docs/pricing) を確認して入れたものですので、お読みになる時点でご自身で最新の値を確かめてください。

## 広告収益で固定費を回すアプリで、Genkit をどこに置くか

最後に、広告収益で固定費をまかなっているアプリで実際に採用している「Genkit の置き場所」の設計を共有します。この構成では、ユーザー体験を 1 秒でも遅らせると広告 impression あたりの収益（eCPM）に直接響くため、Gemini のような「答えが返るまで数秒待たされる API」をどこに置くかが、そのまま収益に効いてきます。
...
<table>
  <thead>
    <tr><th>用途</th><th>推奨</th><th>理由</th></tr>
  </thead>
  <tbody>
    <tr><td>ユーザー操作直結（即時性必要・Gemini を直接呼ぶ）</td><td>Cloud Run + minInstances=1 + Flash</td><td>Cold Start を吸収しないと UX が崩れる</td></tr>
    <tr><td>バッチ処理（オフピーク・大量）</td><td>Cloud Functions Gen2 + 適切な concurrency</td><td>起動の柔軟さとコスト</td></tr>
    <tr><td>ストリーミングが必要</td><td>Cloud Run（Cloud Functions は SSE 不向き）</td><td>Functions の SSE はタイムアウト 60 秒で詰む</td></tr>
    <tr><td>数秒の処理を月数百回</td><td>Cloud Functions（最安構成）</td><td>起動コストが時間課金より安い</td></tr>
    <tr><td>数十秒の処理を月数万回</td><td>Cloud Run（concurrency 活用）</td><td>1 コンテナで多重処理できる</td></tr>
  </tbody>
</table>
...
うまくいかなかったことも書いておきます。入力の長さでモデルを自動的に振り分ける仕組みを一度入れましたが、これは外しました。長い入力ほど難しい、という前提が実際のレビュー本文には当てはまらず、むしろ短い一文のほうが皮肉や言い回しの解釈でつまずくことが多かったためです。いまは用途（要約か、生成か、推論か）で明示的に選ぶ形に戻しています。自動化できそうに見える判断ほど、根拠になる相関を先に確かめたほうが良かったと感じています。
