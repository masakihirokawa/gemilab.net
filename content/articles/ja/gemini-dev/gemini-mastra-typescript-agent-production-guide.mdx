---
title: "Mastra と Gemini API で TypeScript エージェントを 30 分で本番投入する実践ガイド"
...
この記事では、Mastra と Gemini API を組み合わせて、ローカル開発から Cloudflare Workers へのデプロイまでを一気通貫で動かす手順を、実際にハマった落とし穴つきでまとめます。
...
- **Mastra**: AI SDK の関数ラッパーに近く、`createAgent` / `createTool` / `Workflow` といった最小限の構造を提供します。`mastra dev` がローカルでトレースを可視化してくれるので、エージェントのデバッグが圧倒的に楽になります。
...
「Gemini 2.5 Flash と Pro のどちらを選ぶか」で迷うこともあるかもしれません。私は開発初期は Flash、品質を詰める段階で Pro に切り替える方針を取っています。コストとレイテンシのバランスでは Flash が圧倒的に扱いやすく、エージェント特有の試行錯誤回数とよく合います。
...
TypeScript で本格的に AI 周りを設計したい方は、[『AI エージェント開発入門 LangChain と LangGraph による実装と評価』（西見公宏 著）](https://www.amazon.co.jp/dp/4297143461/?tag=pinocchio-22) を手元に置いておくと、フレームワークが違っても通用するエージェント設計の原則が体系的に学べます。
