---
title: ".geminiignore で Gemini CLI と Code Assist のコンテキスト精度を上げる実践ガイド"
...
この問題を解くための公式機能が `.geminiignore` です。`.gitignore` と書式は同じですが、役割は「Git に含めない」ではなく「Gemini に見せない」。本稿では、見落とされがちなこのファイルの実践的な使い方を、私が実際の開発現場で詰まったポイントと一緒に整理していきます。
...
書籍で体系的に学びたい方には、[ChatGPT/LLMシステム構築入門（高橋朋子・山田祐輔）](https://www.amazon.co.jp/dp/4297143356/?tag=pinocchio-22) が、コンテキスト設計の考え方を実装レベルで理解するのに役立ちました。`.geminiignore` の判断基準を磨くうえで、汎用的な前提知識が得られる一冊です。
