...
本記事では、Manifest V3 に対応したサイドパネル型の Chrome Extension を一から開発する方法を解説します。ページコンテンツの自動要約、多言語翻訳、コードスニペットの解析といった実用的な機能を備えた拡張機能を、Gemini API を使って実装していきます。
...
**Q: API キーを Chrome Extension に埋め込んでも安全ですか？**

A: ソースコードに直接記述するのは推奨されません。本記事のように `chrome.storage.local` に保存し、Service Worker 内でのみ使用する方法が基本的な安全対策です。さらにセキュリティを高めたい場合は、自前のプロキシサーバーを経由させて API キーをサーバー側で管理する構成も検討してください。

**Q: Gemini API の無料枠で拡張機能を運用できますか？**

A: 個人利用であれば十分です。2026年3月時点で、Gemini 2.5 Flash は1分間15リクエスト・1日1,500リクエストの無料枠があります。ページ要約を1日に数十回使う程度であれば余裕を持って運用できます。リクエスト数が増える場合は [Gemini API の Function Calling 実践ガイド](/articles/gemini-api/gemini-function-calling-practical-guide) でバッチ処理との組み合わせを検討するのもよいでしょう。

**Q: Chrome Web Store に公開するにはどうすればよいですか？**

A: Chrome Web Store の開発者アカウント（登録料 $5、一度きり）を作成し、拡張機能を ZIP 化してアップロードします。審査プロセスでは、プライバシーポリシーの設置と `permissions` の最小化が求められます。特に `<all_urls>` を使う場合はその正当性を説明する必要があります。

**Q: Manifest V2 と V3 の違いで注意すべき点はありますか？**

A: 最大の違いは、バックグラウンドスクリプトが常駐型の Background Pages から Service Worker に変わった点です。Service Worker はアイドル状態が続くと自動的に停止するため、長時間の処理やタイマーには `chrome.alarms` API を使う必要があります。また、リモートコードの実行が禁止されたため、すべてのコードをパッケージ内に含める必要があります。
