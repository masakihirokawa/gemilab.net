...
この記事では、その本番運用で得た知見を体系的にまとめます。とりわけ「Workers だけで作ろうとして詰まった」「KV や D1 で会話履歴を持たせようとして同時編集が壊れた」といった、私が実際に踏んだ落とし穴を中心に書いていきます。
...
価格面でも、Workers Paid プラン（月 $5）に Durable Objects が含まれており、リクエスト数とコンピュート時間に応じた従量課金で、個人開発で本番投入する規模なら月数ドルの追加で十分です。書籍で体系的に学ぶなら、[Cloudflare Workers サーバーレス開発実践ガイド（柳澤永人 著）](https://www.amazon.co.jp/dp/B0BYVMJJ4Z/?tag=pinocchio-22) が Durable Objects の設計判断まで踏み込んで書かれており参考になります。
