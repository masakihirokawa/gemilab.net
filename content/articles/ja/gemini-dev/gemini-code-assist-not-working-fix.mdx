---
title: "Gemini Code Assist の補完が出ないとき — 症状別の切り分け手順"
...
noindex: true
author: "Gemini Lab"
description: "Gemini Code Assist のコード補完が効かない、VS Code 拡張機能が応答しない、Outline が出ないといった症状を切り分ける手順です。基本チェックリスト、ログでの原因特定、よくあるエラーメッセージ、エージェントモードだけ動かない場合まで扱います。"
...
- `Failed to complete task: context limit exceeded` — 指示が広すぎます。ファイルや機能を絞ってください。
- `Agent mode is not available for your plan` — ライセンスが未対応です。プランを確認してください。
- `Unable to index workspace` — ワークスペースが大きすぎるか権限の問題です。出力パネルの詳細ログを確認してください。
