---
title: "Gemini APIでCore Web Vitalsを自動診断・最適化する実践ガイド"
...
## はじめに — Core Web VitalsとGemini AIの組み合わせが強力な理由
...
本記事では、Node.js（TypeScript）を使ってPageSpeed Insights APIからデータを取得し、Gemini APIで自動診断・最適化提案を生成する実践的なフローを紹介します。
...
本記事のテーマをさらに深めたい方には、[「Webフロントエンドパフォーマンスチューニング」（奥野幸典 著）](https://www.amazon.co.jp/dp/4297142279/?tag=pinocchio-22)がおすすめです。Core Web VitalsとLighthouseの詳細な解説が豊富に収録されています。

---

## よくある質問（FAQ）

**Q. PageSpeed Insights APIは無料で使えますか？**
はい、個人利用の範囲では無料で利用できます。ただし1日あたりのリクエスト数に制限があります（デフォルトは400リクエスト/日）。大量のURLを定期的に監視する場合はGoogle Cloud Projectでのクォータ申請が必要です。

**Q. Gemini APIのどのモデルが診断に向いていますか？**
軽量・低コストで十分な分析精度を持つ `gemini-2.5-flash` が最適です。より詳細な改善提案（アーキテクチャレベルの変更を含む場合など）は `gemini-2.5-pro` の利用も検討してください。なお、コスト感は[Gemini API料金完全ガイド](/articles/gemini-api/gemini-api-pricing-billing-complete-guide-2026)でも確認できます。

**Q. CIパイプラインに組み込むことはできますか？**
可能です。GitHub ActionsやCloud Build上でこのスクリプトを実行し、Core Web Vitalsが閾値を超えた場合にSlackに通知する構成が人気です。`gemini-api-slack-bot-production-guide` の実装パターンが参考になります。

**Q. モバイルとデスクトップ、どちらを優先すべきですか？**
Googleの検索インデックスはモバイルファーストのため、まずモバイルの指標を優先してください。本記事のサンプルコードでは `strategy=mobile` を指定しています。デスクトップも確認したい場合は `strategy=desktop` でリクエストを追加してください。
