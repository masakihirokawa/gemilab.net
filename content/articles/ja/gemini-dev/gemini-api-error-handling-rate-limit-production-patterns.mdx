...
---
## 取り組みの背景 — なぜ本番AIアプリは「気づかないうちに壊れる」のか
...
AI を組み込んだアプリの障害は、この形で届きます。プロセスは落ちません。アラートも鳴りません。機能だけが静かに抜け落ちて、気づくのは数日後のレビューです。200 で中身が空、ストリーミングが途中で切れる、安全フィルターで応答がブロックされる——従来の Web API のように「例外が飛んだから止まった」という分かりやすい壊れ方をしてくれません。
...
- **404 Not Found** — 指定したモデル名が存在しません。タイポだけでなく、**使っていたモデルが世代交代で消えたとき**にも同じ 404 が返ります。後述の「モデルの世代交代は 404 として届く」で扱います
...
- **OPEN（遮断）**: リクエストを即座に拒否。クールダウン期間を待機
...
Gemini API の[非推奨情報ページ](https://ai.google.dev/gemini-api/docs/deprecations)（2026年8月27日更新の表記時点）を開くと、少し意外な並びになっています。`gemini-2.5-pro` / `gemini-2.5-flash` / `gemini-2.5-flash-lite` には停止日が入っていません。一方で `gemini-3.1-flash-lite` には 2027年5月7日という日付があり、推奨移行先として `gemini-3.5-flash-lite` が指定されています。新しいほうに期限があり、古いほうに期限がないのです。バージョン番号の大小と寿命は一致しません。
...
これはカスケードの並び順に直接効きます。私が最初に書いたコードには、Flash 段の単価を入力 $0.15 / 出力 $0.60 と置いていました。料金ページを引き直したら実際は $0.30 / $2.50 で、出力側が4倍ずれていました。単価をコードに埋めたまま何ヶ月も放置すると、フォールバックが起きるほど原価が想定から離れていきます。落ちないための仕組みが、知らぬ間に赤字を積む仕組みに変わっていたのです。エラーハンドリングの話が、そのまま原価管理の話になる瞬間でした。
...
対策はシンプルで、デプロイのたびに一度だけモデルの存在を確認し、世代が偏っていたら起動を止めます。3時に叩き起こされるより、デプロイの30秒で気づくほうがずっと安く済みます。
...
世代を分散させるという方針で組み直したのが、先ほどのカスケードです。`gemini-3.1-pro` / `gemini-3.7-flash` / `gemini-3.1-flash-lite` で 3.1 と 3.7 の二世代にまたがっており、`assert_cascade_healthy` を通ります。最下段の 3.1 Flash-Lite には 2027年5月7日の告知が出ているので、そこは `gemini-3.5-flash-lite` への差し替えを予定に入れてあります。移行先が既に名指しされている状態は、むしろ扱いやすいものです。困るのは、期限が書かれていないまま黙って消えるほうです。
...
ここからは机上の設計論ではなく、私自身が個人開発で運営している壁紙アプリと癒し系アプリの中で Gemini API を組み込んだ際に、実際に踏んだ落とし穴を共有します。モバイルアプリを長く続けてきた経験で言えるのは、API ドキュメントだけ読んでも見えない問題が必ず本番で出てくるということです。
...
対策としては、Gemini API 用のクライアントを独立したコネクションプールで運用する、もしくはサーキットブレーカーで OPEN 状態のときには非同期キューを早期に空にする処理を入れる点が有効でした。個人開発の土台になっている AdMob の収益を守るためにも、AI 機能と広告枠の独立性は守ったほうが安心です。
...
Gemini API の本番運用は、ひとつの API 呼び出しを守るのではなく、エラー分類・指数バックオフ・サーキットブレーカー・トークンバケット・モデルカスケードという多層の防御線を組み合わせて、「障害が目に見えないまま広がらない構造」をつくる仕事だと感じています。運用してきて手元に残った教訓は三つです。最初から複雑に組みすぎないこと。本番投入の初日からサーキットブレーカーを入れること。そして、守っているモデルそのものに寿命があると忘れないこと。
