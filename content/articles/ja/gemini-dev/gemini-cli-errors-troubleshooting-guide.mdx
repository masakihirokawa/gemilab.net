...
Gemini CLI（コマンドラインインターフェース）は非常に強力なツールですが、セットアップの段階でいくつかのトラブルに遭遇しやすいです。このガイドでは、最も一般的なエラーと、それぞれの解決方法を詳しく説明します。
...
**Q1: インストール後、ターミナルを再起動しても gemini コマンドが見つかりません。**

A: PATH 設定が正しく反映されていない可能性があります。ターミナルアプリケーション自体を完全に閉じて、新しいウィンドウを開いてください。macOS の場合、Command + Q でアプリを終了し、Dock または Spotlight から再度起動してください。それでも解決しない場合、`echo $PATH` コマンドで PATH を確認し、npm のグローバルディレクトリが含まれているか確認してください。

**Q2: API キーを設定しても「API key not found」エラーが出ます。**

A: 環境変数が正しく設定されていない可能性があります。まず `echo $GEMINI_API_KEY` で環境変数の内容を確認してください。空の場合、`~/.bashrc` または `~/.zshrc` に正しく追加されていません。設定ファイルを編集した後、必ず `source ~/.bashrc` または `source ~/.zshrc` を実行して変更を反映してください。また、API キーに余分なスペースや改行が含まれていないか確認してください。

**Q3: Plan Mode を有効化したのに「Plan mode is not available」というエラーが出ます。**

A: Pro プランの登録が Google のサーバーに反映されるまでに時間がかかることがあります。1〜2時間待ってから再度試してください。それでも解決しない場合、`gemini auth clear` で認証情報をクリアし、`gemini auth login` で再度ログインしてください。Google AI Studio（https://aistudio.google.com/app/plan）で Pro プランが有効になっていることを確認してください。
