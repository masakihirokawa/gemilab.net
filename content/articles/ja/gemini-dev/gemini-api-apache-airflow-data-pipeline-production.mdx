---
title: "Gemini API × Apache Airflow で本番AIデータパイプラインを構築する完全ガイド"
...
この記事では、実際の本番環境を想定した以下のシナリオを題材にします。
...
**Q1. Airflow の Worker が OOM（メモリ不足）になった場合はどうすればよいですか？**

A. 大量のドキュメントを処理する場合、XCom でデータをやり取りするとメモリ問題が起きやすいです。XCom には最小限のメタデータ（ファイルパスや ID など）のみを保存し、実際のデータは GCS や BigQuery を経由させるアーキテクチャに切り替えることを推奨します。また、`AIRFLOW__CORE__XCOM_BACKEND` を GCS ベースの Custom XCom Backend に変更することも有効です。

**Q2. Gemini API のレートリミットに頻繁に引っかかる場合の対処法は？**

A. まず、利用しているプロジェクトの Tier を確認してください（Google AI Studio の無料版と Tier 1/2 ではレートリミットが大きく異なります）。Airflow の Pool を使って同時実行数を制限し、テナシティライブラリの指数バックオフを実装することで、大半のケースは解消できます。それでも不十分な場合は、Gemini 2.5 Flash（レートリミットが緩い）と Pro を動的に切り替える戦略が効果的です。

**Q3. 本番 DAG のデバッグに役立つ Airflow のツールは何ですか？**

A. Airflow の `airflow tasks test` コマンドを使うと、スケジューラーを経由せず特定タスクだけをローカル実行できます。また、`airflow dags backfill` で過去日付のデータを再処理できます。DAG ファイルのシンタックスエラーは `python dags/your_dag.py` で事前にチェックできます。本番では OpenLineage と Airflow の連携によるデータリネージ追跡も強くお勧めします。

**Q4. コストを最小化するためのモデル選定指針を教えてください。**

A. ユースケース別の推奨モデルは次の通りです。単純な分類・要約（精度よりスピード重視）は `gemini-2.5-flash`、複雑な推論・長文処理・コード生成は `gemini-2.5-pro`、実験・開発段階のプロトタイピングは `gemini-2.0-flash` が費用対効果が高いです。パイプライン全体で `gemini-2.5-flash` を使うと、`gemini-2.5-pro` 比で約 95% のコスト削減が可能です。

**Q5. Airflow の DAG テストはどのように行えばよいですか？**

A. `pytest` と `airflow.models.DagBag` を使って DAG の構造テストを行えます。各タスクの Python 関数は純粋なユニットテストとして `unittest.mock` で Gemini API をモックしてテストできます。また、Airflow の `@task` デコレーターを使うと関数を直接呼び出せるため、テストが書きやすくなります。

---

## まとめ

Apache Airflow と Gemini API を組み合わせることで、単発の AI 呼び出しを超えた「継続的に動く本番 AI パイプライン」を構築できることを確認しましました。重要なポイントを整理します。
