...
---
手元の Flutter アプリに AI チャットを足すかどうかで、半年ほど決めきれずにいました。理由は単純で、このアプリは AdMob の広告収益で回っているため、トークン単価がその収益を食い潰した時点で機能ごと畳むしかないからです。Gemini Flash の単価が実用ラインまで降りてきた 2026 年、ようやく計算が合いそうだと判断して、個人開発のアプリ 1 本に組み込み、実運用に回しました。このノートはその記録です。
...
私の落とし所は「ストリーミング中はローカル状態だけで、`onDone` の段階でメッセージ全文と `usageMetadata` を 1 回だけ書き込む」です。途中経過の保存はオフラインキャッシュ（Hive）に逃がします。これで Firestore 書き込みが約 90 ％減り、月額 $7 前後だったコストが $0.9 前後まで落ちました。
...
<tbody>
<tr><td>Gemini API（Flash・入出力合計）</td><td>約 $4.8</td><td>チャット利用は MAU の 12 ％</td></tr>
<tr><td>Firestore（メッセージ書き込み・読み出し）</td><td>約 $0.9</td><td><code>onDone</code> でまとめ書きに変更した後の値</td></tr>
<tr><td>Cloud Functions（Web 用プロキシのみ）</td><td>約 $0.4</td><td>モバイルは直接呼び出しのため対象外</td></tr>
<tr><td><strong>AdMob 追加収益</strong>（滞在時間増加分の eCPM 上振れ）</td><td><strong>約 ¥3,200</strong></td><td>チャット導線を置いた画面のみを比較</td></tr>
</tbody>
</table>
...
## 安全性フィルタで返ってこない応答と、ストア審査で増える宿題

リリースして最初に驚いたのは、`response.text` が空のまま返ってくる場面が実運用では珍しくない、という点でした。手元で自分の質問だけを投げていた開発中には一度も起きなかったのに、公開後の 1 週間で数十件観測しています。

弾かれ方は 2 通りあります。入力側で止まった場合は `candidates` が空のまま `promptFeedback.blockReason` だけが返り、生成側で止まった場合は candidate 自体は返るのに `finishReason` が `safety` になります。この 2 つを区別せずに `response.text!` と書いていると、後者で null 例外になりチャット画面ごと落ちます。

```dart
// lib/services/gemini_service.dart — 弾かれた応答を例外として扱う
class ChatBlocked implements Exception {
  ChatBlocked(this.stage, this.reason);
  final String stage; // 'prompt' か 'response'
  final String? reason;
}

String? unwrap(GenerateContentResponse res) {
  // 入力が弾かれると candidates は空で、promptFeedback だけが返る
  final promptBlock = res.promptFeedback?.blockReason;
  if (promptBlock != null) {
    throw ChatBlocked('prompt', promptBlock.name);
  }
  if (res.candidates.isEmpty) return null;

  final c = res.candidates.first;
  // 生成側が弾かれた場合は candidate は返るが finishReason が変わる
  if (c.finishReason == FinishReason.safety ||
      c.finishReason == FinishReason.recitation) {
    throw ChatBlocked('response', c.finishReason?.name);
  }
  return c.text;
}
```

ストリーミングでは、`finishReason` が届くのは最後のチャンクです。つまりユーザーの画面には途中まで文章が流れた後で「実は弾かれていた」と分かることになります。私は途中まで表示したテキストを消さず、末尾に停止理由の一行を添える形に落ち着きました。消してしまうと通信エラーと区別がつかず、問い合わせが増えたためです。ChatNotifier 側では `ChatBlocked` を捕まえて `state` のメッセージに `stoppedReason` を持たせ、UI はそれを薄いグレーの注記として描画するだけにしています。

### ストア審査は「AI が喋る」時点で観点が増える

もう一つ、Gemini のドキュメントには書かれていない宿題があります。AI が生成した文章をユーザーに見せるアプリは、両ストアともユーザー生成コンテンツを扱うアプリに近い扱いとなり、審査の観点が増えます。2026 年時点で私が対応した範囲を並べておきます。

<table>
<thead>
<tr><th>観点</th><th>iOS（App Store）</th><th>Android（Google Play）</th></tr>
</thead>
<tbody>
<tr><td>不適切な出力のフィルタ</td><td>ガイドライン 1.2 の対象。既定の <code>safetySettings</code> を使っている旨を審査メモに明記</td><td>生成 AI 関連ポリシーの対象。同様に明記</td></tr>
<tr><td>アプリ内の通報導線</td><td><strong>必須</strong>。AI の返信そのものを報告できること</td><td><strong>必須</strong>。生成された出力を報告できること</td></tr>
<tr><td>問題ユーザーの遮断</td><td>ユーザー同士のやり取りがある場合に必要（AI との 1 対 1 だけなら対象外）</td><td>同左</td></tr>
<tr><td>年齢レーティング</td><td>質問票の再回答が必要になる場合あり</td><td>コンテンツレーティングの再申請</td></tr>
</tbody>
</table>

実装としては、AI メッセージの長押しメニューに「この返信を報告」を足し、会話 ID・該当メッセージ・任意コメントを Firestore の別コレクションに書くだけの導線を用意しました。作業量としては半日程度です。ただ、これが無い状態で審査に出すと止まります。年齢レーティングも、チャット機能を足した時点で質問票の回答が変わる可能性があるため、アップデート時に見直しておくほうが安全です。

`safetySettings` を緩めればフィルタの挙動そのものは調整できますが、緩めた分だけ審査側で説明すべきことが増えます。個人開発で運用負荷を抑えたいなら、フィルタは既定のまま、弾かれたときの見せ方と通報導線に手をかけるほうが割に合うと感じています。
...
7. 安全性: `promptFeedback.blockReason` と `finishReason` を必ず分けて処理し、弾かれた応答は消さずに理由を添えて残す（通報導線とセットで審査に出す）
