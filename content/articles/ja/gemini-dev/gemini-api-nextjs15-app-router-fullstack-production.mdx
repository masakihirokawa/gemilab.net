---
title: "Next.js 15 App Router × Gemini API フルスタック本番構築完全ガイド"
...
## はじめに：Next.js × Gemini API が最強の組み合わせである理由

Next.js 15 は 2025 年末に安定版がリリースされ、App Router によるサーバーコンポーネント・Server Actions・Partial Prerendering（PPR）が実用段階に達しましました。これと Gemini 2.5 Pro の組み合わせは、AI 機能を持つウェブアプリ開発において現在最も生産性の高い選択肢の一つです。

この記事では、単なる "hello world" を超えた **本番運用に耐えうるフルスタック AI アプリ** を構築するために必要な知識を体系的に解説します。対象読者は Next.js の基礎を理解しており、Gemini API を本格的なプロダクトに組み込みたい中〜上級者です。
...
**Q1: Server Components と Client Components のどちらから Gemini API を呼ぶべきですか？**

基本的には **Server Components・Server Actions・Route Handler（サーバーサイド）からのみ** 呼び出してください。クライアントコンポーネントから直接 Gemini API を呼ぶと、APIキーがブラウザに露出します。クライアントから使う場合は、必ず Route Handler `/api/...` を経由させ、認証・レート制限を実装します。

**Q2: Vercel の Edge Runtime と Node.js Runtime どちらを使うべきですか？**

Streaming レスポンス（`streamText`）には **Edge Runtime** が推奨です。レイテンシが低く、サーバーレス関数より起動が速いです。ただし、Edge Runtime では Node.js のすべての API（`fs`, `path` など）が使えません。データベース接続（Prisma など）を使う場合は Node.js Runtime を選択してください。

**Q3: Gemini 2.5 Pro の API コストが心配です。どう対策しますか？**

3つの対策が有効です。①用途に応じたモデル選択（Flash vs Pro）、②`unstable_cache` による同一クエリのキャッシュ、③Context Caching の活用です。また、Vercel の Analytics で API レスポンスタイムとトークン使用量を監視し、異常を早期発見することも重要です。[Gemini API のコスト最適化戦略](/articles/gemini-api/gemini-api-cost-optimization-complete-guide)の記事も参考にしてください。

**Q4: マルチユーザー対応のチャット履歴はどう管理しますか？**

`useChat` フックは `initialMessages` プロップをサポートしているため、データベース（Vercel Postgres, PlanetScale など）から取得したメッセージ履歴を初期値として渡せます。セッションIDをキーにしてメッセージを保存・取得する Route Handler を実装します。NextAuth.js と組み合わせてユーザーごとの履歴管理が可能です。

**Q5: Gemini API を使った機能を A/B テストしたい場合は？**

Vercel の Edge Config と Feature Flags を組み合わせるのが最も簡単です。`@vercel/edge-config` でフラグを取得し、ユーザーをグループ分けして異なるプロンプトやモデルを提供します。Vercel Analytics でメトリクスを測定し、どの設定が最も効果的かを検証できます。

---

## まとめ

本記事では、Next.js 15 App Router と Gemini API を組み合わせたフルスタック AI アプリの本番構築について、以下の点を包括的に解説しましました。
