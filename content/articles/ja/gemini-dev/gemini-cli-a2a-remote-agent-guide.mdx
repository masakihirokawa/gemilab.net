---
title: "Gemini CLI × A2A プロトコル — リモートエージェント連携の実践ガイド"
...
highlights:
  - "A2Aプロトコルの仕様とリモートエージェント設定の完全手順を実践的に習得できる"
  - "HTTP認証・エージェントカード探索の実装パターンとセキュリティ設計"
  - "Gemini Enterprise・Cloud Run上でのA2Aエージェント本番デプロイ手法を習得できる"
---
...
この記事では、A2Aプロトコルの仕様から実装、本番デプロイまでを完全に解説します。

## A2A プロトコルの基本概念

### A2A とは何か

A2A（Agent-to-Agent）は、エージェント同士が相互に通信するための標準プロトコルです。2023年にGoogleが Linux Foundation に寄贈した オープンスタンダードをベースとしており、2026年3月にGemini CLIで本格採用されました。
...
Gemini CLI の A2A プロトコルにより、エージェント連携が本格的に可能になりました。
...
- [マルチエージェントシステム入門（横尾 真 著）](https://www.amazon.co.jp/dp/4320124081/?tag=pinocchio-22)
- Gemini CLI 公式ドキュメント
