...
author: "Gemini Lab"
description: "Gemini APIとPWAを組み合わせて、Service Workerによるオフライン対応・IndexedDBでのAIコンテキスト永続化・Web Push通知の統合まで、本番レベルのAI搭載Webアプリを実装する手順と、キャッシュヒット率からAPIコストを可視化する実装をまとめます。"
tags: ["gemini-api", "PWA", "Service Worker", "オフライン", "Web Push", "IndexedDB", "JavaScript"]
highlights:
  - "Service Workerのヒット/ミスを計測し、回避できたGemini API呼び出しを実数で把握する実装"
...
<table>
<thead>
<tr><th>キャッシュ戦略</th><th>ヒット率の目安</th><th>向くケース</th></tr>
</thead>
<tbody>
<tr><td>完全一致キーのみ</td><td>約10〜15%</td><td>入力の揺れが小さいフォーム系</td></tr>
<tr><td>正規化キー（小文字化・空白圧縮）</td><td>約25〜30%</td><td>チャット・検索の言い換え吸収</td></tr>
<tr><td>正規化＋意味的近傍（埋め込み照合）</td><td>約35〜45%</td><td>FAQ・定型回答が多いアプリ</td></tr>
</tbody>
</table>
...
[Cloudflare Workersと組み合わせたバックエンドプロキシの構築はこちら](/articles/gemini-api/gemini-api-cloudflare-d1-edge-ai-production-masterclass)、[Next.js + App RouterでのGemini API統合はこちら](/articles/gemini-dev/gemini-api-nextjs15-app-router-fullstack-production)も参考にしてください。
