...
highlights:
  - "Gemini APIでWebページを解析し、テストシナリオを自動生成する仕組みを習得できる"
  - "Playwright + Gemini連携によるセルフヒーリングテストの実装パターンを学べる"
  - "CI/CDパイプラインに組み込む本番運用アーキテクチャを構築できる"
---
...
Gemini API の強力なマルチモーダル解析能力と、Playwright のブラウザ自動操作を組み合わせることで、この課題を根本から解決できます。本記事では、WebページのスクリーンショットやHTML構造を Gemini に渡し、テストシナリオの策定からテストコードの生成、さらにはUI変更時の自動修復（セルフヒーリング）までを実現するシステムを構築します。
...
### Q1: Gemini が生成したテストの精度はどのくらいですか？

実際のプロジェクトでの検証では、初回生成時の精度は約70〜80%です。残りの20〜30%は、アプリケーション固有のビジネスロジック（正しいテストデータの指定、認証トークンの取得方法など）に起因するもので、セルフヒーリング機能で半数程度は自動修復できます。最終的に手動修正が必要なのは全体の10〜15%程度です。

### Q2: セルフヒーリングの成功率はどのくらいですか？

セレクター変更のみで修正可能なケースでは約90%、ページ構造の大幅な変更を伴うケースでは約50〜60%の成功率です。Gemini 2.5 Pro のマルチモーダル解析能力により、スクリーンショットの差分からUI変更の意図を推測できるため、単純なテキストベースの修復ツールよりも高い精度を実現しています。

### Q3: APIコストの目安はどのくらいですか？

1ページあたりのテスト生成コストは、Gemini 2.5 Pro 使用時で約$0.02〜0.05です。100ページのWebアプリに対して週次で全テストを再生成した場合、月額$10〜20程度になります。キャッシュ戦略とFlashモデルの併用で、これをさらに半分以下に抑えることが可能です。

### Q4: 既存のテストスイートとの共存は可能ですか？

はい、可能です。生成されたテストファイルを `tests/generated/` のように専用ディレクトリに配置し、手動で書いたテストとは別に管理することを推奨します。`playwright.config.ts` でプロジェクトを分割すれば、実行タイミングや並列度も個別に設定できます。
