...
noindex: true
author: "Gemini Lab"
description: "TypeScript で Gemini API を呼ぶコードを Vitest でテストする実践パターンです。SDK のモック設計、Function Calling とストリーミングの検証、MSW への降ろし方、夜間に実 API を一本だけ通すスモークまで扱います。"
tags:
  - "gemini-api"
  - "vitest"
  - "typescript"
  ...
  - "function-calling"
highlights:
  - "429・503 を意図的に発生させてリトライ上限と即時失敗を固定するテスト設計"
  - "SDK の戻り値の形が変わってもテストが道連れにならない MSW ハンドラの書き方"
  - "実 API を一本だけ夜間に通してモデル非推奨と認証失効だけを検知する構成"
---
...
## vi.mock で足りなくなる日 — MSW で HTTP レイヤーに降りる

前節の最後で「本番に近づくにつれ MSW」と書きましたので、実際にその日が来たときの手触りを残しておきます。

`vi.mock` は SDK のクラス構造そのものを模倣します。裏を返せば、テストが SDK の内部形状に結びつきます。私の場合、戻り値の形が `result.response.text()` から `result.text` に変わったバージョンに上げたとき、アプリの振る舞いは何も変えていないのにテストだけが十数本まとめて落ちました。壊れたのはコードではなくモックのほうです。

MSW は一段下、HTTP のリクエストとレスポンスを差し替えます。SDK が内部で何をしていても、最終的に `generativelanguage.googleapis.com` に投げる POST の形は変わりません。ここを押さえておけば、SDK のバージョンアップにテストが道連れにされません。
...
export const handlers = [
  http.post(`${ENDPOINT}\\:generateContent`, async ({ request }) => {
    ...
    return HttpResponse.json({
      candidates: [
        {
          content: { role: "model", parts: [{ text: "1. 配送が早い" }] },
          finishReason: "STOP",
        },
      ],
...
ストリーミングは `?alt=sse` が付き、`data:` 行の連なりが返ります。ReadableStream を自前で組み立てれば、SSE の畳み込み処理まで含めて検証できます。

```ts
http.post(`${ENDPOINT}\\:streamGenerateContent`, () => {
  const chunks = ["こん", "にちは", "、世界"];
...
ここでつまずきどころを一つ。Gemini のエンドポイントは `models/gemini-2.5-flash:generateContent` のようにコロン区切りです。MSW のパスパターンではコロンがパス変数の記号なので、`\\:generateContent` とエスケープしないとハンドラが一致しません。しかも一致しなかったリクエストは既定では素通りするため、テストが静かに実 API を叩きます。私はこれで課金が発生しかけました。ハンドラを書いたら、サーバ起動時に `onUnhandledRequest: "error"` を必ず指定して、素通りを成功ではなく失敗として検出してください。

二つのやり方は、守れるものが違います。

<table>
<thead>
<tr><th>観点</th><th>vi.mock（SDK 全体）</th><th>MSW（HTTP レイヤー）</th></tr>
</thead>
<tbody>
<tr><td>テストが壊れる条件</td><td>SDK の戻り値の形が変わったとき</td><td>Gemini のワイヤ仕様が変わったとき</td></tr>
<tr><td>書き始めるまでの手間</td><td>数行。その場で書ける</td><td>ハンドラ一式の初期投資が要る</td></tr>
<tr><td>検証できる範囲</td><td>呼び出し引数と分岐</td><td>リクエストボディ・ステータス・SSE の形</td></tr>
<tr><td>SDK を乗り換えたとき</td><td>ほぼ全書き換え</td><td>そのまま通ることが多い</td></tr>
<tr><td>向いている用途</td><td>分岐を厚く固めたいテスト</td><td>契約として守りたい数本</td></tr>
</tbody>
</table>

全部を MSW に寄せる必要はない、というのが私の実感です。落とし所として使っているのは二層構成で、リトライ判定やツールのループ制御のような分岐の厚いテストは `vi.mock` のまま置き、「リクエストボディに何が入るか」「SSE をどう畳むか」を守る三、四本だけを MSW に降ろしています。全部を下げると実行時間が伸びますし、全部を上げたままにすると SDK のマイナーバージョンにテストスイートの寿命を預けることになります。
...
1. 1 回失敗 → リトライして成功すること（呼び出し回数 2）
2. 上限まで失敗 → 例外を投げ、それ以上呼ばないこと（呼び出し回数 = 上限）
3. 429 と 503 以外（例：400 のリクエスト不正）は即座に投げ、リトライしないこと
4. バックオフの待機時間がフェイクタイマーで飛ばせること
...
## モックだけでは見えない一点 — 夜間に実 API を一本だけ通す

モックはよくできた嘘です。内部で辻褄が合っているぶん、外の世界が動いたことは教えてくれません。私が実際に取りこぼしたのは、指定していたモデル ID が非推奨になっていたことでした。テストは緑のまま、本番だけが静かに失敗していました。

そこで、実 API を叩くテストを一本だけ残しています。PR の CI では走らせず、夜間のスケジュール実行でのみ有効化する形です。
...
アサーションを意図的に薄くしているのが、このテストの肝です。出力の中身は見ません。見ているのは「モデル ID が生きているか」「認証が通るか」「レスポンスの骨格が変わっていないか」の三点だけです。ここに文面の期待を足した瞬間、モデルのゆらぎで毎晩落ちるテストになり、やがて誰も見なくなります。

`setupFiles` で API キーをダミーに上書きしている場合、このテストだけは本物のキーが必要です。上書きに条件を一つ足しておきます。
...
費用は 1 日 1 回で月 30 回程度。無料枠に十分収まりますし、`gemini-2.5-flash` に数トークンを投げるだけなので体感できるコストにはなりません。

落ちたときの扱いも先に決めておいてください。このテストの失敗は「私のコードが壊れた」ではなく「足場のほうが動いた」の合図です。マージをブロックする対象ではなく、通知を受けて翌朝確認する対象として扱っています。プラットフォーム側の都合で CI が赤くなり、無関係な変更のマージまで止まるのは筋が違うからです。前節でプロンプトのスナップショットを警告に留めた話と、引いている線は同じです。
...
そのうえで余力ができたら、次の一手は二つのどちらかだと思っています。SDK の乗り換えが視野に入っているなら、MSW に数本だけ降ろしておく。夜間バッチや定期実行にコードが載っているなら、実 API のスモークを一本だけ足しておく。どちらも作業としては一日で入りますし、入れた翌朝から効き始めます。
