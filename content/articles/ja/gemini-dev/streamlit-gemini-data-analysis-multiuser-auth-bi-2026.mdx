...
第一に、Gemini に生成させるのは **必ず構造化出力**にし、自由テキストで SQL を返させない。`response_schema` で `{"chart_type": "bar"|"line", "x_column": str, "y_column": str}` のように制約します。
...
- **コスト計算**: トークン数と単価から JPY を即時計算し、ユーザー別・日別・月別に集計可能にする。
