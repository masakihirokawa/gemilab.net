...
複数 CSV を結合して分析するモードを追加します。Slack のスラッシュコマンドから CSV を投げて Slack 上に洞察を返す（Slack Bolt Python が 30 行で書けます）。Stripe で従量課金を載せて B2B SaaS にする（私の [Gemini API × Stripe SaaS 課金](/articles/gemini-api/gemini-api-stripe-saas-billing-webhook-production-guide) のパターンがほぼそのまま使えます）。
