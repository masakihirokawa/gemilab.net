---
title: "Google Forms × Gemini API 回答自動分析システム完全構築ガイド：感情分析・テーマ抽出・レポート自動生成まで"
...
highlights:
  - "Formsアンケートの集計・分析に毎週何時間も費やしていた人が、Gemini APIと組み合わせることで回答を自動で感情分析・テーマ抽出できるようになる"
  - "Apps ScriptとGemini APIの完全な実装コードを入手でき、コピペ＆設定変更だけで自社のアンケート運用に即日適用できる"
  - "単なる集計を超えた「インサイト抽出レポート」を自動生成できるようになり、マーケティング・HR・CS部門の意思決定スピードが飛躍的に向上する"
---
...
本記事では、Google Forms の回答を Gemini API で自動分析するシステムを **Apps Script（ノーコード寄り）** と **Python（より柔軟な実装）** の両アプローチで構築する方法を、実際のコードと共に詳細解説します。
...
本記事では両方の実装を提供します。まず Apps Script で動く基本システムを構築し、その後 Python で拡張する流れで解説します。
...
Gemini のレスポンスにマークダウンのコードブロック記法（```json ... ```）が含まれることがあります。`responseMimeType: 'application/json'` を明示的に指定することで、純粋なJSONのみが返るようになります。また、Python側では `json.loads(response.text.strip('```json\n').strip('```'))` として前処理するのも有効です。
...
本記事では、Google Forms の回答を Gemini API で自動分析するシステムを構築するための、Apps Script と Python の完全な実装コードを提供しましました。
