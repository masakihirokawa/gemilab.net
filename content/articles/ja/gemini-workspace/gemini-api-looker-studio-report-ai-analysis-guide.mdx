...
ビジネスの意思決定に欠かせないデータレポート。Google Looker Studio（旧 Google Data Studio）は、各種データソースを可視化できる無料の BI ツールとして多くの企業に利用されています。しかし、グラフや表を眺めるだけでは「なぜ売上が下がったのか」「次のアクションは何か」といったインサイトを導き出すには、まだ人間の解釈が必要でした。
...
この記事では、**Google Apps Script + Gemini API を使ったLooker Studio レポート自動分析**の実装方法を、コードの詳細まで丁寧に解説します。
...
**Q. Looker Studio の有料版でないと連携できませんか？**

いいえ、Looker Studio は無料版（個人利用）でも Google Sheets と連携できます。Google Workspace のサブスクリプションは必須ではありませんが、Looker Studio Pro（有料）では SLA やチーム共有機能が強化されます。

**Q. Gemini API の費用はどのくらいかかりますか？**

`gemini-2.5-flash` の場合、入力トークン 1M あたり $0.15、出力 1M あたり $0.60（2026年4月時点）です。1日1回の分析実行であれば、月額 $1 以下に収まるケースがほとんどです。無料枠内での利用も十分可能です。詳細は [Gemini API 料金ガイド](/articles/gemini-api/gemini-api-pricing-billing-complete-guide-2026) をご覧ください。

**Q. BigQuery や他のデータソースにも対応できますか？**

はい。Apps Script の `fetchSalesData()` 関数を BigQuery API や他の REST API に差し替えることで対応できます。Vertex AI Gemini を利用すれば BigQuery との直接連携も可能です。さらに複雑なデータパイプラインには [Gemini API × BigQuery データ分析ガイド](/articles/gemini-dev/gemini-api-bigquery-ai-data-analysis) が参考になります。

**Q. 分析結果の精度を高めるには？**

プロンプトの改善が最も効果的です。業界固有の背景情報（「これは小売業の月次データです」）や、注目してほしいKPIを明示することで、より的確なコメントが生成されます。System Instructions に会社のビジネスコンテキストを設定する方法については、[Gemini API System Instructions 活用ガイド](/articles/gemini-api/gemini-25-pro-system-instructions-mastery-guide) も参考にしてください。
