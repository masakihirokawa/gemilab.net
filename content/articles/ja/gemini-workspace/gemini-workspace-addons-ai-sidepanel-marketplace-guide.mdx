...
highlights:
  ...
  - "サブスクリプション課金モデルの組み込みと収益最適化の実践的ノウハウが得られるを実践的に習得できる"
---

## はじめに — なぜ Workspace Add-ons × Gemini なのか
...
本記事では、Google Workspace Add-ons の基本アーキテクチャから始め、Gemini API を統合したカスタムAIサイドパネルの開発、OAuth スコープ設計、セキュリティレビュー対応、そして Google Workspace Marketplace での公開と収益化までを体系的に解説します。実際に動作するコードを豊富に提供しますので、読み終わる頃にはご自身のアドオンを公開できる状態になっているはずです。
...
### Q1: Workspace Add-ons の開発に費用はかかりますか？

開発自体は無料です。Apps Script のクォータ内（1日の実行時間 6 時間、トリガー 20 個など）であれば追加料金はかかりません。ただし、Gemini API の利用料は別途発生します。Gemini 2.5 Flash は無料枠（1日1,500リクエスト）がありますので、個人利用やプロトタイプ段階ではコストを抑えられます。Marketplace に公開する場合、Google Cloud の認証費用は不要ですが、プライバシーポリシーのホスティング費用などは別途必要です。

### Q2: セキュリティレビューに落ちた場合、どうすればいいですか？

審査チームからのフィードバックが具体的に返ってきますので、指摘事項を一つずつ対応してください。最も多い不合格理由は「不要な OAuth スコープの要求」と「プライバシーポリシーの不備」です。スコープを見直し、`currentonly` で対応できるものは切り替えましょう。再提出は回数制限なく可能ですが、繰り返し不合格になると審査期間が長くなる傾向があります。初回申請の前に、Google の「OAuth アプリ検証 FAQ」を熟読することをお勧めします。

### Q3: 組織内だけに配布する場合も Marketplace 公開が必要ですか？

いいえ。Google Workspace の管理者が「内部アプリ」として組織内のみに配布する場合は、Marketplace の一般公開やセキュリティレビューは不要です。Apps Script エディタから「デプロイ」→「新しいデプロイ」→「アドオン」を選択し、「自分のドメイン内のユーザー」に限定して配布できます。組織外のユーザーにも提供する場合のみ、Marketplace への一般公開とセキュリティレビューが必要になります。

### Q4: Gemini API の代わりに Vertex AI を使うメリットはありますか？

企業向けのアドオンでは Vertex AI の利用を検討する価値があります。Vertex AI では、データの処理リージョンを指定できる（EU / US / Asia 等）ため、GDPR や社内のデータポリシーへの準拠が容易です。また、VPC Service Controls による ネットワーク境界制御、モデルのファインチューニング、Grounding with Google Search などの追加機能も利用できます。一方、API キーベースの Gemini API はセットアップが簡単なため、個人開発者やスタートアップには Gemini API から始めることをお勧めします。

## まとめ

本記事では、Google Workspace Add-ons と Gemini API を組み合わせたカスタム AI サイドパネルの開発から Marketplace 公開・収益化までを一通り解説しました。Card Service v2 による UI 構築、Gemini API の統合パターン、OAuth スコープ設計、セキュリティレビュー対策、そしてサブスクリプション課金モデルの実装まで、実際のコードを交えて体系的にカバーしています。
...
Gemini API を活用した収益化の全体像を知りたい方には「[Gemini 収益化マスタープラン 2026](https://www.amazon.co.jp/dp/B0D1LM1V1Y/?tag=pinocchio-22)」（AI ビジネス関連書籍）も参考になるでしょう。また、Google AI エコシステムでの収益の柱について体系的に学びたい方には「[Gemini 収益化マスタープラン 2026](/articles/gemini-api/gemini-monetization-masterplan-2026)」もおすすめです。
