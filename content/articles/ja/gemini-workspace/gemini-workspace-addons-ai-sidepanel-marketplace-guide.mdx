...
本記事では、Google Workspace Add-ons と Gemini API を組み合わせたカスタム AI サイドパネルの開発から Marketplace 公開・収益化までを一通り解説しましました。Card Service v2 による UI 構築、Gemini API の統合パターン、OAuth スコープ設計、セキュリティレビュー対策、そしてサブスクリプション課金モデルの実装まで、実際のコードを交えて体系的にカバーしています。
