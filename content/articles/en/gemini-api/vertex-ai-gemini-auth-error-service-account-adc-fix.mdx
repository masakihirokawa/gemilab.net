...
This guide walks you through the most common Vertex AI + Gemini authentication errors, what causes them, and exactly how to fix them.
...
**Q1: It works locally but fails on Cloud Run with `UNAUTHENTICATED`. What's going on?**

A: Locally, you're probably using your personal Google account's ADC. Cloud Run uses a service account instead. Check that a service account is attached to your Cloud Run service (via the `--service-account` flag) and that it has the `roles/aiplatform.user` role.

**Q2: I've set `GOOGLE_APPLICATION_CREDENTIALS` but I'm still getting `DefaultCredentialsError`.**

A: Make sure you're using an absolute path, not a relative one (like `./key.json`). Relative paths aren't reliably resolved in all environments. Also verify the JSON is valid: `python -c "import json; json.load(open('key.json'))"`.

**Q3: How do I manage credentials when working with multiple projects?**

A: You can call `vertexai.init(project="...", location="...")` per request, or create separate `credentials` objects per service account and pass them explicitly:
...
**Q4: What's the benefit of Workload Identity Federation?**

A: It allows non-Google Cloud environments like GitHub Actions or AWS to authenticate to Vertex AI without any service account keys. You get temporary, short-lived credentials scoped to specific workloads—eliminating the risk of key leakage entirely. It's the recommended approach for any external CI/CD pipeline.
