...
author: "Gemini Lab"
description: "A deep dive into designing event-driven asynchronous AI pipelines using Gemini API with Google Cloud Pub/Sub, webhooks, and Redis queues. Includes the design pitfalls and live cost/throughput numbers from running this stack across the four Dolice Labs sites and several iOS/Android apps."
...
def claim_task(task_id: str) -> bool:
    """Return True if we successfully claimed the task, False if it's already taken."""
    db = firestore.Client()
    task_ref = db.collection("tasks").document(task_id)
...
```python
# slack_dlq_notifier.py — Slack notification with a human gate for DLQ replay
import json
        ...
        blocks=[
            ...
            {"type": "actions", "elements": [
                {"type": "button", "action_id": "dlq_replay",
                 "text": {"type": "plain_text", "text": "✅ Replay"},
                 "style": "primary",
...
| --- | --- | --- |
| Gemini API calls per month | ~12,000 | 480 article gen, 7,500 integrity checks, 4,000 SEO helpers |
| Gemini 2.5 Pro / Flash ratio | ~25% / 75% | Pro for article bodies + highlights, Flash for extraction + diff verification |
| Monthly Gemini API cost | ~$42 | Down from $108 on Pro-heavy config — 61% saved by routing to Flash |
| Monthly Cloud Run cost | ~$5 | min=1, max=8, p95 CPU utilization 18% |
| Monthly Pub/Sub cost | < $0.40 | Around 40,000 messages |
| Failure → DLQ rate | 0.03% | 11 messages in the last 90 days |
| End-to-end p95 task latency | 38 seconds | During the 02:00–05:00 JST peak window |
...
For deeper coverage of related topics, see [Gemini 2.5 Flash API Errors — Troubleshooting Notes](/articles/gemini-api/gemini-25-flash-api-errors-troubleshooting) and the [Gemini API Cost Optimization — Practical Notes](/articles/gemini-api/gemini-api-cost-optimization-complete-guide).

To deepen your understanding of the theoretical foundations behind these patterns, [Designing Distributed Systems by Brendan Burns](https://www.amazon.co.jp/dp/4873118751/?tag=pinocchio-22) is an outstanding reference — written by a Google engineer, it covers the architectural primitives this article builds on.
