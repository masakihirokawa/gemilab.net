...
highlights:
  - "Fully understand how to implement async pipelines using Google Cloud Pub/Sub + Gemini API to handle high-volume requests reliably"
  - "Master scalable background processing design combining webhook callbacks with Redis queues"
  - "Learn production-grade error management with dead-letter queues, retry strategies, and distributed tracing"
---
...
**Q1: Should I choose Pub/Sub or a Redis queue?**

If you're already on Google Cloud, Pub/Sub is the natural fit — fully managed, highly available, and scales to millions of messages per second with zero ops overhead. For on-premise environments or other cloud providers, Redis with Celery or ARQ is easier to set up and offers low latency with fine-grained control. For workloads under tens of thousands of jobs per day, Redis is more than sufficient. At higher scale, Pub/Sub's managed guarantees become invaluable.

**Q2: Will messages be lost if Gemini API rate limits trigger?**

Not if you handle nacks correctly. When you nack a Pub/Sub message, it's redelivered after the acknowledgment deadline expires (default: 10 minutes). The key is to avoid immediate nack-then-requeue loops — always combine nacks with exponential backoff to avoid hammering a rate-limited API endpoint.

**Q3: How do I secure webhook endpoints against spoofing?**

HMAC-SHA256 signature verification is the baseline. On top of that, protect your endpoint with a WAF (Cloudflare or Cloud Armor) and enforce IP allowlisting for known sender addresses. Always validate incoming JSON schemas using Pydantic to prevent malformed payload injection.

**Q4: How do I determine the right number of workers for production?**

It depends on your Gemini API tier. Tier 1 (free/low-volume) limits you to 2–10 RPM, so 1–2 workers are typically sufficient. Tier 3+ (high-volume) allows 1,000+ RPM, enabling 20+ parallel workers. Start conservatively, monitor queue depth and p99 latency, then scale out methodically. Cloud Run's autoscaling handles burst demand well once you've set sensible min/max bounds.

**Q5: How do I debug issues in a distributed async pipeline?**

For local development, use the Pub/Sub emulator to run the same code as production without touching real infrastructure. In production, attach a trace ID to every message at creation time and propagate it through all processing stages. Cloud Logging + Cloud Trace together give you end-to-end visibility. Set up log-based alerts for DLQ entries and nack rate spikes to catch issues before they compound.

**Q6: How should I handle idempotency in my workers?**

Idempotency is critical in any at-least-once delivery system. Since Pub/Sub may redeliver the same message more than once, your worker must handle duplicates gracefully. The simplest approach is to use `task_id` as a unique key in your database. Before calling Gemini API, check if a result already exists for that `task_id` — if it does, skip processing and ack the message immediately. This "check-then-process" pattern prevents duplicate API calls and avoids charging you twice for the same content.

**Q7: How should large files (PDFs, videos) be passed through the pipeline?**

Never embed large binaries directly in Pub/Sub messages — there is a 10 MB size limit, and large payloads slow the broker significantly. Instead, store files in Cloud Storage and pass only the GCS URI in the message payload: `{"task_id": "...", "gcs_uri": "gs://my-bucket/file.pdf"}`. Workers fetch the file directly from GCS and pass it to Gemini via the File API. This keeps messages lightweight and makes the pipeline resilient to arbitrarily large input files.
