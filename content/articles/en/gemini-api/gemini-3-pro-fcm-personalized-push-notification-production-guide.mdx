...
---
Open rates on one of my apps sat at 0.8% and refused to move. Lowering the send frequency did nothing. Shifting the day of the week did nothing. Changing the delivery hour did nothing.

Then one message I had hand-written landed at 4.2%. Same feature being announced, same segment receiving it, different sentence. The variable was the copy itself, not the delivery schedule — and once generic copy saturates an audience, no amount of frequency tuning brings the numbers back.
...
There is one reason this cost structure holds: Gemini is only doing copy generation. Segment extraction, send-time selection, and delivery itself all stay in ordinary rule-based code and managed services.

Narrowing the model's job to a single step drops the bill by an order of magnitude and makes incidents trivial to isolate. If generation fails, you fall back to the static string and the send still goes out. Hand segment logic to an LLM as well and you lose both the price and the ability to answer "why didn't this user get it?"
...
## Frequency caps — protect permission, not just open rate

Once personalization starts working, the copy gets good enough that you start thinking "maybe I can send a little more often." That is the most dangerous moment in this whole system. A dip in open rate is recoverable. A user who turns notifications off is not — no amount of prompt tuning reaches someone who has revoked permission.

So I keep the frequency ceiling as a separate guard, outside the send-decision logic. The point is to put it somewhere that editing prompts or segment definitions cannot accidentally break.

```python
from datetime import datetime, timedelta

# Per-segment minimum gap (days) and rolling 14-day ceiling
FREQUENCY_CAP = {
...
def allowed_to_send(user_doc: dict, segment: str, now: datetime) -> bool:
    """Evaluate before calling Gemini so skipped users cost nothing to generate."""
    cap = FREQUENCY_CAP[segment]
    history = user_doc.get("notification_history", [])  # list of ISO8601 strings
    if not history:
        return True
...
Running this check **before** the Gemini call is the part that matters operationally. Generate first and decide not to send afterwards, and you have paid for tokens nobody will ever read. On my sends, 12–18% of users are filtered out by this single function on any given run, and that generation cost simply disappears.

I append to `notification_history` on successful delivery and trim it to a fixed length of 15 entries, dropping the oldest. That keeps Firestore document sizes predictable and is still more than enough history for a 14-day rolling window.

One counterintuitive detail: the dormant segments get the *longest* minimum interval, not the shortest. Cross-referencing the dates when tokens went `UNREGISTERED` against the sends that preceded them, the users who had been away longest were the quickest to turn notifications off. The people you most want back are the ones you should contact least often. It reads backwards, but the data is consistent about it.
...
For deeper dives, the [Gemini API × Cloud Tasks Async Job Queue Production Guide](/articles/gemini-dev/gemini-api-cloud-tasks-async-ai-job-queue-production-guide) and the [Gemini API Rate Limiting & Quota Management Production Guide](/articles/gemini-api/gemini-api-rate-limiting-quota-management-production-guide) pair well with this architecture.
