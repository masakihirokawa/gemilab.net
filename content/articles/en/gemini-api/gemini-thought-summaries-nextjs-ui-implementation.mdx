...
---
After six months of running Gemini in production apps, I keep hearing the same question from users: "Why did it answer that way?" Saying "because the model decided so" erodes trust, whether you are an indie developer shipping alone or a team running B2B SaaS. Support tickets go up, refund requests go up, and the lifetime value of your customers gradually drifts down. Retrieval citations and source links help, but they only show what the model was given, not how it reasoned. For tasks that do not touch external information at all — summarization, classification, draft generation, comparative judgement — you cannot even offer citations, so the transparency gap is even wider.
...
The `thinking_budget` value of `-1` (auto) is fine in almost all real-world scenarios. If you do want to set it explicitly, a lower bound of 128 on Pro and 64 on Flash tends to keep the summaries from shrinking to a useless trickle. If you need an upper bound for cost control near end-of-month, capping at 512–1024 avoids runaway spending while still giving you usable reasoning. For the deeper story on model choice between Pro and Flash, [Gemini 2.5 Pro thinking budget reasoning control guide](/articles/gemini-advanced/gemini-25-pro-thinking-budget-reasoning-control) will save you a lot of trial and error.

Model choice is not only about whether summaries appear at all — it is about the tokens and the wait they add. Here are the averages I measured by sending the same prompt ten times against each model. Prompt length moves these numbers around, so read them as orders of magnitude rather than benchmarks.

<table>
<thead>
<tr><th>Model</th><th>Thought summaries</th><th>Practical minimum thinking_budget</th><th>Output token increase</th><th>Added time to first chunk</th></tr>
</thead>
<tbody>
<tr><td><strong>gemini-2.5-pro</strong></td><td>Supported</td><td>128</td><td>~1.6x</td><td>~+15%</td></tr>
<tr><td><strong>gemini-2.5-flash</strong></td><td>Supported</td><td>64</td><td>~1.3x</td><td>~+8%</td></tr>
<tr><td><strong>gemini-2.5-flash-lite</strong></td><td>Not supported (returns an empty array)</td><td>—</td><td>—</td><td>—</td></tr>
</tbody>
</table>

Seeing the numbers side by side makes the "always on or per-screen" decision concrete. I settled on Pro for screens where the user makes a judgement call, and Flash for list summaries and draft generation.
...
## Wiring cancellation, plan gating, and persistence into one path

I deferred three things above: request cancellation, plan-based gating, and persisting thoughts. They all converge on the same Route Handler, so here is the version I actually run in production.

```typescript
// app/api/ask/route.ts (with cancellation, plan gating, and persistence)
import { GoogleGenAI } from "@google/genai";
...
export async function POST(req: Request) {
  const { question, plan, conversationId } =
    (await req.json()) as { question: string; plan: Plan; conversationId: string };

  // Decide server-side whether reasoning is visible. If you trust a client flag,
  // free-tier users will happily flip it and read the thoughts anyway.
  const includeThoughts = plan === "pro";
    ...
    async start(controller) {
      ...
      try {
        chunks = await ai.models.generateContentStream({
          model: includeThoughts ? "gemini-2.5-pro" : "gemini-2.5-flash",
          contents: question,
          config: {
            thinkingConfig: { includeThoughts, thinkingBudget: -1 },
            abortSignal: req.signal, // propagate cancellation into the SDK
          },
        });

        for await (const chunk of chunks) {
          // The user hit back or closed the tab: stop burning tokens.
        ...
        // Save whatever we produced, even on an aborted request. Without this,
        // the history of users who navigated away has silent holes in it.
        if (answer || thoughts.length > 0) {
          await saveMessage({
            conversationId,
            role: "assistant",
            content: answer,
            metadata: { thoughts, aborted: req.signal.aborted },
          });
        }
        controller.close();
      }
...
The `finally` block is the part I would highlight. I originally saved right after a successful stream, and the result was that conversations from users who left mid-answer were simply missing from the history — precisely the sessions I needed when someone filed a support ticket. A partial record at least tells you where things stopped.

On the client, pass an `AbortController` signal into `fetch` and abort on both unmount and an explicit stop button.

```tsx
// app/ask/useAsk.ts (excerpt)
const controllerRef = useRef<AbortController | null>(null);

async function ask(question: string, plan: "free" | "pro") {
  controllerRef.current?.abort();          // collapse the previous request on rapid clicks
  ...
  const res = await fetch("/api/ask", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ question, plan, conversationId }),
    signal: controller.signal,
  });
  // ...SSE reading continues as shown earlier
}

useEffect(() => () => controllerRef.current?.abort(), []); // stop generation when the screen unmounts
```

The storage schema only needs to keep the answer and the reasoning in separate places. In PostgreSQL:

```sql
-- Keep thoughts inside metadata and out of the full-text index on the answer
create table messages (
  id           uuid primary key default gen_random_uuid(),
  conversation_id uuid not null references conversations(id) on delete cascade,
  role         text not null check (role in ('user', 'assistant')),
  content      text not null,
  metadata     jsonb not null default '{}'::jsonb,  -- { thoughts: string[], aborted: bool }
  created_at   timestamptz not null default now()
);

create index messages_conversation_idx on messages (conversation_id, created_at desc);
-- Index the answer only; mixing reasoning into search adds noise
create index messages_content_fts on messages using gin (to_tsvector('simple', content));
```

Keeping thoughts inside `metadata` means requests like "purge all reasoning older than 90 days" or "return thoughts only for paid accounts" become a query change instead of a migration. The longer an app runs, the more that one layer of indirection pays for itself.
...
Laid out side by side, it becomes easier to see which one to act on first.

<table>
<thead>
<tr><th>Metric</th><th>How to collect it</th><th>What I see in my own apps</th><th>What to do when it falls short</th></tr>
</thead>
<tbody>
<tr><td><strong>Thought-expansion rate</strong></td><td>Expand events divided by answer-render events</td><td>~35% high-stakes / ~8% low-stakes</td><td>Below 5% on a screen: turn thoughts off there and reclaim the tokens</td></tr>
<tr><td><strong>Acceptance lift</strong></td><td>Copy/save rate for expanders vs. non-expanders</td><td>A 10-point gap or more means it is working</td><td>No gap: revise the granularity or the wording of the summaries</td></tr>
<tr><td><strong>"Why did it say this?" tickets</strong></td><td>Support tag counts compared quarter over quarter</td><td>30–40% fewer in the launch quarter</td><td>No change: the answer body itself is under-explaining</td></tr>
<tr><td><strong>Reasoning stability</strong></td><td>Same prompt five times weekly, eyeball agreement</td><td>Key points agree in at least 3 of 5</td><td>High variance: pin thinking_budget explicitly instead of -1</td></tr>
</tbody>
</table>
