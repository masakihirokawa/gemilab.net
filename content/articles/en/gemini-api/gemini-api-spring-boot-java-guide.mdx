...
If you're brand new to the Gemini API, start with the [Gemini API Quickstart guide](/en/articles/gemini-api/gemini-api-quickstart) to get your API key set up first.
...
To master system instruction design for production applications, the [Gemini 2.5 Pro System Instructions Mastery Guide](/en/articles/gemini-api/gemini-25-pro-system-instructions-mastery-guide) covers advanced patterns in depth.
...
**Q. Is it safe to add spring-boot-starter-webflux to an existing Spring MVC app?**

A. Yes. WebClient is usable independently of the full reactive stack. Adding `spring-boot-starter-webflux` to a servlet-based MVC app solely to use WebClient for outbound HTTP calls is a common and supported pattern. You won't break your existing controllers.

**Q. How do I manage the API key securely in production?**

A. Reference it as an environment variable in `application.properties` using `${GEMINI_API_KEY}`. Spring Boot will automatically resolve it at startup. For Kubernetes, use Secrets; for cloud deployments, use the platform's secret management service (e.g., AWS Secrets Manager, GCP Secret Manager).

**Q. How do I switch models (e.g., Gemini 3 Pro)?**

A. Simply change the `gemini.model` value in `application.properties` to `gemini-3-pro`, `gemini-3-1-flash`, or any supported model. The code needs no changes — that's the benefit of externalizing configuration.

**Q. Can I implement streaming responses (real-time character-by-character output)?**

A. Yes. Change the API endpoint from `:generateContent` to `:streamGenerateContent` and use WebClient's `bodyToFlux(String.class)` combined with Server-Sent Events (SSE) on the Spring side. This enables typing-effect UIs for chat interfaces.
...
For those who want to go deeper into AI development with Java, [*Spring AI in Action* by Mark Pollack](https://www.amazon.co.jp/dp/1633436985/?tag=pinocchio-22) is an excellent companion resource.
