...
### Q1: What's the difference between JSON Mode and Structured Output?

**JSON Mode** (`response_mime_type: "application/json"`) only guarantees the response is valid JSON. **Structured Output** (adding `response_schema`) constrains the JSON structure including field names, types, and required fields. For reliable data extraction, always use Structured Output. For a deeper dive, check out our [complete guide to Gemini API Structured Output](/en/articles/gemini-api/gemini-structured-output-advanced).

### Q2: How deeply nested can `response_schema` objects be?

Gemini 2.5 Pro and Flash handle up to about 5 levels of nesting reliably. For deeper structures, flatten your schema and reconstruct the nesting on the client side after parsing.

### Q3: Enum values are sometimes ignored. How can I fix this?

Combine the `enum` field with a clear `description` stating "Only these values are allowed." Then add client-side validation to detect and retry when invalid values slip through.

### Q4: Can I use non-English field names in schemas?

It's technically possible, but English field names tend to produce more accurate and consistent results. The recommended approach is English field names with localized values.

### Q5: Does JSON Mode work with streaming?

Yes, JSON Mode and streaming are compatible. However, individual chunks contain partial JSON, so you must concatenate all chunks before parsing. Never try to parse individual stream chunks as standalone JSON.
...
To build a solid foundation in API design and error handling, we recommend [Web API: The Good Parts by Takaki Mizuno](https://www.amazon.co.jp/dp/4873116864/?tag=pinocchio-22).
