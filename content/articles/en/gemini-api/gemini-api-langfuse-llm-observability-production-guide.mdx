...
## Cost attribution — assume the price table drifts

Langfuse ships default prices for the popular models, Gemini included, and those defaults are maintained in the open-source repo with a **daily automated audit** that checks official provider sources for changed prices, tiers, and newly released models. So the old advice — "register every new model yourself" — is no longer accurate.

Two properties downstream of that are what actually bite.

**Cost is computed at ingestion time.** Langfuse looks up the price the moment it receives the trace, calculates the number, and stores it. Editing a model definition afterwards does **not** recompute traces that are already stored. I had this backwards for a long stretch: I assumed registering a new model's price would backfill last month. It doesn't. I only reread the docs after counting the rows that stayed empty.

**Price changes have effective dates.** A price table gets replaced on each revision, but a bill has a "starting when." A pipeline sized around a promotional rate comes back at double the cost the month after the cutover — and the more you trust the table, the less likely you are to notice. When I write a price into a design doc now, I write down how long that price is good for, right beside it.

**A price table is a copy of the truth, not the truth.** Once a month I put the Langfuse total next to the provider's invoice and look at the gap. If the gap grows past a threshold, the suspect is either the model definition or the token accounting covered in the next section.

### Models that change price above 200K need pricing tiers

A model like Gemini 2.5 Pro, whose rate depends on input size, can't be expressed with a single price. Langfuse supports **pricing tiers**: one model definition can carry several price points, each with a name, a priority, conditions, and prices. Tiers are evaluated in priority order and the first one whose conditions match wins; if none match, the default tier applies.

Conditions can draw on three sources:

<table>
<thead>
<tr><th>Source</th><th>How it matches</th><th>Where it helps with Gemini</th></tr>
</thead>
<tbody>
<tr><td><strong>Usage details</strong></td><td>Match usage keys with a regex, sum the values, compare with <code>gt</code> / <code>gte</code> / <code>lt</code> / <code>lte</code> / <code>eq</code> / <code>neq</code></td><td>Switch to long-context pricing above 200,000 input tokens</td></tr>
<tr><td><strong>Model parameters</strong></td><td>Match a top-level model-parameter key against exact values</td><td>Request attributes that carry a different rate</td></tr>
<tr><td><strong>Metadata</strong></td><td>Match a top-level metadata key against exact values</td><td>Split rates by <code>region</code> or your own billing bucket</td></tr>
</tbody>
</table>

Nested paths aren't supported, and when a tier carries several conditions, all of them have to match. Spending one careful hour here is what keeps a long-document feature and a short-chat feature from smearing into each other on the same dashboard.

With that in place, **feature-level cost slicing** becomes the view you return to daily. If you've been tagging `metadata.feature`, you'll see "article_summary accounts for 38% of spend" or "code_review is 17%" — decisions that were guesses a week ago are now reports.
...
## Translating Gemini's token counts into Langfuse's exclusive buckets

When your cost comes out higher than the provider's invoice, suspect the **token accounting** before the prices. This is where Langfuse and Gemini quietly disagree, and I walked into it myself.

Langfuse treats every key in `usage_details` as a **non-overlapping bucket**. `input` excludes any `input_*` value such as `input_cached_tokens`; `output` excludes any `output_*` value; `total` is the sum of the buckets rather than a bucket itself. Hand it overlapping numbers and both usage and inferred cost get counted twice.

Gemini's `usage_metadata` is **inclusive**. `prompt_token_count` is the whole input, cache reads included. `cached_content_token_count` tells you how much of that came from cache. Thinking tokens arrive separately as `thoughts_token_count`.

Put `prompt_token_count` straight into `input` and every cache-heavy call reads higher than your bill: the cached portion is charged at a discount, but you've counted it at full input rate and then added it a second time. In my case, only the batch job that attaches a long document on every call looked strangely expensive, and I spent half a day blaming the price table. The price table was fine. I had skipped a subtraction.

So I convert inclusive counts to exclusive ones before anything reaches Langfuse, and I keep that in a single function so no caller touches `usage_metadata` directly.
...
def to_langfuse_usage(um: Any) -> dict[str, int]:
    """Convert Gemini's inclusive usage_metadata into Langfuse's exclusive buckets.

    prompt_token_count includes cache reads, so always subtract them.
    """
    def n(name: str) -> int:
        return int(getattr(um, name, 0) or 0)

    prompt = n("prompt_token_count")
    cached = n("cached_content_token_count")
    total = n("total_token_count")

    usage = {
        "input": max(prompt - cached, 0),          # fresh input only
        "input_cached_tokens": cached,             # cache reads
        "output": n("candidates_token_count"),
        "output_reasoning_tokens": n("thoughts_token_count"),
    }

    # The day this drifts is the day the provider's breakdown changed
    drift = total - sum(usage.values()) if total else 0
    if drift > 0:
        usage["untracked"] = drift             # unaccounted tokens: a new field appeared
    elif drift < 0:
        logger.warning("usage buckets overlap: drift=%d", drift)  # double counting
    return usage
```

Those last few lines do the most work. Providers add breakdown fields, and when they do, the numbers shift without saying anything. Parking the difference in a bucket named `untracked` means the day that bucket appears on your dashboard is the day the contract changed. A negative drift points the other way — overlapping buckets — so that one goes to a warning log instead of into the cost.

Callers just pass the dict through:
...
with langfuse.start_as_current_observation(
    ...
    gen.update(
        output=resp.text,
        usage_details=to_langfuse_usage(resp.usage_metadata),
    )
```

Usage keys have to match the price keys on the model definition **exactly**. If there's no price on `input_cached_tokens`, that bucket bills at zero — and a zero can mean "free" or "you forgot," which are very different things. Whenever I add a bucket, I open the model definition in the same sitting.

One more: for models that return thinking tokens, Langfuse can't infer cost unless you ingest the counts. The docs say as much for reasoning models. Passing `usage_details` explicitly, rather than leaning on auto-instrumentation, is what saves you at month end.
...
**Pitfall 3 — traces that ship before a price exists never get fixed.** If a brand-new Gemini model reaches production before it lands in the default price table, those traces keep a `—` forever: registering the price later does **not** recompute them. Your monthly report quietly understates spend. On any day you promote a new model ID, either land the model definition first or switch that path to ingesting `cost_details` explicitly.
...
export async function summarize(
  ...
  generation.end({
    output: out,
    usage: {
      // promptTokenCount includes cache reads — subtract before sending
...
The snippet above is the v3-era JS SDK. In v4 this moved to `startObservation` from `@langfuse/tracing`, and the usage key changed from `usage` to `usageDetails`. Miss the rename during an upgrade and traces keep arriving while the cost column silently empties out. On any release where I bump the SDK, I check that column the next day.
...
**Week 2 — cost visibility.** Settle the model definitions and pricing tiers, then build the feature-level cost dashboard. Expect to find at least one feature that's spending far more than you assumed.
