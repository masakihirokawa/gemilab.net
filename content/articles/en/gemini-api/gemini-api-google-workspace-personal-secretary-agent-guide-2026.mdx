...
On the eighth morning, the only thing waiting in my terminal was a single line: `invalid_grant`. The daily summary that had arrived on time every day until then simply stopped. I hadn't touched a character of the code. What had changed was the one thing I'd never looked at — my OAuth consent screen was still set to "Testing".

The hard part of a secretary AI that spans Gmail and Calendar turned out not to be Gemini Function Calling. It was everything around it: how long the auth actually lives, how finely to slice scopes, and where to hand control back to a human before a write tool fires.

What follows is the implementation I've been running once every morning for about ninety days, along with the mistakes I made getting there. I've tried to leave in not just the working code but the reason each piece has to be written that way if you want it to survive production.
...
### A "Testing" Consent Screen Expires Your Refresh Token in Seven Days

That `invalid_grant` from the opening came from here.

Your OAuth consent screen in Google Cloud has a publishing status. While it sits at "Testing" with an external user type, a test user's grant expires seven days after consent — and the refresh token you received with `access_type="offline"` dies on the same clock. You never notice during development, because you re-authorize constantly. You notice on day eight, right when things have gotten stable enough to leave alone.

**Auth isn't a step you complete once. It's a promise with an expiry date.** I worked that out after three mornings without a summary.

<table>
  <thead>
    <tr><th>Publishing status</th><th>Refresh token lifetime</th><th>When it fits</th></tr>
  </thead>
  <tbody>
    <tr><td>Testing</td><td>Expires <strong>7 days</strong> after consent; renewed only by re-authorizing</td><td>While you're still shaping scopes</td></tr>
    <tr><td>In production</td><td>Does not expire (barring revocation, long disuse, or a password change)</td><td>Anything on a daily cron</td></tr>
  </tbody>
</table>

The scopes a secretary AI needs — `gmail.readonly`, `gmail.compose` — are restricted scopes, so moving to production with an external user type puts an unverified-app warning in front of the consent flow. You can click through it for your own account; you need the verification process once you hand the app to anyone else. If you have a Google Workspace account, setting the user type to Internal was the quietest way out for me. The primary sources are [Google's OAuth 2.0 documentation](https://developers.google.com/identity/protocols/oauth2) and the [app publishing status overview](https://developers.google.com/identity/protocols/oauth2/production-readiness/overview).

Beyond fixing the status, I'd rather learn about an expiry the moment it happens than three days later. A small gate at the top of the cron job does that: try the refresh once, and if it fails, notify instead of proceeding.

```python
# preflight.py — verify auth before the real work starts
from datetime import datetime, timezone
from google.auth.exceptions import RefreshError
from google.auth.transport.requests import Request

from workspace_auth import get_credentials


class ReauthRequired(Exception):
    """Auth needs human re-consent. Notify instead of running the job."""


def preflight(notify) -> "Credentials":
    """
    Check auth once, before any Gemini call.
    Failing here avoids the worse ordering: pay for inference, then die on auth.
    Pass a notify callable that reaches somewhere you actually read in the morning.
    """
    creds = get_credentials()
    ...
    if creds.expired and creds.refresh_token:
        try:
            ...
            # invalid_grant lands here: revocation, the 7-day expiry, or a password change
            notify(f"[secretary] re-authorization required: {e}")
            raise ReauthRequired(str(e)) from e

    # Speak up just before a silent failure rather than after it
    if creds.expiry:
        remaining = creds.expiry.replace(tzinfo=timezone.utc) - datetime.now(timezone.utc)
        if remaining.total_seconds() < 300:
            notify("[secretary] access token has under five minutes left")

    return creds


if __name__ == "__main__":
    preflight(notify=print)
```

I chain it as `python preflight.py && python morning_brief.py`. On a morning where auth is broken, the job ends without calling Gemini even once, so there's no stray spend to explain later.
