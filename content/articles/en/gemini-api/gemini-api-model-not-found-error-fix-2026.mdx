...
This guide walks you through every common cause of "model not found" errors on `generativelanguage.googleapis.com`, the correct model names to use today, and the exact code changes you need to make.
...
**Q: My app worked fine last week, but now I'm getting "model not found" out of nowhere. What happened?**

A: Google likely deprecated the model version you were using. This is especially common with preview models (e.g., `gemini-2.5-pro-preview-03-25`) — they have expiration dates and get replaced by newer previews or a stable `-latest` alias. Switching to `gemini-2.5-pro-latest` usually resolves it immediately.

**Q: Do I need to include the `models/` prefix?**

A: With the Python SDK, no — the SDK adds it for you automatically. Just write `"gemini-2.5-pro-latest"`. When calling the REST API directly, you do need the full path in the URL: `models/gemini-2.5-pro-latest`.

**Q: Can I use `gemini-2.5-pro-latest` on the free tier?**

A: Yes, the Free Tier supports `gemini-2.5-pro-latest`, but with rate limits (requests per minute and daily token caps). If you start hitting those limits, check out [Gemini API Quota and 429 Rate Limit Errors: How to Fix Them](/en/articles/gemini-api/gemini-api-quota-429-error-troubleshooting).

**Q: Should I hard-code the model name or use an environment variable?**

A: For any serious project, an environment variable is the better choice. It lets you update the model without touching your code:

```python
import os

# Falls back to gemini-2.5-pro-latest if the env var isn't set
...
For a deeper dive into building production-ready apps with the Gemini API, [「Google Gemini APIではじめる生成AIアプリ開発」by 我妻幸長](https://www.amazon.co.jp/dp/4295018686/?tag=pinocchio-22) is an excellent resource covering both the fundamentals and advanced integration patterns.
