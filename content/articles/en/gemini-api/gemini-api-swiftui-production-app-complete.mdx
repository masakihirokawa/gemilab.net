...
---
The review result came back one morning, and the reason listed wasn't a bug in the code — it was a single line in the privacy label. That's a kind of rejection the simulator never shows you.

What runs cleanly on your desk tends to come apart the moment it meets other people's devices and a reviewer's checklist. A stream that freezes without raising anything. A response that never produces a single character, but only on 4G. A bill that spikes on exactly the requests that carried an image. I couldn't have anticipated any of them from the documentation alone.

So rather than reaching for the Firebase AI Logic SDK, this piece builds directly on `URLSession` and Swift's `async/await`, and records where I actually got stuck — streaming, multimodal input, caching, testing, App Store review — along with the code I ended up keeping.
...
For apps distributed publicly at scale, consider a backend proxy architecture where the API key lives exclusively on your server. See our guide on [Production Security Design for the Gemini API](/en/articles/gemini-api/gemini-api-production-security) for full implementation guidance.
...
Beyond code-level optimizations, a few architectural choices have outsized impact on token costs. Defaulting to `gemini-2.5-flash` instead of `gemini-2.5-pro` keeps most of the capability at roughly a quarter of the cost — the [official pricing page](https://ai.google.dev/gemini-api/docs/pricing) lists $0.30 input / $2.50 output for Flash against $1.25 / $10.00 for Pro — so I reserve Pro for the cases where the quality difference genuinely shows. Setting `maxOutputTokens` to a value appropriate for your feature (128 for a quick classification, 512 for a short summary, 2048 for a detailed explanation) prevents the model from generating unnecessarily verbose responses. And the caching layer described in the previous section remains the single highest-leverage optimization for apps with repetitive query patterns.

## What the Docs Don't Tell You: Lessons from Production

The code so far runs cleanly on the simulator and over stable Wi-Fi. Ship it to real users, though, and a few traps surface that you can't learn from the documentation alone. Here are three I picked up while wiring Gemini into apps I've run for years as an indie developer — drawn from Crashlytics reports and App Store Connect review notes.
...
## Retirement Dates Arrive Faster Than Your Release Cycle

I stopped scrolling the deprecation table when I saw the shutdown date next to `gemini-2.5-flash-image`: **October 2, 2026**. The build that shipped with image work in it went to the store long before that date.

On the web, this is a one-line change and a redeploy. On iOS it isn't. As long as the model name is compiled into the binary, replacing it means another review cycle — and more importantly, **the old string stays on the devices of everyone who doesn't update**. The day the endpoint goes away, what those users get is a 404 and an error screen that explains nothing.

Pulling only the rows that matter to someone who ships a binary, [the official deprecations page](https://ai.google.dev/gemini-api/docs/deprecations) (updated September 17, 2026) reads like this:

<table>
  <thead>
    <tr><th>Model</th><th>Shutdown date</th><th>Recommended replacement</th><th>What it means if you ship apps</th></tr>
  </thead>
  <tbody>
    <tr><td><code>gemini-2.5-flash</code></td><td>None announced</td><td>—</td><td>Safe to keep compiled in, for now</td></tr>
    <tr><td><code>gemini-2.5-pro</code></td><td>None announced</td><td>—</td><td>Same</td></tr>
    <tr><td><code>gemini-2.5-flash-image</code></td><td>October 2, 2026</td><td><code>gemini-3.1-flash-image-preview</code></td><td>Any shipped build doing image work needs a swap</td></tr>
    <tr><td><code>gemini-3.1-flash-lite</code></td><td>May 7, 2027</td><td><code>gemini-3.5-flash-lite</code></td><td>Over a year out — but the date is <strong>already published</strong></td></tr>
    <tr><td><code>gemini-2.0-flash</code></td><td>June 1, 2026 (already shut down)</td><td><code>gemini-3.6-flash</code></td><td>Older builds carrying this string no longer answer at all</td></tr>
  </tbody>
</table>

The same page is careful to say these are the *earliest possible* dates, and that the actual shutdown comes with advance notice; preview models carry a two-week notice as a guideline. That notice, though, reaches the developer. Whether the app gets updated is decided by the person holding the phone, which is a different question entirely.

**Put the model name on a server, not in the binary.** Decide that one thing early and the day a retirement notice lands, the work is editing a JSON file rather than queueing for review.

```swift
// ModelCatalog.swift — peel the model name off the binary
struct ModelCatalog: Codable {
    /// Ordered candidates. The order itself is a record of a decision.
    let chat: [String]
    let vision: [String]
    let revision: Int
}
...
actor ModelResolver {
    /// Shipped defaults — the last line of defence when the server is unreachable
    ...
    private var retired: Set<String> = []          // names that returned 404 this session
    private let catalogURL = URL(string: "https://example.com/ios/model-catalog.json")!

    /// Called once at launch. Failure is fine — we keep the built-in values.
    func refresh() async {
        var request = URLRequest(url: catalogURL)
        request.timeoutInterval = 3          // never hold up app start
        request.cachePolicy = .reloadRevalidatingCacheData   // let ETag do the work
    ...
    /// The name to use right now, skipping anything known to be gone
    ...
    /// A 404 / NOT_FOUND means the name expired, not that the network hiccuped
...
The call site treats 404 as an expired name rather than a transient failure, drops one step down the list, and retries once. Exponential backoff is the wrong tool here: a deleted endpoint does not come back.

```swift
// GeminiClient+Fallback.swift — don't retry into a name that no longer exists
func send(_ prompt: String, task: ModelTask = .chat) async throws -> String {
    ...
    do {
        ...
        return try await rawSend(prompt, model: next)   // exactly one retry
    }
}
```

I deliberately did not make the list "always newest." The [pricing page](https://ai.google.dev/gemini-api/docs/pricing) puts `gemini-3.8-flash` at $0.75 input / $3.75 output per million tokens (an introductory rate through December 31, 2026; $1.50 / $7.50 from January 1, 2027), against $0.30 / $2.50 for `gemini-2.5-flash` — 2.5× on input. For a chat surface built out of many short round trips, that difference lands directly on the monthly invoice. A newer generation earns its price on long-horizon work and complicated instructions, which may not be the screen where you simply want the first token to appear quickly.

So the catalog is an ordering I chose, not a subscription to the latest release. When a retirement notice arrives I reorder the list; when pricing changes I reorder it again. The judgement stays with me — the deployment doesn't.

Looking back, I wish I'd built this into the very first version. What I didn't see until I was staring at an actual shutdown date was that changing one string carries a review queue behind it, plus everyone who never updates.

## Where I'd Start Next

If you change one thing today, I'd skip the new library and the `Info.plist` and instead **collect every hardcoded model name into a single place**. Even extracting one constant makes it obvious what you'll touch the day a notice arrives.

Moving it to a server can come later. That was the order I took, too.
