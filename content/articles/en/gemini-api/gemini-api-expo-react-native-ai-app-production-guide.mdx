...
author: "Gemini Lab"
description: "An implementation record of shipping an Expo + Gemini API chat app through both store reviews — measured streaming latency, the chunk-boundary JSON bug, image payload limits, cost ceilings, and the pre-release checklist I run."
...
<table>
  <thead>
    <tr><th>Approach</th><th>Time to first character</th><th>Time to full response</th><th>Implementation cost</th></tr>
  </thead>
  <tbody>
    <tr><td>Single request (<code>generateContent</code>)</td><td>— (blank until 4,820 ms)</td><td>4,820 ms</td><td>Low</td></tr>
    <tr><td><code>streamGenerateContent</code> + manual chunk parsing</td><td>640 ms</td><td>5,110 ms</td><td>Medium</td></tr>
    <tr><td><code>expo/fetch</code> streaming API</td><td>610 ms</td><td>5,040 ms</td><td>Medium</td></tr>
  </tbody>
</table>

Notice that <strong>streaming is 200–300 ms slower to finish</strong>. On total processing time, streaming loses.
...
```typescript
// ❌ Before: assumes every chunk is a complete JSON object
for await (const chunk of stream) {
  const json = JSON.parse(chunk); // throws intermittently
...
```typescript
// ✅ After: keep the incomplete fragment in the buffer
let buffer = "";
...
**Don't leave reasoning depth at its default.** On models that expose a thinking budget, the default is often more than a chat interface needs. Capping it visibly lowers the bill — the implementation is in [controlling thinking_budget to keep costs down](/articles/gemini-api/gemini-25-pro-thinking-budget-cost-control-guide).
...
1. No API key embedded in the client (`EXPO_PUBLIC_` variables ship inside the bundle)
2. Launch in airplane mode — an error surfaces, nothing crashes
3. Navigating away mid-response aborts the stream (`AbortController` actually wired up)
4. Pasting a 5,000-character passage doesn't freeze the UI
5. Hitting the daily free limit shows the right message and the right next step
6. Image input without `base64: true` takes a handled branch, not an exception
7. Privacy policy states that input is processed on Google's servers
8. Token usage is being logged from day one
