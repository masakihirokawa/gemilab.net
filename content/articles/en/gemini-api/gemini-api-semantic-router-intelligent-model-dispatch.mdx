...
## A Router With Hardcoded Model Names Breaks Its Own Premise

(Updated July 2026.) When this article was written in April, the route definitions carried `gemini-2.5-flash` and `gemini-2.5-pro` as literal strings. As of July, the newest generally available Flash model is `gemini-3.5-flash`, and the `gemini-flash-latest` alias now resolves to it.

That deserves a pause. `gemini-3.5-flash` is priced at $1.50/1M input and $9.00/1M output. The `gemini-2.5-pro` referenced earlier in this article is $1.25/1M input and $10.00/1M output. The newest Flash sits in essentially the same price band as the previous generation's Pro.

In other words, "Flash is the cheap one" stops being true the moment you cross a generation boundary.
...
<tr><td>gemini-3.5-flash</td><td>$1.50</td><td>$9.00</td></tr>
</tbody>
</table>

If you "just upgrade Flash to the latest," the router keeps working perfectly and the savings quietly disappear. I spent a confused afternoon on exactly this: the routing-ratio dashboard was unchanged, and the bill had crept back up anyway.

The value of a router was never "send traffic to the cheap model." It's "pick the smallest model the task actually needs." When a generation changes, the savings estimate has to be recomputed from scratch.

### Separate Role Names From Concrete Model IDs

The fix is unglamorous: route definitions carry only a role name, and one place resolves that role to a concrete model ID plus its pricing.
...
class ModelSpec:
    model_id: str        # The ID sent to the API. Always pinned, never an alias.
    input_per_1m: float  # USD
    output_per_1m: float

# On a generation change, this is the only block you touch.
MODEL_REGISTRY: dict[str, ModelSpec] = {
    "fast":  ModelSpec("gemini-2.5-flash", 0.15, 0.60),
    "smart": ModelSpec("gemini-2.5-pro",   1.25, 10.00),
}
...
def resolve(tier: str) -> ModelSpec:
    try:
        ...
        # Unknown tiers always fail upward, toward the stronger model.
        return MODEL_REGISTRY["smart"]

def estimate_cost(tier: str, in_tokens: int, out_tokens: int) -> float:
    spec = resolve(tier)
    return (in_tokens * spec.input_per_1m + out_tokens * spec.output_per_1m) / 1_000_000
```

Route definitions now hold `"tier": "fast"` instead of `"model": "gemini-2.5-flash"`. Swapping a model becomes a three-line edit to `MODEL_REGISTRY`, and the class of bug where your cost-estimation code is still multiplying by last year's price simply stops existing.

### Don't Put `gemini-flash-latest` in Production Route Definitions

Aliases are convenient, but they hand the upgrade timing to someone else. For a router specifically, a model swap is an event where the pricing table and the route embeddings both go stale at the same moment. New prices invalidate your savings math; a different response profile invalidates your low-confidence threshold.

I keep latest-style aliases only in the staging `MODEL_REGISTRY` and pin versioned IDs in production. Staging runs for one to two weeks, I diff the routing logs for Flash ratio and repeat-question rate, and only then does the production `model_id` move. Since adopting that sequence, no generation change has broken the cost premise underneath the router.

Mixing generations is also a legitimate strategy. Keep simple classification and short translations on `gemini-2.5-flash`, and promote only mid-complexity reasoning to `gemini-3.5-flash`. Expanding role names to `fast` / `mid` / `smart` is a single added line in `MODEL_REGISTRY`. Being able to add a tier without touching route definitions turned out to be the largest payoff from this separation.
...
The discipline here is simple: the closer a change sits to revenue, the more it needs to be both reversible and observable. A router tempts you to flip everything at once. Resisting that urge is what determines how large the eventual incident is.
...
What I keep relearning is that the design choices you make in the first week show up in the P&L a year later. Pay-per-token APIs amplify that pattern: a fallback line of code, a monthly embedding refresh—these small, repeatable habits separate sustainable margins from quietly draining ones. Boring consistency beats clever optimization here.
