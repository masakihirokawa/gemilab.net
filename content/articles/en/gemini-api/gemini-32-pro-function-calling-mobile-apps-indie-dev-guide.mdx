---
title: "Integrating Gemini 3.2 Pro Function Calling into iOS/Android Apps: Production Design Patterns"
...
author: "Gemini Lab"
description: "A practical guide to integrating Gemini 3.2 Pro Function Calling into iOS and Android apps. Includes working SwiftUI, Kotlin, and Python code, plus production patterns proven in a real indie wallpaper app — cost, latency, staged rollout, and regression testing."
...
For foundational background on Function Calling mechanics, see the [Gemini API Function Calling Complete Guide](/articles/gemini-api/gemini-api-function-calling-complete).
...
For async usage patterns: [Gemini API asyncio production patterns](/articles/gemini-api/gemini-api-asyncio-production-patterns-indie-dev).
...
For cost monitoring and optimization patterns: [Gemini API Cost Optimization Guide](/articles/gemini-api/gemini-api-cost-optimized-saas-pricing-strategy).
...
# Expected routing: input phrase -> functions that should be called
ROUTING_CASES = [
    {"input": "Show me something calming",
     "expect_any": {"filter_wallpapers"}},
    {"input": "Something in my usual style that I haven't seen yet",
     "expect_all": {"get_user_preferences", "filter_wallpapers"}},
    {"input": "How do I use this feature?",
     "expect_none": True},  # A general question should NOT trigger a function call
]
...
## The Key Deadline Arrives Disguised as a Routing Regression

The first time I ran the harness in September, only the `expect_all` case went red. I had added a single word to the `filter_wallpapers` description the week before, so of course that's where I looked. I reverted the wording, ran it again, still red, and spent most of an afternoon staring at phrasing that turned out to be fine.

The problem wasn't the declaration. It was the credential.

Standard API keys — including the restricted ones — stop being accepted through September 2026. The replacement is an auth key tied to a Google Cloud service account. On the surface it looks like swapping one string for another. What actually changes is how permission is held: the unit of issuance moves from "mint a key" to "bind a service account."

What made this expensive was that the harness from the previous section is blind to the difference. `called_functions()` returns the set of function names the model invoked. When the model chooses not to call anything, you get an empty set. When the request never reaches the model because auth rejected it, you also get an empty set. One is a routing regression; the other is an expired foundation. Both print the same `FAIL`.

**Did the model decline to call, or did the call never arrive? Separate those two before anything else.** Until I wrote that line down, I kept reading unauthenticated requests as routing problems.

So I added a small preflight that runs before the regression suite. It classifies the credential, sends exactly one minimal request, sorts the failure into a category, and exits with a code that means something.

```python
# preflight.py — confirm the call can land before testing where it routes
import os
import sys
import requests
...
def credential_kind(value: str) -> str:
    """Identify what kind of credential is sitting in the environment."""
    if not value:
        return "missing"
    if value.lstrip().startswith("{") or value.rstrip().endswith(".json"):
        return "service_account"   # auth key
    return "legacy_api_key"        # stops working during September

def probe(access_token: str) -> tuple:
    """Send one minimal request and classify the failure."""
    body = {
...
if __name__ == "__main__":
    ...
    if kind != "service_account":
        # Exit 2 means "the foundation moved" — never mixed with routing failures (1)
        print("Legacy credential detected. Migrate before running the regression suite.")
        sys.exit(2)
...
In CI these run as `python preflight.py && python routing_regression.py`. Exit 2 points at the foundation, exit 1 at routing, and the distinction is visible from the job title alone — which means I now suspect the key before I suspect the description. I also stopped swallowing exceptions inside `called_functions()`: when the send itself fails, it raises rather than quietly returning an empty set.

I keep the `modelVersion` that `probe()` returns in the CI log as well. Even when you call through an alias, the response tells you which build actually answered. And the tolerance I described earlier — matching on containment rather than exact equality — hasn't changed, because generation parameters can't be relied on to pin determinism. There are reports of `temperature` and `top_p` being accepted without error while quietly falling back to defaults, so a constraint you believe you set may have stopped applying some time ago.

One note specific to mobile. The key never lives in the app, so this migration touches only the backend proxy and CI, and no store review sits in the way. The flip side is that nothing external will stop you from forgetting it. The deadline is yours to hold. I inventory it in three layers:

<table>
<thead>
<tr><th>Layer</th><th>What to inventory</th><th>Easy to miss</th></tr>
</thead>
<tbody>
<tr><td>CI</td><td>Secrets and job-level env blocks</td><td>The second key issued just for regression runs</td></tr>
<tr><td>Server</td><td>Backend proxy environment variables</td><td>Staging and production issued from different accounts</td></tr>
<tr><td>Local</td><td><code>.env</code> files and shell profiles</td><td>An <code>export</code> line written years ago</td></tr>
</tbody>
</table>

While I'm here, a word about the Flash model doing intent classification. The measurements in this article were taken against `gemini-3.2-pro` and the Flash build available at the time. On the Flash side, `gemini-3.8-flash` reached general availability on September 2, 2026, at the same introductory rate as the previous generation ($0.75 input / $3.75 output per MTok). That rate is stated to run through December 31, 2026, so I now estimate the classifier overhead twice — once for this year, once for next. A generation change that leaves the invoice untouched may be the harder kind to notice, precisely because nothing on the bill moves.
