...
## Three Accounting Mistakes I Actually Shipped

Correct design still leaks once it meets production traffic. Here are three billing-accounting mistakes I hit running a Gemini-backed solo SaaS, and what I changed. None of them caused an immediate loss. They quietly bent the numbers, which is exactly what made them hard to notice.

### Recording a missing usageMetadata as zero

Streaming responses and responses cut short by safety filters can come back with `usageMetadata` undefined. Swallowing that with `?? 0`, as the earlier snippet does, silently drops those tokens from billing. In my logs roughly 1–2% of requests fell into this bucket, which produced about a 3% gap between what I reported to Stripe and what Google Cloud actually charged me.

The fix is small: park the failed requests in a separate table and reconcile them daily.
...
if (!usage?.promptTokenCount) {
  ...
  // Fall back to the user's 30-day median, not their mean
  return estimateFromMedian(userId);
}
```

Use the median rather than the mean for the fallback estimate. Means get dragged upward by a handful of heavy sessions and you end up over-billing. After switching to this approach my reported-versus-actual gap settled inside 0.5%.

### Double-counting usage records when the monthly batch reruns

If your aggregation batch fails halfway and you rerun it with `action: "increment"`, the same usage lands twice. That is why the implementation above uses `action: "set"`.

<table>
<tr><th>action</th><th>Behavior</th><th>On rerun</th></tr>
<tr><td><strong>increment</strong></td><td>Adds to the existing value</td><td>Bill grows with every rerun</td></tr>
<tr><td><strong>set</strong></td><td>Overwrites the period total</td><td>Result is identical every time</td></tr>
</table>

Idempotency is the whole game here. Keep a processed flag keyed on `subscription_item_id` plus the billing period on your side, and let Stripe take an overwrite on theirs. With both layers in place, a failed 3 a.m. batch stops being an incident.

### Free tiers dissolving across one person's many accounts

Assume your free tier will be farmed from the day it ships. If a throwaway email address resets the allowance, free-tier cost per real user lands several times over your model. I have watched the same pattern play out on the App Store side of my indie developer work for years.

What I recommend is anchoring the free tier to a payment method or a device rather than an account. Requiring a card on file to start the trial lowers signups, but it turns free-tier cost into a number you can actually forecast. Which side to optimize depends on your stage. In my case I flipped to the stricter model the month free users started eating into gross margin.
