...
This guide walks you through **calling the Gemini 3.1 Pro REST API using curl, Python requests, and JavaScript fetch** — from authentication setup all the way to working code examples and common error fixes.
...
**Q. Should I use `v1` or `v1beta`?**

A. If you need the latest models (Gemini 3.x) or features like thinking, Function Calling, or structured output, use `v1beta`. The `v1` endpoint is more stable but may lag behind on newer capabilities. Many production teams continue using `v1beta` for this reason.

**Q. What is the difference between the Google AI API and Vertex AI?**

A. `generativelanguage.googleapis.com` (the Google AI API) is easy to get started with using just an API key. Vertex AI adds enterprise features like SLAs, regional data residency, VPC connectivity, and fine-tuning pipelines. The typical pattern: use the Google AI API for personal projects and prototypes, and Vertex AI for production services at scale.

**Q. Why should I never hard-code my API key in source code?**

A. API keys committed to a repository can be detected by scanners like GitHub Secret Scanning and exploited. Always use environment variables (`.env` + `.gitignore`) or a secrets manager. For local development, `python-dotenv` makes this simple.
...
If you want to deepen your understanding beyond this guide, [AI Engineering: Building Applications with Foundation Models](https://www.amazon.co.jp/dp/1098166302/?tag=pinocchio-22) (Chip Huyen) offers an excellent framework for designing production-grade AI systems that go well beyond simple REST API calls.
