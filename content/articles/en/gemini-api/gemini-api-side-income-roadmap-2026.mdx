...
# In the Python and JavaScript SDKs, cache-hit tokens land in
# usage.total_cached_tokens
print(response.usage.total_cached_tokens)
```

One migration detail trips people up here. Older posts and samples read `response.usage_metadata.cached_content_token_count`; the current field is `usage.total_cached_tokens`. If your code still uses the old path, it will either raise on a missing attribute or quietly hand you `None` — and `None` reads exactly like "caching isn't working." I nearly rebuilt a cache that was already doing its job because of that.
...
Context caching has a per-model minimum input token count. Anything below the floor is not cached — implicitly or explicitly.
...
The numbers come from Google's [context caching documentation](https://ai.google.dev/gemini-api/docs/caching): 4,096 across the 3.x line, 2,048 on 2.5. Newer generations raise the floor, which means a model upgrade can disable caching that used to work — your unit cost rises without a single line of code changing.

There's a second constraint worth knowing before you pick an architecture. **The Interactions API supports implicit caching only.** Building explicit caches with `client.caches.create` is a `generateContent` path exclusively, so the moment you choose to keep conversation state on Google's side, you've also given up explicit caching. I'd rather make that trade knowingly, in Phase 2, than discover it in Phase 5.
...
MIN_CACHE_TOKENS = 4096  # 3.x floor; drop to 2048 if you stay on 2.5
...
3. **Aggressive context caching**: any prompt template repeated three or more times in a session is a caching candidate — provided it clears the model's token floor.
...
When 3.5 Flash gives way to 3.7, and 3.7 to 3.8, you want the swap to be a config change. Keep model names in environment variables and add an admin toggle. Newer models are usually cheaper *and* better, so this one discipline buys you 10–20% margin roughly every six months. One caveat: review the cache token floor (4,096 on 3.x) every time you move. Swapping the model name and leaving the floor stale raises your unit cost on the day you ship.

### Lesson 3: Don't run production on Free Tier

Free Tier is great for development. Designing every production path around it, though, is hard to walk back later — depending on the version of the [API terms](https://ai.google.dev/gemini-api/terms) in effect, your customers' input and output may be used for model improvement, and you won't be able to answer "does this service hand my data to Google?" honestly. Add a card and move to Tier 1 before you take a real customer's request.

And here is the part I had backwards for months.

**Rate limits apply per project, not per API key.**

Per the [rate limits documentation](https://ai.google.dev/gemini-api/docs/rate-limits), limits are evaluated across three dimensions — requests per minute (RPM), input tokens per minute (TPM), and requests per day (RPD) — and exceeding any one of them returns a 429. The one a side project hits first is almost always RPD. You don't get throttled by a spike; you run out of day.

All three are attached to the project. Issuing a separate key for production changes nothing if both keys live in the same project. Run a validation batch at 2 a.m. and your paying users hit 429 the next morning — that was exactly my failure. **Split projects, not keys.**

RPD has one more quirk: the daily quota **resets at midnight Pacific time**. If your service mysteriously starts working again at the same hour every day, you're not imagining it — you're crossing an RPD boundary.

#### Tier 1 has a spend limit you'll only meet once

There's a second limit that only appears after you enable billing. Alongside RPM and TPM, the Gemini API enforces **spend-based rate limits**, evaluated on a rolling ten-minute window.

<table>
<thead>
<tr><th>Usage tier</th><th>How you get there</th><th>Spend limit per 10 min</th></tr>
</thead>
<tbody>
<tr><td>Free</td><td>Active project or free trial</td><td>—</td></tr>
<tr><td>Tier 1</td><td>Linked, active billing account</td><td>$10</td></tr>
<tr><td>Tier 2</td><td>$100 paid + 3 days from first payment</td><td>$50</td></tr>
<tr><td>Tier 3</td><td>$1,000 paid + 30 days from first payment</td><td>$200</td></tr>
</tbody>
</table>

Ten dollars per ten minutes is unreachable on ordinary traffic. You reach it the moment one user drops a few hundred pages of PDF into your uploader. What comes back is `429 RESOURCE_EXHAUSTED` — identical on the wire to an ordinary quota error, different in cause. If RPM and TPM both have headroom and you're still seeing 429s, look at spend.

If heavy bulk processing is the product, route it away from the interactive path and into the [Batch API](https://ai.google.dev/gemini-api/docs/batch-api), which carries its own separate limits: 100 concurrent requests, a 2GB input file cap, 20GB of file storage. Separating "work that doesn't need to return right now" is what protects the quota your live users depend on.

Free to Tier 1 usually takes effect instantly; later upgrades land within about ten minutes. **Keep the dev project on Free, stand up a second project for production, and enable billing only there.** That's the split I settled on.
