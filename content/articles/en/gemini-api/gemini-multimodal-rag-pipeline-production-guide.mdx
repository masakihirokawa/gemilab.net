...
## Introduction — Why Multimodal RAG Matters
...
### Q1: Which phase of multimodal RAG is the most expensive?

The initial document processing (Ingest phase) is by far the costliest, especially for video analysis which consumes significant tokens. Cache embeddings and text representations for processed documents, and only reprocess when the source material changes. The Query phase can be dramatically reduced with Context Caching and model tiering.

### Q2: How does Gemini Embeddings compare to OpenAI Embeddings?

Gemini's `text-embedding-004` produces 768-dimensional vectors and supports a `task_type` parameter that explicitly distinguishes between document and query embeddings. OpenAI's `text-embedding-3-small` produces 1536-dimensional vectors. Accuracy is comparable, but Gemini's `task_type` specification tends to improve search precision. Check each service's latest pricing for cost comparisons.

### Q3: What if a video exceeds Gemini's context length?

For very long videos (over 2 hours), first split the video into fixed-interval segments and analyze each segment separately with Gemini. Gemini 2.5 Pro's 1-million token context window handles most videos, but for those that exceed it, switch to a frame extraction + image analysis pipeline.

### Q4: What vector database should I use in production?

Choose based on scale and requirements. For small to medium scale (under 1 million chunks), **pgvector** (PostgreSQL extension) pairs well with Supabase and keeps operational costs low. For large scale (10 million+ chunks) with complex filtering, **Pinecone** or **Weaviate** are better suited. Start with pgvector and migrate as needed.
