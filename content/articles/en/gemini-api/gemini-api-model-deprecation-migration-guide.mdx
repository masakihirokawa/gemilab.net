...
```javascript
// audit-models.mjs — inventory model names in the repo and check them against sunset dates
import { readdir, readFile } from "node:fs/promises";
import { join, extname } from "node:path";

// Sunset dates (maintained by hand, or pulled from the deprecations doc)
const SUNSET = {
...
for (const [model, files] of [...hits].sort()) {
  ...
  const flag = days != null && days <= 30 ? `⚠️ ${days} days left (${sunset})` : "";
  if (flag) risk++;
  console.log(`${model.padEnd(36)} ${files.size} file(s) ${flag}`);
}
...
<table>
  <thead>
    <tr><th>Pattern</th><th>Time to notice deprecation</th><th>Emergency fixes</th><th>Production outage risk</th></tr>
  </thead>
  <tbody>
    <tr><td>Relying on email alerts (no audit)</td><td>Days — or never</td><td>Frequent</td><td>High</td></tr>
    <tr><td>Monthly manual audit</td><td>Up to ~30 days</td><td>Occasional</td><td>Medium</td></tr>
    <tr><td>Automated audit in CI (above)</td><td>Immediate, at PR time</td><td>Almost none</td><td>Low</td></tr>
  </tbody>
</table>
...
```javascript
// config/models.mjs — always reference model names through here
export const MODELS = {
  draft: "gemini-3.5-flash",   // high-frequency, low-cost work like drafting and classification
  finalize: "gemini-3.1-pro",  // work that needs stronger reasoning
};
```
