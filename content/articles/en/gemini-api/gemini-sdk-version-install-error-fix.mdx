...
This guide walks you through **every major cause of SDK installation and version mismatch errors** in both Python and Node.js, along with clear, step-by-step fixes.
...
**Q1. `pip install google-genai` finished without errors, but `from google import genai` still fails. Why?**

There are likely multiple Python binaries on your system. The `pip` you ran and the `python` you're using to execute your script may not be linked. Use `python3 -m pip install google-genai` to make sure they match.

**Q2. Can I use both `google-generativeai` and `google-genai` in the same project?**

Technically both can be installed, but since their class names partially overlap, mixing them leads to unpredictable behavior. Always commit to one SDK throughout a project.

**Q3. `node_modules/@google/genai` exists after `npm install`, but I still get `Cannot find module`. What's wrong?**

The new SDK is distributed as an ES Module. If your project uses CommonJS (`require()`), you'll hit this error. Add `"type": "module"` to `package.json`, or switch to `import` syntax. In TypeScript projects, set `"module": "ESNext"` in `tsconfig.json`.

**Q4. `pip install` fails in my Docker container. How do I fix it?**

Binary packages like `grpcio` require build tools to compile from source on Linux. Add `RUN apt-get install -y build-essential` to your `Dockerfile`, then rebuild the image.

**Q5. I'm using Vertex AI instead of the direct Gemini API. Does the same fix apply?**

Vertex AI uses a different package — `google-cloud-aiplatform`. Check `pip show google-cloud-aiplatform` and upgrade to version `1.40.0` or above. Authentication flows differ too: Vertex AI relies on a service account or Application Default Credentials (ADC) rather than a plain API key.
...
For a deeper dive into Python dependency management, [*Automate the Boring Stuff with Python* by Al Sweigart](https://www.amazon.co.jp/dp/4873119936/?tag=pinocchio-22) is a great companion resource.
