...
Third, build observability for it. Always emit `finish_reason` to structured logs, then aggregate in your APM. As a rule of thumb, once RECITATION exceeds 1% of total requests, your prompt design needs surgery, not patching. For a deeper grounding on retry, fallback, and graceful-degradation patterns that apply equally to Gemini, [*LangChain Complete Introduction* (Yu Tamura)](https://www.amazon.co.jp/dp/4295202215/?tag=pinocchio-22) covers the architectural patterns well.
