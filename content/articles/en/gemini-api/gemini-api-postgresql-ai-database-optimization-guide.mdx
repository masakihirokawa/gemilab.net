...
highlights:
  - "Master Text-to-SQL generation with Gemini 2.5 Pro and secure query execution patterns"
  - "Automate EXPLAIN plan analysis, index recommendations, and schema reviews with AI — ready to implement today"
  - "Build a production-ready system with proper security, cost management, and monitoring"
---

## Introduction — Why AI × PostgreSQL?
...
**Q1: Is there a real risk of unsafe SQL being generated?**

The system uses three layers of protection: ① The system prompt explicitly instructs Gemini to generate SELECT-only statements; ② The `_is_safe_sql()` method uses regex to block DDL and DML keywords; ③ Generated SQL is executed against a read-only database user. Combined, these make unsafe execution virtually impossible in practice.

**Q2: What happens with very large schemas (100+ tables)?**

Gemini 2.5 Pro supports up to 1M tokens of context, so even large schemas fit comfortably. However, sending the full schema on every request increases cost. Consider implementing Gemini's context caching, or building a "schema filter" that dynamically selects only the tables relevant to the incoming query.

**Q3: Should I use EXPLAIN ANALYZE instead of EXPLAIN?**

`EXPLAIN ANALYZE` actually executes the query, which means it adds real load to your production database. Use `EXPLAIN (FORMAT JSON)` for production API endpoints and reserve `EXPLAIN ANALYZE` for development or staging environments. Combining `pg_stats` data with Gemini's analysis provides sufficient accuracy without the execution overhead.

**Q4: Can this be adapted for databases other than PostgreSQL?**

Yes, with modifications. The core AI patterns are database-agnostic — only the `SchemaIntrospector` and connection layer need to change. Replace the system catalog queries with MySQL's `information_schema` equivalents and swap `asyncpg` for `aiomysql`, and the rest of the architecture applies directly.

**Q5: Does the server need to restart after schema changes?**

It's best to add a `/schema/refresh` endpoint that updates the cached schema context on demand. For teams with frequent schema migrations, you could also use PostgreSQL's `LISTEN/NOTIFY` to detect DDL changes and automatically refresh the cache, keeping the agent's context current without restarts.
...
For a deeper dive into PostgreSQL internals and query optimization theory, [PostgreSQL: Up and Running by Regina O. Obe and Leo S. Hsu](https://www.amazon.co.jp/dp/4873119634/?tag=pinocchio-22) is an excellent companion — covering everything from system catalogs to advanced performance tuning that pairs well with the AI-driven approach in this article.
