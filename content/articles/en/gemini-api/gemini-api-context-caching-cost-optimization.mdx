...
<table>
  <thead>
    <tr><th>Item</th><th>Standard Input</th><th>Cached Input</th><th>Savings</th></tr>
  </thead>
  <tbody>
...
<table>
  <thead>
    <tr><th>Use Case</th><th>Recommended TTL</th><th>Reason</th></tr>
  </thead>
  <tbody>
    <tr><td>Customer support bot</td><td>12–24 hours</td><td>Active during business hours</td></tr>
    <tr><td>Batch document processing</td><td>1–2 hours</td><td>Not needed after processing</td></tr>
    <tr><td>Daily report generation</td><td>2–4 hours</td><td>Only during report generation</td></tr>
    <tr><td>Code review assistant</td><td>4–8 hours</td><td>During the review session</td></tr>
  </tbody>
</table>
...
```python
# break_even.py — estimate the break-even point for caching
def break_even_requests(doc_tokens: int, cache_hours: int) -> float:
    INPUT = 1.25 / 1_000_000
    CACHED = 0.3125 / 1_000_000
    STORAGE = 4.50 / 1_000_000
    # Fixed cost: creation (one full input) + storage (for the TTL window)
    fixed = doc_tokens * INPUT + doc_tokens * STORAGE * cache_hours
    # Savings per request: full-input price -> cached price
    saved_per_req = doc_tokens * (INPUT - CACHED)
    return fixed / saved_per_req

for h in (8, 4, 1):
    n = break_even_requests(328_450, cache_hours=h)
    print(f"TTL {h}h: ~{n:.0f} requests to break even")

# Expected output:
# TTL 8h: ~40 requests to break even
# TTL 4h: ~23 requests to break even
# TTL 1h: ~6 requests to break even
```
