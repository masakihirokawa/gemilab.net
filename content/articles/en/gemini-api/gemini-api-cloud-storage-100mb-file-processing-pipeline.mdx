...
highlights:
  - "If you've been struggling with the 20MB limit, you can now build a production pipeline that feeds 100MB files directly into Gemini API"
  - "You'll get copy-paste-ready code for secure file integration using GCS buckets and private DB pre-signed URLs"
  - "You can apply a complete operational architecture — batch processing, error recovery, and cost optimization — directly to your own projects"
---

## Introduction — The File Size Barrier Has Been Lifted
...
### Q1: Should I use GCS URIs or pre-signed URLs?

If your infrastructure is fully within GCP, GCS URIs (`gs://`) are recommended. Authentication is handled entirely through IAM, eliminating the need to manage signed URL expiration. Use pre-signed URLs when integrating with multi-cloud environments or data sources outside GCP.

### Q2: How do I handle files larger than 100MB?

Files exceeding 100MB need to be split into chunks before processing. For PDFs, use `PyPDF2` or `pdfplumber` to split by page. For videos, use `ffmpeg` for segment splitting. Upload each chunk to GCS, process them individually with Gemini API, then merge the results. For approaches that don't require chunking or RAG, check out our [Gemini File Search API complete guide](/en/articles/gemini-api/gemini-file-search-api-grounding-guide).

### Q3: What's the difference between Flex Tier and Priority Tier?

These are new inference tiers introduced in April 2026. **Flex Tier** is designed for batch processing — it offers no latency guarantees but can be up to 50% cheaper. **Priority Tier** guarantees low latency for real-time processing but costs more than standard pricing. For bulk large file processing, Flex Tier is the optimal choice.

### Q4: Does Google retain my file data during processing?

Gemini API file processing is stateless. Files uploaded via the Files API are automatically deleted after 48 hours by default. When using GCS URIs or pre-signed URLs, Gemini temporarily reads the file contents for processing but does not permanently store them. For enterprise requirements, check the Data Governance settings in Vertex AI.

### Q5: What Cloud Functions timeout considerations should I be aware of?

The default timeout for Cloud Functions (2nd gen) is 60 seconds, which is insufficient for large file processing. You can extend it up to 60 minutes — we recommend setting `--timeout=540` (9 minutes) based on your expected file sizes. If that's still not enough, consider migrating to Cloud Run.
...
To deepen your understanding of the pipeline architecture covered in this article, we recommend [Designing Data-Intensive Applications by Martin Kleppmann](https://www.amazon.co.jp/dp/1449373321/?tag=pinocchio-22), which provides foundational principles for building reliable, scalable data systems.
