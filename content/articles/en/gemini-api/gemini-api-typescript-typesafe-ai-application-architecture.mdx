...
// An invariant asserts minimal content validity, not structure
...
export function recordValidation<T>(
  ...
  for (const inv of invariants) {
    if (!inv.holds(parsed.data)) {
      // Structurally valid but semantically broken — the face of silent decay
...
```typescript
// Example invariants for a summary schema
