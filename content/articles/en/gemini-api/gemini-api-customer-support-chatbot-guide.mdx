...
For more detail on prompt design, see the [Gemini 2.5 Pro System Instructions Master Guide](/articles/gemini-api/gemini-25-pro-system-instructions-mastery).
...
To suppress wrong facts, don't let the model answer from its own knowledge. Give it a FAQ database via Function Calling and let it pull authoritative answers when relevant. The basics are covered in [Production Patterns for Function Calling with the Gemini 2.5 Pro API](/articles/gemini-api/gemini-25-pro-api-function-calling-production-patterns).
...
Here they are at a glance. If you arrived searching for one specific symptom, read only the paragraph that matches.

<table>
<thead>
<tr><th>Symptom</th><th>Immediate cause</th><th>Where the fix goes</th></tr>
</thead>
<tbody>
<tr><td>Answers with stale facts after calling a tool</td><td>Internal knowledge outranks tool output</td><td>State in the system prompt that tool results are the sole source of fact</td></tr>
<tr><td>Latency and cost spike past ~20 turns</td><td>Conversation history grows linearly</td><td>Summarize and replace history at a threshold (code below)</td></tr>
<tr><td>Angry users cut the conversation short</td><td>Repeated apologetic preambles</td><td>Cap the preamble at once per turn, in writing</td></tr>
<tr><td>Replies fall apart when an image is attached</td><td>Text-only prompt assumptions</td><td>Add an image branch to the prompt</td></tr>
<tr><td>Bot reverses an escalation decision</td><td>Model still being called after the flag</td><td>Stop the call in application code</td></tr>
<tr><td>Usage dashboards are unreadable</td><td>One key shared by dev and prod</td><td>Separate keys per environment</td></tr>
<tr><td>Chat window goes silent</td><td>Safety filter leaves <code>text</code> empty</td><td>Inspect <code>finish_reason</code> and swap in a fallback (code below)</td></tr>
</tbody>
</table>
...
### Putting History Compaction and the Empty-Response Guard in One Place

Pitfalls two and seven both live at the entrance and exit of the chat loop, so they can be absorbed by two small helpers. I keep these in a shared utility module and route every channel through them.

```python
import google.generativeai as genai

SUMMARIZE_AFTER_TURNS = 10   # compact once history exceeds this
KEEP_RECENT_TURNS = 4        # leave the most recent turns verbatim
FALLBACK_TEXT = "Sorry — I wasn't able to answer that here. Let me connect you with a person."


def compact_history(history: list) -> list:
    """Fold older turns into a single summary, keeping recent turns intact."""
    ...
    summary = model.generate_content(
        "Summarize this support conversation as bullet points, keeping only "
        "unresolved issues and confirmed facts. Do not speculate or editorialize.\n\n" + text
    ).text
    return [{"role": "user", "content": f"[Conversation so far]\n{summary}"}] + recent


def safe_text(response) -> tuple[str, str | None]:
    """Never surface an empty reply. Returns (text, stop_reason)."""
...
The load-bearing detail in `compact_history` is the instruction not to speculate. Drop it and the summary starts carrying the model's interpretation, which the next turn then treats as established fact. I learned this when a summary confidently recorded "the user is requesting a refund" and nearly routed a plain feature question into the refund flow.

`safe_text` deliberately returns a tuple. The string shown in the UI and the reason written to logs serve different purposes. Count how often the fallback fires, broken down by `reason`, and a week where the safety filter starts misfiring becomes obvious immediately.
...
For per-status error triage, see [Classify Gemini API Errors by Status](/articles/gemini-api/gemini-api-error-troubleshooting-guide). For holding costs down with caching, see [Gemini API Caching Strategy Notes](/articles/gemini-api/gemini-api-caching-strategy-masterclass-production).
...
Stick with it for six months and the support response time visibly shrinks. I've run app support solo alongside development for a long time, and ever since I added Gemini API to the loop, that heavy feeling of "I'm late on replies again" has receded.
