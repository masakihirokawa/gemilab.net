...
```python
# eval_flakiness.py
# Judge the same case N times and measure score flakiness.
# Output: per-case mode, agreement, spread, and an "unreliable" flag.
import os
import statistics
...
def measure_flakiness(question: str, answer: str, n: int = 5) -> dict:
    ...
    agreement = mode_count / n            # share of the most common score
    spread = max(scores) - min(scores)    # min-to-max range
    return {
        "mode": mode,
        "agreement": round(agreement, 2),
        "spread": spread,
        "stdev": round(statistics.pstdev(scores), 2),
        # Unreliable if agreement < 0.6 or the spread is 2 or more
        "unreliable": agreement < 0.6 or spread >= 2,
        "raw": scores,
    }

if __name__ == "__main__":
    q = "Show me the minimal code to call the Gemini API from a Cloudflare Worker."
    a = "POST to the endpoint with fetch and authenticate via x-goog-api-key; the model name goes in the URL."
...
If you want to go deeper on evaluation design, pair this with [Prompt Evaluation and Optimization Pipeline for the Gemini API](/articles/gemini-api/gemini-api-prompt-evaluation-optimization-pipeline) and [Quality Monitoring with a Golden Dataset and an LLM Judge](/articles/gemini-api/gemini-api-quality-monitoring-golden-dataset-llm-judge) for a clearer view of the whole operations phase.
