...
highlights:
  - "Engineers stuck wiring streaming audio plus screen frames to the Gemini Live API will leave with a working TypeScript implementation today"
  - "You will learn the exact pattern for combining getDisplayMedia, AudioWorklet, WebSocket, and Function Calling into a stable bidirectional voice loop"
  - "Solo developers will gain a practical second set of eyes during long coding sessions, with operational tactics for keeping monthly cost under five dollars"
---
...
For the broader patterns of bidirectional streaming protocols — back-pressure, framing, recovery — the WebSocket chapter of [*Real-Time Web Engineering: WebSocket, WebRTC, SSE* by Yusuke Nakamura](https://www.amazon.co.jp/dp/4297139383/?tag=pinocchio-22) is the reference I keep returning to. The taxonomy of failure modes generalizes well beyond Gemini.
