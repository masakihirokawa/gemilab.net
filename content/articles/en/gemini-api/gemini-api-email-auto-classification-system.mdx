...
In this article, we'll walk through building an email auto-classification and summarization system using Gemini 2.5 Flash and TypeScript.
...
**Q: Are there privacy concerns with sending email content to the API?**

Gemini API under Google Cloud's terms of service does not use submitted data for model training. However, for sensitive content, we recommend adding a preprocessing step that converts email bodies into summaries or keywords before sending them to the API.

**Q: Can I use plain JSON output instead of Function Calling?**

For simple prototypes, JSON output works fine. For production systems, we recommend Function Calling because the `enum` constraints on categories significantly reduce the risk of unexpected return values.

**Q: What kind of processing speed can I expect?**

Gemini 2.5 Flash completes classification in approximately 1-2 seconds per email. With batch processing at concurrency 5, you can classify 100 emails in about 40 seconds.

**Q: How can I improve classification accuracy?**

Custom rules are the most effective lever. Specifically, enriching your `urgentSenders` list and `customPromptRules` with business-specific context dramatically improves accuracy for your particular workflow.
...
For a broader perspective on AI-powered workflow automation, "[AI時代の業務自動化](https://www.amazon.co.jp/dp/4297145758/?tag=pinocchio-22)" offers comprehensive coverage.

For Gemini API basics, see our "[Gemini API Quickstart](/en/articles/gemini-api/gemini-api-quickstart)". For Function Calling details, check out "[Complete Guide to Function Calling](/en/articles/gemini-advanced/function-calling-guide)". If you're interested in streaming responses, "[Implementing Streaming Responses and Multi-Turn Chat](/en/articles/gemini-api/streaming-and-chat)" is also a great resource.
