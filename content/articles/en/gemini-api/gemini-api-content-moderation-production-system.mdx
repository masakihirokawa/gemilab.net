---
title: "Building a Production Content Moderation System with Gemini API: A Complete Guide"
...
## Introduction: Why Use Gemini API for Content Moderation?

For any service handling user-generated content (UGC), content moderation is unavoidable. The traditional approach relies on dedicated moderation APIs such as Amazon Rekognition or Microsoft Azure Content Moderator. In 2026, however, custom moderation systems built on the Gemini API are gaining rapid traction.

The reason is straightforward. Gemini 2.5 Pro and Flash bring multimodal support and sophisticated contextual understanding that goes far beyond keyword matching or simple image classification, enabling **judgment that accounts for the full context** of a piece of content. A mention of "knife techniques" reads very differently in a cooking recipe than in a threatening post, and Gemini handles that distinction reliably.

The built-in safety filters (`safety_settings`) cover basic needs, but they fall short in scenarios like:
...
This guide walks you through building a **production-ready content moderation system** on the Gemini API, complete with working code throughout.
...
- Borderline cases where confidence < 0.7
...
Expected output for a benign post: `{"decision": "approved", "confidence": 0.95, "categories": [], "reason": "General user inquiry with no harmful content."}`
...
class ModerationWorker:
    ...
    async def process_item(self, item: dict) -> dict:
        ...
        if result.confidence < self.pro_escalation_threshold and content_type == "text":
            logger.info(f"[{content_id}] Flash confidence={result.confidence:.2f} -> escalating to Pro")
...
Storing human decisions in `training_feedback` lets you periodically update the few-shot examples in your System Instructions, continuously improving model accuracy without any retraining.
...
The three metrics that deserve the closest attention: **REVIEW rate** (aim for 15-25% of all decisions), **Latency P95** (target under 5 seconds), and **human review queue depth** (consistent growth signals a need to tighten your thresholds).

---

## FAQ

**Q1. Should I use both the built-in safety filters and custom moderation logic?**

Yes, we recommend it. Keep the built-in filters as a baseline safety net and layer custom moderation on top for business-specific decisions. Tune the built-in thresholds via `safety_settings` as needed (see [Gemini Safety Settings](/articles/gemini-basics/gemini-safety-settings)).

**Q2. Is it safe to send user-generated content to the Gemini API from a privacy standpoint?**

For the paid tier, Gemini API does not use prompts for model training by default. That said, we strongly recommend anonymizing or stripping sensitive personal information before sending. Always review the applicable terms of service and privacy policy.

**Q3. How should I handle multi-language content?**

Gemini 2.5 handles multiple languages well, so you generally do not need language detection before sending. However, System Instructions written in **English tend to produce more consistent results**, even for non-English content. If you have language-specific rules, consider detecting the language first and routing to language-specific prompts.

**Q4. What are the approximate monthly costs for one million items?**

With Flash only at an average of 200 tokens per item, expect roughly $200 to $400 per month. If 10% of items escalate to Pro, add another $100 to $200. Actual costs depend on average content length and your Flash/Pro mix ratio.

**Q5. Can I use moderation results for fine-tuning?**

As of April 2026, Gemini API fine-tuning is limited to text generation tasks. However, using accumulated human review decisions as few-shot examples in your System Instructions -- a form of prompt tuning -- is highly effective and requires no model retraining.
...
For a broader perspective on building LLM-powered applications, [*Generative AI with LangChain* by Ben Auffarth](https://www.amazon.co.jp/dp/4814400764/?tag=pinocchio-22) is an excellent companion resource.
