...
---
Images, video, audio, and PDFs — all in a single request. For anyone who has maintained a separate pipeline per modality, Gemini 3.1 Pro's headline capability quietly rewrites the design assumptions.

Mixing them isn't automatically free, though. Token consumption and accuracy degrade differently for each modality. What follows are the implementation-level details that only surface once real traffic hits.
...
Across the Dolice Labs apps as an indie developer, mixing image, video, audio, and PDF tripped me up on input variance long before accuracy. Normalizing size, length, and format before the call pays off in both quality and cost — and for long media I clip the relevant span instead of sending everything, processing only what matters at high fidelity.
