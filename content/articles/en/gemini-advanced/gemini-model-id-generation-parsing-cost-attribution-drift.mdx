...
You can do this by eye, but I ended up leaving a small audit script in place and running it nightly from a non-blocking CI job. It does one thing: exit 1 when it finds something.

```python
# id_parse_audit.py - find every place that takes a model ID apart as a string
import re, sys
...
Here is what a run looks like (paths substituted for the write-up):
...
The first run turned up more than the two spots I had in my head. Seeing that output was how I learned the routing layer still carried the same assumption, well after I thought the rollup fix had closed the issue. I keep the job non-blocking because a red pipeline in the middle of a migration only stops the work; instead I get a notification on days when the count goes up.
