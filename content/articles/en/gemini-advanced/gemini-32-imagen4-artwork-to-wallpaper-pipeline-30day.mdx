...
highlights:
  - "You can pick up a complete, working production pipeline that uses Gemini 3.2 Pro to read the structure of one of your own paintings and Imagen 4 to expand it into a coherent wallpaper series"
  - "You will be able to lift the style-consistency tactics, resolution and color-palette quality gates, and cost-tracking implementation directly into your own indie wallpaper apps"
  - "You will see 30 days of real numbers — API spend, rejection rates, downstream app KPIs — so you can make a grounded call on whether to port this approach into your own operation"
---
...
This is a fairly natural fit for Function Calling, and the basic patterns from [the Function Calling complete guide](/en/articles/gemini-api/gemini-api-function-calling-complete-guide) carried over directly. I did not actually need the tool-use plumbing for this pipeline — the JSON response shape from Gemini 3.2 Pro was enough — but the mental model is the same: separate "what does the model read" from "what does the model write."
...
For adjacent reading, the production patterns in [Vertex AI Gemini for production](/en/articles/gemini-dev/vertex-ai-gemini-production) are useful when you want to put this kind of orchestration on a more serious deployment substrate, and [the ADK persistent assistant guide](/en/articles/gemini-advanced/gemini-25-pro-adk-persistent-assistant-production-guide) is the right next step if you want the pipeline to keep state across sessions. If you want one book on the shelf to deepen your intuition for diffusion-based generation, [Generative Deep Learning, 2nd Edition (David Foster)](https://www.amazon.co.jp/dp/4814400756/?tag=pinocchio-22) is a solid companion — orthogonal to this pipeline, but it makes the safety-filter and composition failure modes much easier to reason about once you understand them from the inside.
