...
Now the benefit side. I ran 4 languages x 3 approaches x 500 requests on `gemini-flash-latest` and classified the language of each response body. Classification reads the characters rather than asking a model, because otherwise the instrument and the subject share the same failure mode. I had to rebuild that classifier partway through, which is the next section.
...
## I had put three Latin-script languages in the same bucket

Not classifying with a model was the right call. The classifier I actually wrote, though, was not doing its job.

The first version looked only at script composition. Enough Thai characters and it returned Thai; any hiragana, katakana, or kanji and it returned Japanese. That part was fine. The trouble was everything else: Vietnamese, Indonesian, and English all came back from that function as "Latin."

So whenever English came back where Vietnamese should have, I was counting it as a match.

I noticed because Vietnamese moved differently from the others once the pin was in. Thai climbed from 91.2% to 99.1%, which made sense, but Vietnamese had been high before I changed anything. I pulled thirty raw responses and read them, and English sentences were sitting on the "match" side of several of them.

**A measurement can't contain a distinction the instrument can't make.** Obvious enough — and easiest to miss when you are counting languages you cannot read.

The rebuilt version works in two stages. Script composition sorts first, and anything that lands in Latin gets sorted again by diacritic density and function-word overlap.

```python
# detect_output_language.py
# Two stages: sort by script, then split Latin-script languages by diacritics and stopwords
import re
import unicodedata
...
ID_STOP = {"yang", "dan", "untuk", "dengan", "ini", "itu", "tidak", "adalah", "pada", "dari"}
EN_STOP = {"the", "and", "for", "with", "this", "that", "not", "is", "are", "from", "of"}

def _script(ch: str):
    if not ch.isalpha():
        return None
    name = unicodedata.name(ch, "")
    for s in ("THAI", "HIRAGANA", "KATAKANA", "CJK", "LATIN"):
        if s in name:
            return s
    return None

def detect(text: str) -> str:
    counts = Counter(s for s in map(_script, text) if s)
    total = sum(counts.values()) or 1
    ratio = {k: v / total for k, v in counts.items()}

    # Stage 1: languages that script alone settles
    if ratio.get("THAI", 0) >= 0.15:
        return "th"
    if ratio.get("HIRAGANA", 0) + ratio.get("KATAKANA", 0) + ratio.get("CJK", 0) >= 0.15:
        return "ja"
    if ratio.get("LATIN", 0) < 0.5:
        return "unknown"

    # Stage 2: split the Latin-script group by diacritic density and stopword overlap
    low = text.lower()
    if sum(ch in VI_MARKS for ch in low) / max(len(low), 1) >= 0.02:
        return "vi"
    words = set(re.findall(r"[a-z]+", low))
    return "id" if len(words & ID_STOP) > len(words & EN_STOP) else "en"
```

Across my five languages, Vietnamese came out at a diacritic ratio of 0.217 while Thai, Japanese, Indonesian, and English all sat at 0.000. The 0.02 threshold is there so that a few accented loanwords in an English or Indonesian sentence don't tip the result.

Classification itself came in under 60 microseconds per item. Even at 500 requests x 4 languages x 3 approaches, the classifier accounts for about 0.4 seconds in total. There was never a reason to ask a model.

Every match rate in this article was re-measured with the rebuilt classifier. The Vietnamese numbers from the first version I threw away.
...
**2. Don't classify with a model.** Ask Gemini to identify the output language and your detector inherits the same wobble you are trying to measure. Reading the characters is deterministic, and the two-stage version below still ran in under 0.1 ms per item.
...
The whole exercise cost me half a day. In exchange, an entire translation pipeline I had been about to build turned out to be unnecessary. On days when the list of things I didn't have to build gets longer, I walk away from the desk a little lighter.
