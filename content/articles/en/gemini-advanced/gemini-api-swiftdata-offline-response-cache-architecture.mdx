...
I have shipped indie iOS apps since 2014, and the wallpaper, calm, and manifestation categories within my roughly 50-million-download portfolio open disproportionately often in places with poor connectivity. After integrating Gemini API in earnest from 2025 onward, I found that "returning the previous response without calling the network" was directly tied to perceived production quality. This article walks through an architecture that uses SwiftData as the storage tier to persist and reuse Gemini API responses, with both the implementation and the operational lessons.
