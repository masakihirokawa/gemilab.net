...
This guide walks you through building a **Gemini API + Apps Script pipeline for automated Looker Studio report analysis**, with complete code and step-by-step instructions.
...
**Q: Do I need Looker Studio Pro to connect to Google Sheets?**

No. The free version of Looker Studio fully supports Google Sheets as a data source. Looker Studio Pro adds features like SLA guarantees and enhanced team sharing, but it's not required for this integration.

**Q: How much will the Gemini API cost?**

With `gemini-2.5-flash`, pricing is approximately $0.15 per million input tokens and $0.60 per million output tokens (as of April 2026). For one analysis run per day, you'll likely stay well within the free tier or spend under $1/month. See the [Gemini API Pricing Guide](/articles/gemini-api/gemini-api-pricing-billing-complete-guide-2026) for details.

**Q: Can I use BigQuery or other data sources instead of Sheets?**

Absolutely. Replace the `fetchSalesData()` function with a call to the BigQuery API or any other REST endpoint. For direct BigQuery integration via Vertex AI, check out the [Gemini API × BigQuery Data Analysis Guide](/articles/gemini-dev/gemini-api-bigquery-ai-data-analysis).

**Q: How do I improve the quality of AI analysis?**

Prompt engineering matters most here. Add business context to your prompt (e.g., "This is monthly data for a SaaS company") and specify the KPIs you care about. For more on crafting precise system instructions, see the [Gemini API System Instructions Mastery Guide](/articles/gemini-api/gemini-25-pro-system-instructions-mastery-guide).
...
If you'd like to deepen your Apps Script knowledge, [*Google Apps Script: A Developer's Notebook*](https://www.amazon.co.jp/dp/4297134632/?tag=pinocchio-22) is a highly practical resource for building reliable automation workflows.
