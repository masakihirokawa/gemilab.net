...
highlights:
  - "Developers stuck on integrating Gemini API with Apps Script can pick up 12 copy-paste-ready patterns spanning Gmail, Docs, Sheets, and Calendar—and run them today"
  - "Go beyond single-service automation and master the architectural skills to design cross-service AI workflows like weekly business reports and customer feedback pipelines"
  - "Walk away with production-ready operational design: rate limiting, error retry with exponential backoff, GAS execution time workarounds, and security best practices"
---
...
**Q. Can I use UrlFetchApp with Gemini API without hitting timeout issues?**

A. `UrlFetchApp.fetch()` defaults to a 60-second timeout. Gemini 2.5 Pro can take 30–60 seconds for complex reasoning tasks, so always set `muteHttpExceptions: true` to receive error details rather than a generic timeout. For very long generations, consider using `gemini-2.5-flash` (faster) or splitting the task into smaller prompts.

**Q. How many Gemini API requests can I make per day from Apps Script?**

A. The Gemini API free tier (Gemini 2.5 Pro) allows 2 requests per minute and 1,500 per day as of April 2026. For business automation that processes hundreds of rows daily, upgrading to Pay-as-you-go is recommended. Context Caching can reduce token consumption by up to 75% for scripts that use the same system prompt repeatedly.

**Q. How do I reliably get structured JSON back from Gemini in Apps Script?**

A. Set `temperature` to `0.1–0.2` and include "Respond in JSON:" explicitly in your prompt. For more reliable JSON output with Gemini 2.5 Pro, add `responseMimeType: "application/json"` to `generationConfig` in your payload. Always wrap `JSON.parse()` in `try-catch` and implement a retry when parsing fails—Gemini occasionally wraps JSON in markdown code fences.

**Q. My onFormSubmit trigger is slow because of Gemini API latency. How do I handle this?**

A. Use a two-phase approach: in `onFormSubmit`, send an immediate acknowledgment email to the user (no Gemini call), then create a time-based trigger delayed by 5–10 seconds to run the Gemini analysis separately. This keeps the form submission experience fast while still processing feedback intelligently.

**Q. How do I deploy these automations organization-wide?**

A. In Google Workspace Admin Console, approve the OAuth scopes your scripts need for domain-wide use. For scripts accessing data across your org (not just the script owner's account), configure Domain-Wide Delegation (DWD) with a service account. Apply the principle of least privilege—request only the specific scopes your automation actually needs—and enable audit logging in the Admin Console.
