---
title: "Google Workspace × Gemini API Automation: Production Notes on 12 Apps Script Patterns"
...
author: "Gemini Lab"
description: "12 Gemini API + Apps Script patterns for Gmail, Docs, Sheets, and Calendar automation—plus the production snags I hit running this across four sites and an app support inbox: swallowed 429s, JSON code fences, the 6-minute cap, and flash-vs-pro routing, with measured numbers."
...
### 1. `muteHttpExceptions: true` silently swallows 429s
...
function callGemini(prompt, options = {}) {
  ...
  const code = response.getResponseCode();          // ← always inspect the status
  if (code === 429 || code === 503) {
    throw new Error(`RETRYABLE_${code}`);            // mark as backoff-eligible
  }
  const json = JSON.parse(response.getContentText());
  if (json.error) throw new Error(`Gemini API Error: ${json.error.message}`);
  return json.candidates[0].content.parts[0].text;
...
function parseGeminiJson(text) {
  // Strip code fences and surrounding prose, then take the first JSON object
