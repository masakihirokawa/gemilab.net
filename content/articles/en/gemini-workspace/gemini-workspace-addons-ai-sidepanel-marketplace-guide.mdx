...
slug: "gemini-workspace-addons-ai-sidepanel-marketplace-guide"
...
## Introduction — Why Workspace Add-ons × Gemini?
...
This utility supports both plain text responses and structured output (JSON mode). For details on structured output, see "[Gemini Structured Output Production Guide](/en/articles/gemini-advanced/gemini-structured-output-production-guide)."
...
For a systematic approach to monetization strategy, see "[Gemini Monetization Masterplan 2026](/en/articles/gemini-api/gemini-monetization-masterplan-2026)."
...
### Q1: Does it cost anything to develop Workspace Add-ons?

Development itself is free. As long as you stay within Apps Script quotas (6 hours daily execution time, 20 triggers, etc.), there are no additional charges. However, Gemini API usage fees apply separately. Gemini 2.5 Flash has a free tier (1,500 requests per day), which keeps costs low during prototyping and personal use. Publishing to the Marketplace has no Google Cloud certification fees, but you'll need to budget for hosting your privacy policy page and similar overhead.

### Q2: What should I do if my security review is rejected?

The review team provides specific feedback detailing the issues. Address each point systematically. The most common rejection reasons are "requesting unnecessary OAuth scopes" and "inadequate privacy policy." Review your scopes and switch to `currentonly` variants wherever possible. You can resubmit without any limit on attempts, but repeated rejections tend to lengthen subsequent review periods. Before your first submission, thoroughly read Google's "OAuth App Verification FAQ."

### Q3: Do I need Marketplace publication to distribute within my organization only?

No. If a Google Workspace administrator distributes the add-on as an "internal app" within their organization, Marketplace publication and security review are not required. From the Apps Script editor, select "Deploy" → "New deployment" → "Add-on," then restrict distribution to "Users in my domain." Marketplace publication and security review are only necessary when distributing to external users.

### Q4: Are there benefits to using Vertex AI instead of the Gemini API?

For enterprise add-ons, Vertex AI is worth considering. Vertex AI lets you specify data processing regions (EU, US, Asia, etc.), making it easier to comply with GDPR or internal data policies. Additional capabilities include VPC Service Controls for network perimeter security, model fine-tuning, and Grounding with Google Search. That said, the API key-based Gemini API offers simpler setup, making it the recommended starting point for individual developers and startups.
...
For a comprehensive overview of building revenue streams with the Google AI ecosystem, we recommend exploring "[Gemini Monetization Masterplan 2026](/en/articles/gemini-api/gemini-monetization-masterplan-2026)." If you'd like to deepen your understanding of AI-powered business building, "[Building AI-First SaaS Products](https://www.amazon.co.jp/dp/B0D1LM1V1Y/?tag=pinocchio-22)" is an excellent resource as well.
