...
Yes. Google AI Studio itself is free, and the unified Playground is included. However, API usage is subject to free-tier limits, so heavy usage may require a paid plan. See our [Google AI Studio Getting Started Guide]((/articles/gemini-dev/google-ai-studio-guide) for details.

### Q2: Does Maps Grounding cost extra?

Maps Grounding is provided as a tool within the standard Gemini API pricing. There's no separate charge, but grounding requests do consume tokens, so factor this into your cost calculations. Learn more in our [Grounding with Google Maps Guide]((/articles/gemini-advanced/gemini-grounding-google-maps).
