...
In this guide, we'll walk through the fundamentals of Firebase Studio and show you step-by-step how to build a web app with AI features powered by Gemini.
...
**Q: Is Firebase Studio free to use?**

A: Firebase Studio is available on the **Spark (free) plan**. However, calling the Gemini API and deploying Cloud Functions typically requires upgrading to the Blaze (pay-as-you-go) plan. For development and testing, the free tier usually covers your needs comfortably.

**Q: Does Firebase Studio support frameworks other than Next.js?**

A: Yes. Firebase Studio offers starter templates for **Astro, Angular, Vue, SvelteKit**, and more. On the backend side, Cloud Functions (Node.js/Python) and Cloud Run are both supported. You can select your preferred framework during project creation.

**Q: How do I deploy my Firebase Studio + Gemini app to production?**

A: You can deploy to **Firebase Hosting** with a single click from within Firebase Studio. Custom domain configuration is handled through the GUI. For more scalable backends, deploying to Cloud Run is also straightforward. Make sure to manage your API keys and environment variables properly before going live.

**Q: What kinds of apps are best suited for Firebase Studio + Gemini?**

A: Firebase Studio shines for building AI-first products — chatbots, content generation tools, image analysis apps, voice assistants, and more. Combined with Firebase's real-time database, you can efficiently build multi-user AI experiences without managing complex infrastructure.
