...
highlights:
  - "Learn how to analyze web pages with Gemini API and auto-generate meaningful test scenarios"
  - "Master self-healing test patterns using Playwright + Gemini multimodal analysis"
  - "Build a production-ready CI/CD architecture for AI-driven E2E testing"
---

## Introduction — Why Use AI to Auto-Generate E2E Tests?
...
By combining Gemini API's powerful multimodal analysis capabilities with Playwright's browser automation, we can fundamentally solve this problem. In this guide, we'll build a system that feeds web page screenshots and HTML structures to Gemini, generating test scenarios, producing test code, and even auto-repairing tests when the UI changes (self-healing).
...
Bundle multiple page test generation requests together to reduce API call frequency. Leveraging Gemini's [Batch Processing API](/en/articles/gemini-api/gemini-batch-processing-api-guide) can achieve an additional 50% cost reduction.
...
### Q1: How accurate are the tests generated by Gemini?

In real-world projects, initial generation accuracy is approximately 70-80%. The remaining 20-30% failures stem from application-specific business logic (correct test data specification, authentication token retrieval methods, etc.). The self-healing feature can automatically fix about half of these. Ultimately, manual intervention is needed for roughly 10-15% of generated tests.

### Q2: What is the self-healing success rate?

For cases fixable with selector changes alone, the success rate is approximately 90%. For cases involving significant page structure changes, it drops to 50-60%. Thanks to Gemini 2.5 Pro's multimodal analysis, the system can infer UI change intent from screenshot differences, achieving higher accuracy than purely text-based repair tools.

### Q3: What does API cost look like?

Test generation cost per page is roughly $0.02-0.05 when using Gemini 2.5 Pro. For a 100-page web application with weekly full regeneration, monthly costs run approximately $10-20. With caching strategies and Flash model usage for self-healing, this can be reduced to less than half.

### Q4: Can this coexist with existing test suites?

Yes, absolutely. We recommend placing generated test files in a dedicated directory like `tests/generated/` and managing them separately from hand-written tests. By splitting projects in `playwright.config.ts`, you can configure execution timing and parallelism independently for each set.
...
To deepen your understanding of the testing foundations behind AI-driven testing, "[Software Testing Techniques Drill](https://www.amazon.co.jp/dp/4817195649/?tag=pinocchio-22)" by Koichi Akiyama is an excellent resource. Learning systematic test design principles directly improves your Gemini prompt engineering as well.
