...
highlights:
  - "Fully understand Google AI SDK for Android Gradle integration and Kotlin Coroutines × Flow streaming implementation"
  - "Master Jetpack Compose multimodal AI chat UI design patterns and ViewModel architecture"
  - "Learn a systematic security design covering API key protection, secrets-gradle-plugin, and ProGuard for production deployment"
---

## Introduction: Why Android Native × Gemini API?
...
### Q1: Is the free Gemini API key sufficient for development through to production?

A: The free key from Google AI Studio (`gemini-2.0-flash`) allows 15 requests per minute and 1,500 requests per day—plenty for prototyping and personal projects. For commercial apps with real user traffic, you'll want to migrate to Google Cloud's pay-as-you-go plan. Consider Vertex AI for enterprise-grade SLAs, higher rate limits, and regional data residency options.

### Q2: How can I let users cancel a streaming response mid-generation?

A: In the example above, we store the streaming coroutine in `streamingJob` and expose a `cancelStreaming()` method that calls `streamingJob?.cancel()`. The `Flow` collected inside that job will be cancelled automatically. The key is updating the UI state (setting `isLoading = false` and clearing `pendingBotText`) in the same call so the UI reflects the cancellation immediately. Add a `CancelButton` composable that only appears when `uiState.isLoading` is true for a clean UX.

### Q3: What happens when conversation history grows large and exceeds the context window?

A: Gemini 2.0 Flash supports up to 1 million tokens, but sending large histories increases both latency and cost. A practical mitigation is a **sliding window strategy**—only include the last N turns in each API call. For smarter handling, call `model.countTokens()` before each request. If the total exceeds your threshold (say, 200,000 tokens), run a compression pass: summarize older turns into a compact "memory block" and prepend it to the trimmed history. This keeps costs predictable while preserving conversational continuity across long sessions.

### Q4: Should I use `gemini-2.0-flash` or `gemini-2.5-pro` for my Android app?

A: For most interactive chat apps, `gemini-2.0-flash` is the right default. It has the lowest latency (important for streaming feel on mobile), the lowest cost, and supports 1M token context. Upgrade to `gemini-2.5-pro` for tasks requiring deep reasoning—complex code analysis, multi-step problem solving, or high-stakes document review—where quality matters more than speed. A practical pattern is to offer both as a user-visible setting (a "Fast" vs "Thoughtful" toggle), and use Flash for drafts and Pro for final answers.

### Q5: How do I handle network errors and retries gracefully in the UI?

A: The `GoogleGenerativeAIException` thrown by the SDK carries error codes you can inspect for retry logic. For transient errors (rate limits, 503 responses), implement exponential backoff directly in the `Flow`: use `retryWhen` with a delay proportional to `attempt`. For persistent failures, surface a user-friendly error state with a "Try again" button rather than a raw error message. Always distinguish between "no network" (show an offline indicator) and "API error" (show a retry option) so users understand what action to take.
...
To deepen your Android architecture fundamentals—an essential complement to AI integration—[Android App Architecture by Lyla Fujiwara et al.](https://www.amazon.co.jp/dp/4297140748/?tag=pinocchio-22) is a highly recommended resource that provides the design thinking behind patterns like the ones covered here.
