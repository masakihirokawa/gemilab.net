...
For production path security, add a whitelist check after `path.resolve()` to ensure the resolved path starts with an allowed root directory. See [Gemini API Function Calling Complete Guide](/articles/gemini-api/gemini-api-function-calling-complete) for more advanced patterns.
