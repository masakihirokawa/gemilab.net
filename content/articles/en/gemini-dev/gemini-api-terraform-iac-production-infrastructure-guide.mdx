...
**Q: Should I use Terraform or Google Cloud Deployment Manager?**

A: Terraform is the better choice for most Gemini API projects. Its multi-cloud support means the skills and patterns transfer if you ever want to benchmark against AWS Bedrock or Azure OpenAI. The provider ecosystem is also richer — integrations with GitHub Actions, Datadog, and PagerDuty are just a `terraform import` away. Deployment Manager has a steeper learning curve for YAML/Jinja2 templating and lacks community modules at Terraform's scale.

**Q: Is Terraform Cloud required? Is there a free tier?**

A: Terraform Cloud is entirely optional — you can achieve the same remote state with GCS backend and GitHub Actions for the CI/CD. That said, Terraform Cloud's free plan (up to 500 managed resources) is generous enough for most personal or small-team projects. Its "Speculative Plans" feature (posting plan output to PRs automatically) is a particularly nice quality-of-life improvement.

**Q: How do I import existing manually created resources into Terraform?**

A: Use `terraform import`. For example, to bring an existing Cloud Run service under Terraform management:

```bash
terraform import \
  google_cloud_run_v2_service.gemini_app \
  projects/YOUR_PROJECT_ID/locations/us-central1/services/YOUR_SERVICE_NAME
```

After importing, run `terraform plan` and adjust your HCL until the diff shows no changes. From that point forward, all modifications go through your IaC workflow.
...
If you'd like to go deeper on Terraform fundamentals, [Terraform: Up & Running by Yevgeniy Brikman](https://www.amazon.co.jp/dp/4873119685/?tag=pinocchio-22) is the definitive resource for building production-grade infrastructure at scale.
