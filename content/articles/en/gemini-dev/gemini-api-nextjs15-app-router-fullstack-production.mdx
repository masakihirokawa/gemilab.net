...
**Q1: Should I call Gemini API from a Server Component or a Client Component?**

Always call from the server — Server Components, Server Actions, or Route Handlers. Calling directly from a Client Component exposes your API key in the browser. If a client component needs AI output, fetch it through a `/api/...` Route Handler that enforces authentication and rate limiting.

**Q2: When should I use Edge Runtime vs. Node.js Runtime?**

Use Edge Runtime for Streaming responses with `streamText` — it has lower cold-start latency and is globally distributed. Switch to Node.js Runtime when you need full Node.js APIs (like `fs`) or database clients that don't support the edge (e.g., some Prisma adapters).

**Q3: How do I keep Gemini 2.5 Pro API costs under control?**

Three strategies work well together: (1) Use Flash for high-frequency, simpler tasks and Pro only for complex ones. (2) Cache repeated queries with `unstable_cache`. (3) Use Context Caching for system prompts and long documents you reuse often. Also monitor token usage in `onFinish` and set `maxTokens` guardrails. See the [Gemini API cost optimization guide](/en/articles/gemini-api/gemini-api-cost-optimization-complete-guide) for a deeper breakdown.

**Q4: How do I handle multi-user chat history?**

The `useChat` hook accepts an `initialMessages` prop. Store messages in a database keyed by session ID and user ID, then pass the conversation history as `initialMessages` when the page loads. Pair this with NextAuth.js to associate conversations with authenticated users.

**Q5: Can I use Gemini API inside a Cloudflare Worker (not OpenNext)?**

Yes, but the standard `@google/generative-ai` Node.js SDK may require polyfills. The `@ai-sdk/google` package from Vercel works cleanly in both Node.js and edge environments (including Cloudflare Workers), making it a safer choice for cross-platform deployments.
...
To deepen your understanding of Next.js architecture, [Real-World Next.js by Michele Riva](https://www.amazon.co.jp/dp/180107349X/?tag=pinocchio-22) covers the App Router model and production deployment patterns in depth.
