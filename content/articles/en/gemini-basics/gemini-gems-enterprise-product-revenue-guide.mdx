...
## Fitting the Quote on One Page — How I Actually Build the Price Sheet

Even with the principles settled, what goes on page one of the quote determines how fast you hear back. My template puts three blocks on a single page: hours saved, what gets delivered, and price. A quote that is nothing but numbers turns into a "is this expensive or cheap" debate the moment it circulates internally.

The cost side that justifies the price gets built up like this. The internal rate is derived from opportunity cost — what the same hours would earn if spent on my own apps.

<table><thead><tr><th>Work</th><th>Hours</th><th>Internal rate</th><th>Subtotal</th></tr></thead><tbody><tr><td>Intake interview and workflow inventory</td><td>4h</td><td>$60</td><td>$240</td></tr><tr><td>Custom instruction design</td><td>12h</td><td>$60</td><td>$720</td></tr><tr><td>Test case authoring (30 cases)</td><td>8h</td><td>$60</td><td>$480</td></tr><tr><td>Full delivery documentation</td><td>6h</td><td>$60</td><td>$360</td></tr><tr><td><strong>Total</strong></td><td><strong>30h</strong></td><td>—</td><td><strong>$1,800</strong></td></tr></tbody></table>

Trying to recover the full $1,800 from the first customer stalls the conversation. I assume the same Gem will be sold across three companies in one industry, which puts my calculated floor at one third of cost — $600. But the minimum price from the previous section is $1,000, so $1,000 is what I quote. The floor that comes from positioning is often higher than the floor that comes from arithmetic.

When a discount is requested, I hold the price and trim scope instead: 30 test cases down to 15, support response from next business day to three business days. Leaving the removed items visible on the quote gives you something to restore at renewal. Cut the number alone and that number becomes next year's baseline.
...
Here are the thresholds I actually hold, and the first place I look when one breaks:

<table><thead><tr><th>Metric</th><th>Operating threshold</th><th>First place I look when it breaks</th></tr></thead><tbody><tr><td>Monthly churn</td><td>3% or below</td><td>The last 90 days of support tickets — dissatisfaction shows up as questions before it shows up as cancellation</td></tr><tr><td>Net ARR change</td><td>Positive every month</td><td>Whether new business stalled or upsell stalled</td></tr><tr><td>Support hours per customer / month</td><td>Under 2 hours</td><td>Repeat questions, which usually mean the delivered docs are thin</td></tr><tr><td>Usage in the 90 days before renewal</td><td>Flat or better vs. prior quarter</td><td>Staff changes and internal reorganizations</td></tr></tbody></table>

I added that fourth metric later, because I wanted to stop learning about churn after the fact. One customer's usage had nearly stopped a month before renewal, and when I asked why, the person who owned the workflow had moved to another team. It wasn't dissatisfaction with the product — the handoff simply never happened. Now I check usage 90 days out from every renewal, and if it has dropped I offer a free 30-minute handoff session. That one habit changed the tone of renewal conversations.

Build a dashboard for all of this from day one, and operating decisions don't slow down as you scale.

## A Three-Month Plan to Sell Your First Gem Product

Here is everything above, mapped onto the timeline of someone starting from zero.
...
## Shipping Every Customer's Gem From One Source — Base Plus Overlay

Somewhere past the third contract, I noticed how messy my directory had become. Client A version, Client B version, Client B version fixed, Client B version fixed 2. Every single wording change made it harder to tell which file was current. At one point I added a forbidden word to the base but forgot to propagate it to Client B, and the word showed up in output during their review meeting. It ended as a joke, but the cause was entirely my file management.

Since then I keep instructions as exactly two files: one base, one per-customer overlay. The base holds the shared role definition, tone rules, and forbidden words. The overlay holds internal terminology, department names, and anything specific to that company. What I ship is generated mechanically from those two files — I never keep a hand-edited final version.

Deciding where something belongs follows a fixed rule:

<table><thead><tr><th>Content</th><th>Goes in</th><th>Why</th></tr></thead><tbody><tr><td>Role definition and tone rules</td><td>Base</td><td>Without shared tone rules, the regression scores from the previous section aren't comparable across customers</td></tr><tr><td>Output format (heading structure, length caps)</td><td>Base</td><td>Maps one-to-one with the quality checklist you deliver</td></tr><tr><td>Industry-wide forbidden words</td><td>Base</td><td>A missed propagation becomes an incident</td></tr><tr><td>Internal terminology, product and department names</td><td>Overlay</td><td>Always differs per customer</td></tr><tr><td>Approval flow and titles of decision makers</td><td>Overlay</td><td>Depends on org structure</td></tr><tr><td>Experimental instructions under trial</td><td>Overlay</td><td>Mixing these into the base drags every customer along</td></tr></tbody></table>

Generation and change notices are handled by the script below. The part that pays off in daily operation is that the diff against the previously shipped version doubles as the draft of the update notice.

```python
# Build per-customer instructions from base + overlay, and turn changes into a notice
...
STATE = pathlib.Path("release_state.json")  # shipped hash per customer

def compose(customer: str) -> str:
    base = BASE.read_text(encoding="utf-8")
    overlay = (OVERLAY_DIR / f"{customer}.md").read_text(encoding="utf-8")
    # Overlay always goes last, so later instructions win on conflict
    return f"{base}\n\n# Customer-specific instructions (these take precedence)\n{overlay}\n"
...
def release(customer: str, version: str) -> dict:
    ...
    # Stop releases where not a single character changed
    if prev.get("hash") == digest:
        print(f"{customer}: no change ({prev.get('version')})")
        return prev
...
The hash comparison exists to prevent the "I fixed a line break and sent an update notice" failure mode. Once notices become noise, the changes that actually matter get skimmed past too. When a diff does appear, I paste the contents of `*_changes.txt` straight into the notice format required by the "product updates and notification" clause in the contract, add a one-paragraph summary, and send it. Removing the blank-page problem from update notices made me far less reluctant to ship improvements.

How I run regression scoring changed to match this split. Touch the base, and every customer gets re-scored. Touch only an overlay, and just that customer does. Each customer has roughly 30 test cases, and scoring one case takes two API calls — the production output plus the LLM-as-judge pass. Across five contracts, a full run is 150 cases and 300 calls, which took just under ten minutes on my setup. Paying that on every edit isn't realistic, so "full run only when the base changes" is where the practical line landed.

## Three Commitments to Bring to This Work

Design, pricing, quality, distribution — but three things have to be settled before any of that procedure matters.
